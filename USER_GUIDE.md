...
Per-attribute preprocessing for MAET input. Shifts selected attributes' values by a chosen offset, returning the transformed pre-MAET, which feeds directly into `buildExpTens`; the sweep form returns `[pm, sweep]`. Weights and `specs` pass through unchanged — only the values move. The transform is per-row: an offset is one position per row of the $K_\text{total} \times N$ position matrix, held constant across the sequence (column) axis, which is exactly what makes a uniform shift cancel inside relative attributes.
