"""Tests for build_maet / eval_maet / sim_maet — single-multiset core.

Mirror of MATLAB tests/test_maet_core.m.
"""
import numpy as np
import pytest
import warnings
...
class TestTensor:
    def test_build_eval_roundtrip(self):
        dens = mpt.build_maet([0, 4, 7], None, 0.5, 1, False, True, 12, verbose=False)
        vals = mpt.eval_maet(dens, np.arange(12), verbose=False)
    ...
    def test_normalize_pdf(self):
        ...
        dens = mpt.build_maet([0, 400, 700], None, 20, 1, False, False, 1200, verbose=False)
    ...
    def test_batch_cos_sim(self):
        ...
        s = mpt.sim_maet(
            A, None, B, None, 10, 1, False, True, 1200, verbose=False
        )
        assert s.shape == (2,)
