...
%   flat equivalent is struct('r', K, 'exch', exch(outer), 'rel',
...
%   size. Outer exch/rel are carried through so checkAnisoConstraints
...
%   See also internal.resolveSpecsForKernelCov, bindEvents.
    ...
    K = numel(tags);
    rLv = spec.r(:).';
    if ~isfield(spec, 'exch') || ~isfield(spec, 'rel')
        return
    end
    exchLv = spec.exch(:).';
    relLv = spec.rel(:).';
    if numel(rLv) ~= 2 || numel(exchLv) ~= 2 || numel(relLv) ~= 2
        return
    end
    ...
    flat = struct('r', double(K), 'exch', logical(exchLv(2)), ...
                  'rel', logical(relLv(2)));
