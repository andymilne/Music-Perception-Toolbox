...
Cross-cutting concerns – the Gaussian-kernel sum helper, the wrapped-Gaussian kernel, the Möbius package, the toolbox-wide defaults system, the cost models, the canonical-form deduplication, and the `explain_dispatch` diagnostic – sit alongside this stack and are consumed at multiple tiers. Their organization is covered in §3.
...
The terms the selectors and the figures below use, in one place.

**What is being computed.** The **inner product** $\langle T_x, T_y\rangle$ is a double sum over the two densities' tuple sets, and underlies cosine similarity, windowed similarity, the translation sweep, and Rényi-2 entropy. **Point evaluation** $T(q)$ is a single sum, the density read at a query point, and underlies the grid entropies and every plot of a tensor. **Total mass** $Z = \int T$ is the normalizer that turns $T$ into a probability density. `normalize` chooses only the denominator, never the route: `'cosine'` divides by $\sqrt{\langle X,X\rangle\langle Y,Y\rangle}$, `'oneSidedDenom'` by $\langle Y,Y\rangle$ alone, and `'none'` forms no denominator at all, returning the inner product on the canonical (physical) scale.

**The routes.**

- **Bulger enumeration (perm × comb, $r!$)** — organizes the tuple-pair sum by taking combinations on one side against permutations on the other and multiplying by $r!$, so the within-tuple symmetry is enumerated once rather than $r!$ times. It needs two sides, so it exists for the inner product only.
- **Unrestricted centres (perm × perm)** — the baseline: every tuple on one side against every tuple on the other, through the same kernel core as Bulger's route. On an *ordered* attribute the two coincide, since the permutation and combination sides are then the same set.
- **The Möbius method** — rewrites the distinct-index constraint of the tuple sum as an alternating sum over set partitions of the slot indices (Möbius inversion on the partition lattice), each partition's term factorizing across its blocks. Cost then scales with the Bell number $B_r$ and the collection size rather than with $K^r$.
- **Orbit collapse and the orbit table** — for the inner product only, many partition *pairs* contribute identically under the joint slot-permutation symmetry, so each such family is summed once with a counted multiplicity. The equivalence classes are precomputed in the orbit tables of §5, shipped for $r = 2 \ldots 8$.
- **Möbius per-attribute matrix** — the Möbius arm's unit of work: one $N_x \times N_y$ matrix of per-event-pair overlaps for each attribute, which the caller multiplies together (Figure R2).
- **Dense orbit (pw-batched)** — the orbit sum as dense pairwise-batched matrix work, the ordinary case for an absolute attribute at $r \ge 2$.
- **Sparse orbit sub-matrix** — the same sum when the kernel matrix between the two value sets is mostly zeros (large collections, non-periodic, values far apart relative to $\sigma$): the kernel is built sparse and only its non-zeros are orbit-summed.
- **Tuple-centres closed form** — for a relative attribute, the closed-form Gaussian over the tuple centres, with the comb-side restriction that removes the redundant orderings. Cheap when the tuple set is small; it computes the minimum-image reading, so it is inadmissible above the $\sigma/P$ threshold on a full-image periodic attribute.
- **Translation grid ($u$-grid)** — the alternative for a relative attribute. The relative tensor is the marginal of the absolute one over the shared translation $u$, and the partition sum factorizes only at fixed $u$, so the integral is taken over $N_u$ grid nodes.
- **Spectral (Fourier) contraction** — that $u$-integral done in the Fourier domain instead: the translation integral forces each partition's total mode index to zero, so the answer is a coefficient of a product of per-block spectra, which are closed form for a Gaussian mixture. The $u$-grid disappears and $K$ leaves the per-query cost, at the cost of a mode count $M \propto L/\sigma$. Used at $2 \le r \le 4$ when the mode grid is affordable.
- **Lattice contraction** — the same $u$-integral evaluated on the grid itself (orbit grid, pairwise-batched) where the spectral form does not apply.
- **Nested plan / contraction** — for a nested attribute, the tag tree is contracted level by level, each level choosing its own orbit reduction or enumeration, rather than the tree being flattened into one value set, which would compute a different density.
- **$\tau$-grid and line grid** — the nested counterparts of the translation grid: the quadrature over the shared shift within a co-transposition unit, on the circle ($\tau$) or on the line.
...
**Figure R1. Cosine similarity** (`cos_sim_exp_tens` / `cosSimExpTens`). The three rules above the flat selector are structural: an empty operand short-circuits, an ordered attribute removes the symmetry both decompositions exploit, and a nested attribute goes to the plan of Figure R3, which under `'auto'` may decline back to enumeration and under a named method raises instead. `normalize` does not appear in the tree because it does not choose a route: it chooses the denominator, and with it which self inner products are formed. A self product already memoized on its density is not recomputed, and rule 5 prices only the work the call will actually do.

![Figure R1: the cosine similarity dispatcher](docs/figures/routing_r1.png)

The kernel core shared by the Bulger and centres arms, per attribute, over tuple pairs in the log domain: the Gram form when non-periodic and accurate enough; the inner-`[rel]` block form when $r_\mathrm{in} > 0$; the absolute-periodic full-image $\theta$ (wrapped Gaussian, image sum or Fourier) when `wrap='full-image'` and $L > 0$; otherwise a nearest-image reduction and the quadratic form $Q$ (relative-periodic: the pairwise minimum-image wrap). Densities with $r_a = 1$ on every attribute take the direct kernel sum.

**Figure R2. The Möbius arm**, one $(N_x \times N_y)$ matrix per attribute. Reached from rule 3, 4 or 5 of the flat selector, or by an explicit `method='mobius'`. Each attribute is decided on its own, and the caller multiplies the matrices together.

![Figure R2: the Möbius arm, per attribute](docs/figures/routing_r2.png)

The centres route is blocked by measure when the attribute is relative and periodic with $\sigma/P > \mathrm{thr}(t_s)$ — it computes the minimum-image reading — and by cost otherwise; a forced `'mobius'` keeps the grid. Both routes memoize the self products under the key `mobius|`$t_s$`|choices`.

**Figure R3. The nested plan** (`_try_nested_contract` / `internal.nestedContract`). A nested attribute is contracted level by level rather than flattened. The plan estimates the cost of each attribute's admissible routes, then compares the whole plan against plain enumeration of the joint tuple pair.
...
Both self inner products are memoized on their densities (`_self_ip_cache` / the MATLAB `selfIP` map), keyed per route and per truncation width (and, on the Bulger and centres routes, per kernel precision) so that a route never reads another route's numbers. The flag the selectors consult when estimating cost (`_self_ip_memoised` / `internal.selfIpMemoised`) is nonetheless shared across routes, deliberately: estimating each route against its own memo would populate only the selected route's memo on the first call and fix the choice on the second, even where the other route, once its memo was populated, would be faster. Sharing the flag compares the two on their per-matrix costs, at the cost of one misestimated call at the crossover. The post-hoc impossible-value guard purges only the Möbius entries it distrusts.
