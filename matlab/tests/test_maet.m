...
p_be = {[1 2 3 4]};
out_be = cell(1, 3);
[out_be{1}, out_be{2}, out_be{3}] = bindEvents(p_be, [], [], 2);
results{end+1,1} = 'bindEvents: returns three values (pAttrBound, wBound, groupsBound)';
results{end,2}   = all(cellfun(@(x) ~isempty(x) || isnumeric(x), out_be));

% L = 1 is the no-op: each attribute becomes one super-attr equal to itself.
p_id = {[1 2 3], [10 20 30]};
[pb_id, ~, gb_id] = bindEvents(p_id, [], [], 1);
results{end+1,1} = 'bindEvents: L = 1 is the no-op (identity)';
results{end,2}   = numel(pb_id) == 2 && ...
                   isequal(pb_id{1}, [1 2 3]) && ...
                   isequal(pb_id{2}, [10 20 30]) && ...
                   isequal(gb_id, [1 2]);

% L = 2 non-circular: one input attribute becomes 2 super-attrs with N' = N-1.
p_b2 = {[1 2 3 4]};
[pb_b2, ~, gb_b2] = bindEvents(p_b2, [], [], 2, 'circular', false);
results{end+1,1} = 'bindEvents: L = 2 non-circular emits 2 super-attrs each 1 x (N-1)';
results{end,2}   = numel(pb_b2) == 2 && ...
                   isequal(pb_b2{1}, [1 2 3]) && ...
                   isequal(pb_b2{2}, [2 3 4]) && ...
                   isequal(gb_b2, [1 1]);

% L = 2 circular: wraparound; N' = N.
[pb_c, ~, ~] = bindEvents(p_b2, [], [], 2, 'circular', true);
results{end+1,1} = 'bindEvents: L = 2 circular emits 2 super-attrs each 1 x N (wraparound)';
results{end,2}   = numel(pb_c) == 2 && ...
                   isequal(pb_c{1}, [1 2 3 4]) && ...
                   isequal(pb_c{2}, [2 3 4 1]);

% Scalar broadcast: a single L value applies to every attribute.
p_sb = {[1 2 3], [10 20 30]};
[pb_sb, ~, gb_sb] = bindEvents(p_sb, [], [], 2);
results{end+1,1} = 'bindEvents: scalar order broadcasts to every attribute';
results{end,2}   = numel(pb_sb) == 4 && isequal(gb_sb, [1 1 2 2]);

% Per-attribute orders (length A) with trailing-drop alignment.
p_pa = {[1 2 3 4], [10 20 30 40]};
[pb_pa, ~, gb_pa] = bindEvents(p_pa, [], [], [2, 3]);
% A' = 2 + 3 = 5, N' = N - max(L) + 1 = 4 - 3 + 1 = 2.
results{end+1,1} = 'bindEvents: per-attribute orders, trailing-drop to common N''';
results{end,2}   = numel(pb_pa) == 5 && ...
                   isequal(pb_pa{1}, [1 2]) && ...
                   isequal(pb_pa{2}, [2 3]) && ...
                   isequal(pb_pa{3}, [10 20]) && ...
                   isequal(pb_pa{4}, [20 30]) && ...
                   isequal(pb_pa{5}, [30 40]) && ...
                   isequal(gb_pa, [1 1 2 2 2]);

% Length-G vector (G != A): broadcast within each group.
p_pg = {[1 2 3], [10 20 30], [100 200 300]};
groups_pg = [1 1 2];   % A = 3, G = 2
[pb_pg, ~, gb_pg] = bindEvents(p_pg, [], groups_pg, [2 1]);
% A' = 2 + 2 + 1 = 5; N' = 3 - 2 + 1 = 2.
results{end+1,1} = 'bindEvents: length-G vector broadcasts within group';
results{end,2}   = numel(pb_pg) == 5 && ...
                   isequal(pb_pg{1}, [1 2]) && ...
                   isequal(pb_pg{2}, [2 3]) && ...
                   isequal(pb_pg{3}, [10 20]) && ...
                   isequal(pb_pg{4}, [20 30]) && ...
                   isequal(pb_pg{5}, [100 200]) && ...
                   isequal(gb_pg, [1 1 1 1 2]);

% Cell-form per-group, with empty entries treated as L = 1 no-op.
[pb_cf, ~, gb_cf] = bindEvents(p_pg, [], groups_pg, {2, []});
results{end+1,1} = 'bindEvents: cell form (scalar in group, empty = L=1 no-op)';
results{end,2}   = numel(pb_cf) == 5 && ...
                   isequal(pb_cf{5}, [100 200]) && ...
                   isequal(gb_cf, [1 1 1 1 2]);

% Cell-form per-attribute within group.
p_cf2 = {[1 2 3 4], [10 20 30 40]};
groups_cf2 = [1 1];
[pb_cf2, ~, ~] = bindEvents(p_cf2, [], groups_cf2, {[2 3]});
results{end+1,1} = 'bindEvents: cell form per-attribute within group';
results{end,2}   = numel(pb_cf2) == 5 && ...
                   isequal(pb_cf2{1}, [1 2]) && ...
                   isequal(pb_cf2{5}, [30 40]);

% K_a > 1 attributes are accepted (preserve K_a in each super-attr).
p_k2 = {[1 2 3 4; 10 20 30 40]};   % K_a = 2, N = 4
[pb_k2, ~, ~] = bindEvents(p_k2, [], [], 2);
results{end+1,1} = 'bindEvents: K_a > 1 input preserves K_a per super-attr';
results{end,2}   = numel(pb_k2) == 2 && ...
                   isequal(size(pb_k2{1}), [2 3]) && ...
                   isequal(pb_k2{1}, [1 2 3; 10 20 30]) && ...
                   isequal(pb_k2{2}, [2 3 4; 20 30 40]);

% K_a = 0 (empty attribute) is hard-rejected.
p_empty_b = {zeros(0, 3)};
results{end+1,1} = 'bindEvents: K_a = 0 attribute errors (emptyAttribute id)';
results{end,2}   = throwsErrorWithId( ...
    @() bindEvents(p_empty_b, [], [], 2), ...
    'bindEvents:emptyAttribute');

% L > N non-circular errors (window too large).
results{end+1,1} = 'bindEvents: L > N non-circular errors (windowTooLarge id)';
results{end,2}   = throwsErrorWithId( ...
    @() bindEvents({[1 2 3]}, [], [], 4), ...
    'bindEvents:windowTooLarge');

% L > N circular also errors.
results{end+1,1} = 'bindEvents: L > N circular errors (windowTooLarge id)';
results{end,2}   = throwsErrorWithId( ...
    @() bindEvents({[1 2 3]}, [], [], 4, 'circular', true), ...
    'bindEvents:windowTooLarge');

% L = 0 errors (must be >= 1).
results{end+1,1} = 'bindEvents: L = 0 errors (badBindOrders id)';
results{end,2}   = throwsErrorWithId( ...
    @() bindEvents({[1 2 3]}, [], [], 0), ...
    'bindEvents:badBindOrders');

% Scalar weight stays scalar (broadcast in buildExpTens gives the product).
p_w = {[1 2 3 4]};
[~, wb_s, ~] = bindEvents(p_w, 0.7, [], 2);
results{end+1,1} = 'bindEvents: scalar weight passes through as scalar';
results{end,2}   = isequal(wb_s, 0.7);

% Event-dependent weight is sliced per lag.
[~, wb_e, ~] = bindEvents(p_w, {[0.1 0.2 0.3 0.4]}, [], 2);
results{end+1,1} = 'bindEvents: event-dependent weight sliced per lag';
results{end,2}   = iscell(wb_e) && numel(wb_e) == 2 && ...
                   isequal(wb_e{1}, [0.1 0.2 0.3]) && ...
                   isequal(wb_e{2}, [0.2 0.3 0.4]);

% Super-attrs stay in source group (group count unchanged, membership expands).
p_sg = {[1 2 3], [10 20 30], [100 200 300]};
groups_sg = [1 2 1];   % A = 3, G = 2; attrs 1 and 3 in group 1, attr 2 in group 2
[~, ~, gb_sg] = bindEvents(p_sg, [], groups_sg, [2 2 3]);
results{end+1,1} = 'bindEvents: super-attrs inherit source attribute''s group';
results{end,2}   = isequal(gb_sg, [1 1 2 2 1 1 1]);

% A' = sum_a L_a.
p_ap = {[1 2 3 4], [10 20 30 40], [100 200 300 400]};
[pb_ap, ~, ~] = bindEvents(p_ap, [], [], [1 2 3]);
results{end+1,1} = 'bindEvents: A'' = sum(L_a)';
results{end,2}   = numel(pb_ap) == 1 + 2 + 3;

% D then B equals B then D (value-wise) on a single attribute.
p_db = {[0 3 7 12 18]};   % N = 5
[pd_db, ~, gd_db] = differenceEvents(p_db, [], [], 1);
[pdb_db, ~, ~]    = bindEvents(pd_db, [], gd_db, 2);
[pb_db, ~, gb_db] = bindEvents(p_db, [], [], 2);
[pbd_db, ~, ~]    = differenceEvents(pb_db, [], gb_db, 1);
results{end+1,1} = 'bindEvents: D ∘ B == B ∘ D (value-wise, single attribute)';
results{end,2}   = numel(pdb_db) == numel(pbd_db) && ...
...
results{end,2}   = iscell(p_3t_out) && numel(p_3t_out) == 2 && ...
                   ...
                   isnumeric(g_3t_out) && numel(g_3t_out) == 2;
...
% delete_input=true compacts group numbering when input is sole member.
...
% delete_input=true keeps group when input is not sole member.
