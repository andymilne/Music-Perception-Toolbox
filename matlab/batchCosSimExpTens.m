...
%   isRel, isPer, period) instead — the unified entry point dispatches
%   to the same paired-rows batched implementation when the first
%   argument is a 2-D matrix with both dimensions > 1. Existing calls
%   continue to work and produce identical results, but emit an
%   MPT:DeprecatedAPI warning on each invocation.
%
...
% First detect __internalCall and remove it from varargin (used by
% cosSimExpTens to delegate without re-emitting the deprecation warning).
internalCall = false;
toRemove = false(size(varargin));
i = 1;
while i <= numel(varargin) - 1
    if (ischar(varargin{i}) || isstring(varargin{i})) ...
            && strcmpi(varargin{i}, '__internalCall')
        internalCall = logical(varargin{i + 1});
        toRemove(i)     = true;
        toRemove(i + 1) = true;
        i = i + 2;
    else
...
% Deprecation notice (since v2.1). Suppressed for internal delegation
% from cosSimExpTens. Users with existing batchCosSimExpTens calls keep
% getting the same numerical results and the warning is informational.
if ~internalCall
