...
The MAET framework provides two preprocessing helpers that transform an input event sequence into a derived sequence before tensor construction: `differenceEvents` (replacing the sequence with inter-event differences) and `bindEvents` (gathering n consecutive events into a single super-event). Both act per group and propagate per-event weights through the toolbox's standard probability-of-perception reading. They differ in what the derived sequence represents and in their handling of multi-value attributes. `differenceEvents` is restricted to single-value ($K_a = 1$) attributes because column-wise subtraction across adjacent events requires a slot correspondence that the within-event slot exchangeability does not provide. `bindEvents` accepts any $K_a \ge 1$ because each output attribute simply carries the full slot vector of its underlying event, with no cross-event slot alignment imposed. Each operation is useful on its own. They can also be chained, in which case differencing comes first and binding second: differencing requires $K_a = 1$ input and produces $K_a = 1$ output, which `bindEvents` then accepts. The MAET built on the derived sequence then aggregates across its entries in the usual way, so the resulting density is a distribution over derived values rather than a function of position in the derived sequence.
...
`windowedSimilarity` (MATLAB) / `windowed_similarity` (Python) emits a warning on every call involving a windowed periodic group, with the message tailored to the regime. The recommended bound on the window's standard deviation is $\lambda\sigma \le P/(2\sqrt{3}) \approx 0.289 P$ — the standard deviation of a uniform distribution on $(-P/2, P/2)$, beyond which the rectangular component of the window family ceases to be localized on the circle. Within the bound (regime 1 above), the message is a brief informational notice that the line-case approximation is in use and is sub-percent-accurate across the full $(\mathrm{size}, \mathrm{mix})$ window-shape family; past the bound (regimes 2 and 3), the message is stronger, reporting the rectangular component's half-width $\phi = \lambda\sigma\sqrt{3\mathrm{mix}}$ against $P/2$ and noting that behaviour at the boundary depends on `mix` (at `mix = 1` the rectangular window is no longer localized on the circle and ceases to be useful as a window; at `mix = 0` the line-case approximation degrades smoothly). The MATLAB warning identifier is `windowedSimilarity:periodicWindowApprox`; the Python warning class is `WindowedSimilarityPeriodicApproxWarning` (registered with an `"always"` filter so it fires on every offending call, matching the MATLAB per-call behaviour). The warning is suppressible via the standard MATLAB `warning('off', 'windowedSimilarity:periodicWindowApprox')` mechanism or the Python `warnings.filterwarnings('ignore', category=WindowedSimilarityPeriodicApproxWarning)` mechanism if the user has determined that the line-case approximation is acceptable for their use case.
...
Functions are grouped by category. The sections below establish each function's role, the relationships between functions in a group, and the cross-references to the conceptual material in §3. For complete signatures, parameter lists, and the full set of optional name-value arguments, see each function's docstring (`help functionName` in MATLAB; `help(mpt.function_name)` in Python). Code examples below use MATLAB syntax; see [Section 4](#4-api-conventions-matlab-vs-python) for the systematic Python equivalents.
...
| `simplexVertices` | `simplex_vertices` | *Preprocessing:* equidistant vertex coordinates for categorical MAET attributes |
| `estimateCompTime` | `estimate_comp_time` | *Utility:* micro-benchmark-based computation time estimate |

**Call forms.** All four core functions accept both a struct-style call (passing a precomputed `ExpTensDensity` or `MaetDensity` from `buildExpTens`) and a raw-arguments call (passing the underlying `p, w, sigma, r, isRel, isPer, period` directly). The raw form is preferred for one-shot calls; the struct form is preferred when the same density is reused or when working with `WindowedMaetDensity` operands. Multi-attribute (MAET) calls supply a cell array of per-attribute matrices as `p` and per-group vectors for `sigma`, `r`, `isRel`, `isPer`, `period`, together with a `groups` mapping (omittable when each attribute is its own group); see §3.1 for the conceptual scope.

**List and batched modes (v2.1+).** `cosSimExpTens` accepts three top-level input shapes: two density structs (scalar), a list / cell of density structs (pairwise; broadcasts a single struct against the other operand's list), and batched-raw `M`-by-`K` matrices (per-row multisets, with vector broadcast supported). In Python, list × list calls additionally accept `mode='cartesian'` for the full `m`-by-`n` cross-product. The deprecated `batchCosSimExpTens` / `batch_cos_sim_exp_tens` is the legacy face for batched-raw mode; new code should call `cosSimExpTens` directly.

**Windowing.** `windowTensor` wraps a `MaetDensity` with a post-tensor window specification (`size`, `mix`, `centre`); no math runs at construction time. `evalExpTens` accepts the resulting `WindowedMaetDensity` and evaluates the underlying density multiplied pointwise by the window function. `windowedSimilarity` produces a sliding-window similarity profile against query offsets; any `centre` field in the spec is ignored because offsets replace it. The output is a magnitude-aware *windowed similarity*, not strict cosine similarity — see §3.1 ("Magnitude-aware normalisation, not strict cosine similarity") and ("Periodic groups: line-case approximation") for the conceptual rationale and the periodic-window approximation warning. `cosSimExpTens` does not accept `WindowedMaetDensity` operands; use `windowedSimilarity` for windowed cases.

**MAET preprocessing pipeline.** `differenceEvents` replaces selected groups' event sequences with their k-th finite differences (rolling-product weight propagation; shortest-arc wrapping for periodic groups). `bindEvents` slides a width-n window across an event sequence and emits each window as an n-attribute super-event (so that lag-j slots become attribute j of a single group). Composed in sequence, `differenceEvents → bindEvents → buildExpTens → entropyExpTens` produces n-tuple entropy on the integer-step grid (recovering Milne & Dean 2016 at σ → 0) and extends it to weighted events, non-periodic domains, and Gaussian-smoothed continuous-position inputs (Milne 2024). The convenience function `nTupleEntropy` (§6.5) wraps this pipeline. Both preprocessing primitives require K_a = 1 per attribute; see §3.1 (Cross-event preprocessing) for the rationale and the voices-as-attributes pipeline for polyphonic analyses. `simplexVertices` produces an N-by-(N−1) coordinate matrix supplying equidistant vertex positions for the simplex-coded encoding of an N-level categorical attribute (voice identity, instrument, mode), with the corresponding group's σ controlling level discriminability.
...
Adds harmonic, stretched (Sethares), frequency-linear, stiff, or custom partials to each pitch; output weights are the product of each pitch's original weight and the spectral weight of each partial. `'powerlaw'` and `'geometric'` weight families are supported. The `'units'` name-value argument switches between cents (default 1200/octave) and semitones (12/octave).
...
These functions take pitches in cents as absolute pitches (not pitch classes); they transpose internally so the lowest pitch is 0. `roughness` is the exception: it takes frequencies in Hz (use `convertPitch` if needed). All accept a `'spectrum'` name-value pair for spectral enrichment via `addSpectra`; when using empirical peaks from `audioPeaks`, no further enrichment is needed since the peaks already represent the full spectrum.
...
| MATLAB | Python | Description |
|:---|:---|:---|
| `dftCircular` | `dft_circular` | Argand DFT of weighted points on a circle |
| `dftCircularSimulate` | `dft_circular_simulate` | Monte Carlo DFT under independent Gaussian positional jitter |
| `balanceCircular` | `balance` | Balance quotient 1 − \|F(0)\| (closed-form at σ = 0, Monte Carlo at σ > 0) |
| `evennessCircular` | `evenness` | Evenness quotient \|F(1)\| (closed-form at σ = 0, Monte Carlo at σ > 0) |

These functions apply equally to pitches (where the circle is one octave or other period) and to positions (where the circle is one rhythmic cycle or other periodic domain). All four accept weighted events; `evennessCircular` uses uniform weights internally, following Milne et al. (2017). At σ > 0, the Monte Carlo paths return the expected magnitude under positional jitter, with an optional second return giving the across-draw standard deviation (`[m, sd] = balanceCircular(...)` in MATLAB; `mpt.balance(..., return_std=True)` in Python). The closed-form mean of `balanceCircular` at perfectly balanced sets exhibits a positive Rayleigh-style bias: $E[|\widetilde{F}(0)|]$ is strictly positive when $F(0) = 0$ because magnitudes of complex Gaussians with zero mean are Rayleigh-distributed. See §3.5 ("Two scalars, two balance-related questions") for the operational distinction between this scalar and `projCentroid`'s `centMag` (the magnitude of the complex mean centroid).
...
All functions in this group operate on multisets of pitches or positions distributed around a periodic cycle and are applicable to both scales and rhythms. The first three (`coherence`, `sameness`, `nTupleEntropy`) take integer positions and an integer period; the remaining five also accept optional query points for evaluation at non-integer positions.

**Closed-form σ.** `coherence` and `sameness` accept an optional positional-jitter σ with a closed-form soft-equality kernel (Gaussian-CDF for `coherence`, Gaussian-match for `sameness`). The `sigmaSpace` flag (`'position'` default, propagating per-pair variance through shared endpoints; or `'interval'`, $V = 2\sigma^2$ uniformly) controls the variance semantics; see §3.5 (Soft sigma semantics). Float positions and a float period are accepted when σ > 0. At σ > 0 the strict / non-strict tie behaviour is replaced by the soft path's natural averaging.

**nTupleEntropy as a pipeline wrapper.** As of v2.1.0, `nTupleEntropy` is a thin wrapper around the `differenceEvents → bindEvents → entropyExpTens` pipeline (see §6.1 and §3.1). With default arguments it exactly replicates Milne & Dean's (2016) discrete formulation, and σ > 0 gives the smoothed extension of Milne (2024). Call the underlying primitives directly for non-integer values, weighted events, non-periodic domains, or to obtain the n-tuple density itself for similarity comparison and other MAET operations. At n = 1, `sigmaSpace = 'position'` with σ produces identical entropy to `sigmaSpace = 'interval'` with σ√2; at n ≥ 2 with `sigmaSpace = 'position'`, the current implementation uses the marginal-matched approximation ($\sigma_\mathrm{eff} = \sigma\sqrt{2}$, slots independent), flagged with a one-time warning.

**`projCentroid` and `balanceCircular`: two scalars.** `projCentroid` (Fourier-based, continuous-position; analytical under σ > 0 because $F(0)$ is permutation-invariant) returns the projection vector together with `centMag` $= \alpha_1 \cdot |F(0)| = |E[\widetilde{F}(0)]|$ (the *magnitude of the complex mean centroid*), with $\alpha_1 = \exp(-2\pi^2\sigma^2/P^2)$, and the centroid phase. `balanceCircular(p, w, P, \sigma)` instead returns the *mean centroid magnitude* $E[|\widetilde{F}(0)|]$, which picks up the Rayleigh-style positive bias mentioned in §6.4. The two scalars answer two different questions about positional jitter; see §3.5 for the operational distinction.

**Pulse-level measures.** `circApm` returns the period × period autocorrelation phase matrix R, the metrical weight profile rPhase (column sum), and the circular autocorrelation rLag (row sum). `edges` returns absolute and signed edge weights via convolution with the first derivative of a von Mises kernel. `meanOffset` formalizes and generalizes Huron's (2008) "average pitch height" by returning a per-position value around the circle (the same concept appears as "mode height" in Hearne 2020 and Tymoczko 2023). `markovS` is the optimal S-step Markov predictor (default S = 3), due originally to David Bulger.

### 6.6 Ordered sequences

Utility functions for analyses of event sequences over time. Position-sensitive similarity of two sequences is computed via the core tensor functions on pitch-and-time-attributed multi-attribute tensors; see §3.1 and §6.1 for `buildExpTens`, `cosSimExpTens` (including its batched-raw and list dispatch forms for many-pair scanning), `windowTensor`, and `windowedSimilarity`.
...
**`continuity`** returns the expected length and signed magnitude of the backward same-direction run leading up to each query, under Gaussian pitch uncertainty. `count` is non-negative, `magnitude` is signed (positive for ascending trends, negative for descending), and the ratio `magnitude / count` is a trend-slope measure. Modes `'strict'` (θ = 0) and `'lenient'` (θ = −1) set the break threshold; an explicit `'theta'` in [−1, +1] overrides. Optional per-event salience weights scale each interval's contribution by the difference-event salience $w_k \cdot w_{k+1}$ (the same rolling-product rule as `differenceEvents` at order 1); the break threshold acts on the unweighted sign-product so weights modulate contribution size without shifting the halt condition. Defined only on linearly ordered domains.

**`seqWeights`** applies a position-weighting profile to an existing weight vector, returning the pointwise product. Named specs are `'flat'`, `'primacy'`, `'recency'`, `'exponentialFromStart'`, `'exponentialFromEnd'`, and `'uShape'`, with optional `'decayRate'` (default 1; zero recovers `'flat'`), `'alpha'` (default 0.5; uShape mixing — `alpha = 1` recovers `'exponentialFromStart'`, `alpha = 0` recovers `'exponentialFromEnd'`), and `'t'` (strictly-increasing time index so decay operates over elapsed time from the relevant endpoint). The output length N is inferred from `numel(w)` when `w` is a non-empty non-scalar vector; it must be supplied explicitly via the `'N'` name-value argument when `w` is empty or scalar. The returned vector can be passed as the event weights of `buildExpTens` (including in its MAET form as per-event weights for a time-attributed tensor), or fed into `cosSimExpTens` via `addSpectra` for a position-weighted SPCS computation.
...
`convertPitch` converts between seven scales (`'hz'`, `'midi'`, `'cents'`, `'mel'`, `'bark'`, `'erb'`, `'greenwood'`), all routing through Hz, and is vectorised over scalars, vectors, and matrices. The `'cents'` scale is absolute MIDI cents (A4 = 6900, middle C = 6000), not relative interval cents.

`audioPeaks` reads an audio file, computes the magnitude spectrum, and extracts peaks, returning frequencies in Hz and normalised amplitudes in [0, 1]. With σ > 0, the spectrum is resampled onto a uniform log-frequency (cents) grid before smoothing so the Gaussian kernel has a fixed perceptual width at all frequencies; partials separated by more than approximately 2σ cents are individually resolved, while closer partials are merged into a single peak. Smoothing is useful for audio with vibrato or frequency jitter; it is unnecessary for steady-state tones since the downstream toolbox functions already apply their own Gaussian smoothing via the expectation tensor framework.
