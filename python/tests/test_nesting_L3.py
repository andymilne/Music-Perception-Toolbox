...
level, innermost-grouping first) and per-level ``r``/``exch``/``rel``
...
def _spec(rel):
    return [dict(r=[2, 2, 2], exch=[True, True, False], tags=_TAGS3, rel=rel)]
...
def test_L3_absolute_dim_is_full_D():
    d = build_maet(_P3, None, specs=_spec(None), **_KW)
    assert d.dim == 8                               # D = 2*2*2


def test_L3_outer_dim_is_D_minus_one():
    d = build_maet(_P3, None, specs=_spec([0, 0, 1]), **_KW)
    assert d.dim == 7                               # whole-tuple all-ones removed


def test_L3_outermost_string_matches_vector():
    dv = build_maet(_P3, None, specs=_spec([0, 0, 1]), **_KW)
    ds = build_maet(_P3, None, specs=_spec('outermost'), **_KW)
...
def test_L3_absolute_cosine_self_match():
    d = build_maet(_P3, None, specs=_spec(None), **_KW)
    assert float(sim_maet(d, d, verbose=False)) == pytest.approx(1.0, abs=1e-9)
...
def test_L3_outer_is_global_transposition_invariant():
    ...
    d = build_maet(_P3, None, specs=_spec([0, 0, 1]), **_KW)
    dT = build_maet([_P3[0] + 5.0], None, specs=_spec([0, 0, 1]), **_KW)
    assert float(sim_maet(d, dT, verbose=False)) == pytest.approx(1.0, abs=1e-9)


def test_L3_absolute_not_transposition_invariant():
    d = build_maet(_P3, None, specs=_spec(None), **_KW)
    dT = build_maet([_P3[0] + 5.0], None, specs=_spec(None), **_KW)
    assert float(sim_maet(d, dT, verbose=False)) < 0.999
...
def test_L3_renyi2_runs_all_projections():
    ...
    for rel in (None, [1, 0, 0], [0, 1, 0], [0, 0, 1]):
        d = build_maet(_P3, None, specs=_spec(rel), **_KW)
        h = float(entropy_maet(d, method="renyi2", verbose=False))
...
def test_L3_renyi2_decreases_with_finer_quotient():
    ...
    h = [float(entropy_maet(build_maet(_P3, None, specs=_spec(rel),
                                               **_KW),
...
def test_L3_renyi2_matches_grid_integral():
    ...
    d = build_maet(_P3, None, specs=_spec([1, 0, 0]), **_KW)
    h_analytic = float(entropy_maet(d, method="renyi2", base=np.e,
                                        verbose=False))
    ...
    vals = mpt.eval_maet(d, X, verbose=False)
...
def _build(rel, p=_P3):
    return build_maet(p, None, specs=_spec(rel), **_KW)
...
def test_L3_inner_is_per_chord_transposition_invariant():
    ...
    assert float(sim_maet(d, dC, verbose=False)) == pytest.approx(1.0, abs=1e-6)
...
def test_L3_intermediate_is_per_bar_invariant_not_per_chord():
    ...
    assert float(sim_maet(d, dB, verbose=False)) == pytest.approx(1.0, abs=1e-6)
    assert float(sim_maet(d, dC, verbose=False)) < 0.5    # finer: not invariant
...
def test_L3_outer_invariant_global_not_per_bar():
    ...
    assert float(sim_maet(d, dG, verbose=False)) == pytest.approx(1.0, abs=1e-6)
    assert float(sim_maet(d, dB, verbose=False)) < 0.999  # coarser unit only
...
def test_L3_one_d_tags_rejected_for_deep_nesting():
    bad = [dict(r=[2, 2, 2], exch=[True, True, False],
                tags=np.zeros(8, dtype=int), rel=None)]
    with pytest.raises(ValueError):
        build_maet(_P3, None, specs=bad, **_KW)
...
def test_L3_scalar_rel_rejected_when_nested():
    with pytest.raises(ValueError):
        build_maet(_P3, None, specs=_spec(True), **_KW)


def test_L3_infeasible_read_errors():
    """r asks for more groups than the values provide."""
    bad = [dict(r=[2, 2, 3], exch=[True, True, False], tags=_TAGS3, rel=None)]
    with pytest.raises(ValueError):
        build_maet(_P3, None, specs=bad, **_KW)
...
def test_bind_deepens_nested_to_L3():
    ...
    p1, w1, s1 = mpt.unpack_pre_maet(mpt.bind_events(p, None, 2))         # -> L=2
    p2, w2, s2 = mpt.unpack_pre_maet(mpt.bind_events(p1, w1, 2, specs=s1))  # -> L=3
    sp = s2[0]
    assert np.asarray(sp["tags"]).shape == (4, 2)    # K_total=4, L-1=2 columns
    assert list(sp["r"]) == [1, 2, 2]
    assert len(sp["exch"]) == 3 and len(sp["rel"]) == 3
...
def test_bind_deepened_L3_builds_and_self_matches():
    ...
    p1, w1, s1 = mpt.unpack_pre_maet(mpt.bind_events(p, None, 2))
    p2, w2, s2 = mpt.unpack_pre_maet(mpt.bind_events(p1, w1, 2, specs=s1))
    d = build_maet(p2, w2, specs=s2, **_KW)
...
def test_bind_deepen_rel_outer_is_global_transposition_invariant():
    p = [np.array([[0., 2, 4, 5, 7, 9]])]
    p1, w1, s1 = mpt.unpack_pre_maet(mpt.bind_events(p, None, 2))
    p2, w2, s2 = mpt.unpack_pre_maet(mpt.bind_events(p1, w1, 2, specs=s1, rel_outer=True))
    assert list(s2[0]["rel"]) == [0, 0, 1]            # new outermost relative
    d = build_maet(p2, w2, specs=s2, **_KW)
    dT = build_maet([p2[0] + 5.0], w2, specs=s2, **_KW)
    assert d.dim == 3                                 # D - 1
    assert float(sim_maet(d, dT, verbose=False)) == pytest.approx(1.0, abs=1e-6)
