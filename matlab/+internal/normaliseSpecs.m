function [rVec, isRelVec, isExchVec, nestedList, names, specKernel] = ...
    ...
    %   spec struct('r',.,'rel',.,'exch',.,'name',.) (scalar r, bool
    ...
    %   or invented value, and buildMaet refuses an NA it is not given
    %   explicitly.
    %   Returns rVec/isRelVec/isExchVec (placeholders for nested entries,
    ...
    for a = 1:A
        s = specs{a};
        if ~isstruct(s)
            error('buildMaet:specsEntry', 'specs{%d} must be a struct.', a);
        end
        if isfield(s, 'name')
            names{a} = s.name;
        else
        ...
        if isfield(s, 'tags')
            nestedList{a} = s;       % nested machinery derives r/rel/exch
            rVec(a)     = 1;         % placeholder -> prod(level r)
            isRelVec(a) = false;     % placeholder -> resolved projection
            isExchVec(a) = true;      % placeholder -> per-level exch
        else
            if ~isfield(s, 'r')
                error('buildMaet:specsFlatR', ...
                      'specs{%d} (flat) must have an ''r'' field.', a);
            end
            rA = s.r(:).';
            if numel(rA) ~= 1
                error('buildMaet:specsFlatRVec', ...
            ...
            if isfield(s, 'exch'), isExchVec(a) = logical(s.exch); end
        end
    end
end
