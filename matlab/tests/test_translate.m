...
[pOut, wOut, sOut] = unpackPreMaet(translateAttributes({[0 4 7]}, [], {5}));
...
[pOut, ~, ~] = unpackPreMaet(translateAttributes({[0 4; 7 11]}, [], {10}));
...
pR = {[0 4 7 11]};
lastwarn('', '');
[pOut, ~, ~] = unpackPreMaet(translateAttributes(pR, [], {5}, 'specs', flatSpecs(pR, 'rel', true)));
[~, wid] = lastwarn();
...
pR2 = {[0 4; 7 11]};
lastwarn('', '');
[pOut, ~, ~] = unpackPreMaet(translateAttributes(pR2, [], {[10; 20]}, 'specs', flatSpecs(pR2, 'rel', true)));
[~, wid] = lastwarn();
...
pN = {[0 4 7 11 2]};
[pb, ~, sb] = unpackPreMaet(bindEvents(pN, [], 2, 'relOuter', true));   % rel = [0 1]
lastwarn('', '');
[pOut, ~, ~] = unpackPreMaet(translateAttributes(pb, [], {3}, 'specs', sb));
[~, wid] = lastwarn();
...
lastwarn('', '');
[pOut, ~, ~] = unpackPreMaet(translateAttributes({[0 4 7]}, [], {5}));   % specs [] -> absolute
[~, wid] = lastwarn();
...
P = [0 3 7 12];
[pT, ~, ~] = unpackPreMaet(translateAttributes({P}, [], {100}));
[dT, ~, ~]  = unpackPreMaet(differenceEvents(pT, [], 1));
[d0, ~, ~]  = unpackPreMaet(differenceEvents({P}, [], 1));
...
[pb2, ~, sb2] = unpackPreMaet(bindEvents(pIn2, [], 2));       % L=2 -> K_total=4
[pOut, ~, ~] = unpackPreMaet(translateAttributes(pb2, [], {[1; 2; 3; 4]}, 'specs', sb2));
