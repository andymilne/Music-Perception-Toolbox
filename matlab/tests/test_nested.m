...
%  struct('tags',..., 'r',[ri ro], 'exch',[si so], 'rel',<...>), where rel
...
%   * Partial symmetry (exch inner=1, outer=0): inner positions orbit within
...
d_old = buildMaet({[0], [7]}, [], [50 50], [1 1], ...
                     [false false], [false false], [0 0], 'verbose', false);
nsp1  = struct('tags', [0 1], 'r', [1 2], 'exch', [true false]);
d_nest = buildMaet({[0; 7]}, [], 50, 1, false, false, 0, ...
                      'nested', {nsp1}, 'verbose', false);
okp = (d_old.dim == 2) && (d_nest.dim == 2);
queries = {[0; 7], [0; 0], [3; 7], [-5; 12]};
for qi = 1:numel(queries)
    vo = evalMaet(d_old,  queries{qi}, 'verbose', false);
    vn = evalMaet(d_nest, queries{qi}, 'verbose', false);
...
nsp2 = struct('tags', [0 0 1 1], 'r', [2 2], 'exch', [true false]);
d2 = internal.ensureMaetExpensive( ...
    buildMaet({[0; 4; 7; 11]}, [], 50, 1, false, false, 0, ...
...
nsp3 = struct('tags', [0 0 1 1], 'r', [2 1], 'exch', [false false]);
d3 = internal.ensureMaetExpensive( ...
    buildMaet({[0; 4; 7; 11]}, [], 50, 1, false, false, 0, ...
...
nspOut = struct('tags', [0 0 1 1], 'r', [2 2], 'exch', [true false], ...
                'rel', 'outermost');
dO  = buildMaet({[0; 4; 7; 11]},  [], 50, 1, false, false, 0, ...
                   'nested', {nspOut}, 'verbose', false);
dOS = buildMaet({[5; 9; 12; 16]}, [], 50, 1, false, false, 0, ...
                   'nested', {nspOut}, 'verbose', false);
dOe  = internal.ensureMaetExpensive(dO);
dOSe = internal.ensureMaetExpensive(dOS);
okOut = (dO.dim == 3) ...
    && isequal(sort(dOe.Centres{1}, 2), sort(dOSe.Centres{1}, 2)) ...
    && abs(simMaet(dO, dOS, 'verbose', false) - 1) < 1e-9;
...
nspAbs = struct('tags', [0 0 1 1], 'r', [2 2], 'exch', [true false]);
dA  = buildMaet({[0; 4; 7; 11]},  [], 50, 1, false, false, 0, ...
                   'nested', {nspAbs}, 'verbose', false);
dAS = buildMaet({[5; 9; 12; 16]}, [], 50, 1, false, false, 0, ...
                   'nested', {nspAbs}, 'verbose', false);
results{end+1,1} = 'nested: absolute is not transposition invariant';
results{end,2}   = (dA.dim == 4) ...
                   && (simMaet(dA, dAS, 'verbose', false) < 0.999);


% --- rel=[0 1] (outermost level) equals rel='outermost' ---
nspV = struct('tags', [0 0 1 1], 'r', [2 2], 'exch', [true false], 'rel', [0 1]);
dV = internal.ensureMaetExpensive( ...
    buildMaet({[0; 4; 7; 11]}, [], 50, 1, false, false, 0, ...
...
nspIn = struct('tags', [0 0 1 1], 'r', [2 2], 'exch', [true false], ...
               'rel', 'innermost');
dIn = buildMaet({[0; 4; 7; 11]}, [], 50, 1, false, false, 0, ...
                   'nested', {nspIn}, 'verbose', false);
dInPE = buildMaet({[10; 14; 107; 111]}, [], 50, 1, false, false, 0, ...
                     'nested', {nspIn}, 'verbose', false);  % per-event shift
% Interval change must register at a resolving sigma.
dInA = buildMaet({[0; 4; 7; 11]}, [], 1, 1, false, false, 0, ...
                    'nested', {nspIn}, 'verbose', false);
dInB = buildMaet({[0; 5; 7; 11]}, [], 1, 1, false, false, 0, ...
                    'nested', {nspIn}, 'verbose', false);
okInner = (dIn.dim == 2) ...
    && (abs(simMaet(dIn, dInPE, 'verbose', false) - 1) < 1e-9) ...
    && (simMaet(dInA, dInB, 'verbose', false) < 0.95);
...
for q = {[4; 4], [4; -4], [0; 0], [3; 5], [-4; 4]}
    okJoin = okJoin && abs( ...
        evalMaet(dIn,  q{1}, 'verbose', false) ...
      - evalMaet(dRef, q{1}, 'verbose', false)) < 1e-11;
end
...
ws = warning('off', 'buildMaet:nestedRelSubsumption');
dSub = buildMaet({[0; 4; 7; 11]}, [], 50, 1, false, false, 0, ...
        'nested', {struct('tags', [0 0 1 1], 'r', [2 2], 'exch', [true false], ...
                          'rel', [1 1])}, 'verbose', false);
warning(ws);
results{end+1,1} = 'nested: rel=[1 1] subsumes to inner (dim 2)';
results{end,2}   = (dSub.dim == 2) ...
                   && (abs(simMaet(dIn, dSub, 'verbose', false) - 1) < 1e-9);


% --- Guards ---
okScalar = false;
try
    buildMaet({[0; 7]}, [], 50, 1, false, false, 0, ...
        'nested', {struct('tags', [0 1], 'r', [1 2], 'exch', [true false], ...
...
okUserRel = false;
try
    buildMaet({[0; 7]}, [], 50, 1, true, false, 0, ...
        'nested', {struct('tags', [0 1], 'r', [1 2], 'exch', [true false])}, ...
        'verbose', false);
catch
    okUserRel = true;
...
okTags = false;
try
    buildMaet({[0; 4]}, [], 50, 1, false, false, 0, ...
        'nested', {struct('tags', [0 0], 'r', [1 2], 'exch', [true false])}, ...
        'verbose', false);
catch
    okTags = true;
end
...
pAttr  = {[0 4; 7 11]};
d_def  = buildMaet(pAttr, [], 50, 2, false, false, 0, 'verbose', false);
d_none = buildMaet(pAttr, [], 50, 2, false, false, 0, ...
                      'nested', {[]}, 'verbose', false);
okflat = (d_def.dim == d_none.dim);
for qi = 1:numel(queries)
    if numel(queries{qi}) == 2
        okflat = okflat && abs( ...
            evalMaet(d_def,  queries{qi}, 'verbose', false) ...
          - evalMaet(d_none, queries{qi}, 'verbose', false)) < 1e-14;
    end
end
