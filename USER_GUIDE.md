...
**Event translation.** `translateEvents` shifts every value of every attribute belonging to selected groups by a per-group offset, leaving non-selected groups unchanged. The output `pAttrTranslated` feeds directly into `buildExpTens` without further massaging; weights pass through unchanged, since uniform translation does not affect any event's probability of perception. The shift is applied uniformly to every slot in every event's multiset, so the operation is well-defined for any $K_a \ge 1$ — unlike windowing, where a per-event scalar weight would have to summarise multiple slots' distances from a localising centre, translation has no analogous obstruction at $K_a > 1$. For absolute non-periodic groups the operation is `value + mu`; for absolute periodic groups it is `(value + mu) mod P`, with the canonical wrap to $[0, P)$ being purely cosmetic since the wrapped periodic Gaussian kernel of `buildExpTens` is invariant under any additive shift by a multiple of $P$. For relative groups translation is a structural no-op — a uniform shift cancels in every within-tuple difference, so the relative MAET is unchanged — and the function emits a `translateEvents:noOpRelative` (MATLAB) or `TranslateEventsNoOpWarning` (Python) and leaves the group unchanged. Multiple groups can be translated simultaneously in one call by supplying a per-group offset vector (or sparse dict in Python). An entire sweep over $M$ offset configurations is produced in one call by passing offsets as a $G$-by-$M$ matrix; the return is then a length-$M$ list of translated `pAttr` blocks, one per offset column. In MATLAB the vector form requires a $1$-by-$G$ row layout (or a scalar when $G = 1$); a length-$G$ column vector reads as a $G$-by-$1$ matrix and so dispatches to matrix form with $M = 1$ — the only way to specify $M = 1$ in MATLAB without an extra flag.
...
Cross-event preprocessing for MAET input. Shifts every value of every attribute in selected groups by a per-group offset, returning a transformed `pAttrTranslated` that feeds directly into `buildExpTens`. Weights are unaffected and are not part of the signature. Semantics are by group geometry: absolute non-periodic groups are translated as `value + mu`; absolute periodic groups as `(value + mu) mod P`; relative groups pass through unchanged and emit a single `translateEvents:noOpRelative` (MATLAB) / `TranslateEventsNoOpWarning` (Python) per call. `offsets` accepts two shapes — a $1$-by-$G$ row vector in MATLAB (or a sparse `{group_index: mu}` dict / 1-D length-$G$ ndarray in Python) returning a single translated `pAttr`, or a $G$-by-$M$ matrix returning a length-$M$ list of translated `pAttr` blocks (matrix-form sweep). NaN entries in either shape mean "do not translate this group" on that column. The matrix form's output is the natural input to the raw-MA scalar-vs-list mode of `cosSimExpTens` (§6.1 above) for a one-call sliding-comparison sweep. In MATLAB, a length-$G$ column vector reads as a $G$-by-$1$ matrix — i.e. matrix form with $M = 1$, not vector form; Python is shape-driven and has no analogous ambiguity. See §3.1 (Event translation) for the conceptual framing of pre-tensor versus post-tensor sliding.
