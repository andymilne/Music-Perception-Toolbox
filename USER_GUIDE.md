...
Three analytical decompositions are available across the v2.2 surface:
...
The two consumer-level dispatchers under `'auto'` differ in one respect. `cosSimExpTens` picks between Bulger's and the Möbius method using a per-call cost model that considers $r$, $K$, $N$, and $\sigma/\text{period}$, with a **timing-probe fallback** for SA cosine similarity when the cost-model pre-screen is not decisive: the dispatcher times each candidate on a small subset of the workload and routes the full call accordingly. `evalExpTens` picks between centres and the Möbius method using the same cost-model considerations, but with **no timing probe** — the centres-path overhead is small enough that probing would cost more than it saves. Both dispatchers honour the Möbius precision guard ($K - r \ge 2$); outside it, `'auto'` falls back to Bulger's method (cosine) or to the centres path (eval). The Bulger and Möbius methods agree to floating-point precision on inputs where both are valid (the inner-product case); the centres and Möbius methods likewise agree on the eval case in default modes (a small convention difference applies in periodic-relative mode at $\sigma/P > 0.03$, where the centres path remains the canonical output for backward compatibility).

| Method | Description |
|:---|:---|
| `'auto'` (default) | Per-call cost-model dispatcher (with timing-probe fallback for SA cosine similarity on `cosSimExpTens`; analytical-only on `evalExpTens`). Picks between the available alternatives based on $r$, $K$, $N$, and $\sigma/\text{period}$. Agrees with the explicit alternatives to floating-point precision in the regimes where both are valid. |
| `'bulger'` | Bulger's method (v2.1). Inner-product-only. Direct enumeration of ordered $r$-tuples with kernel matrix multiplication. Exact and stable for any input. |
| `'mobius'` | The Möbius method (v2.2). Closed-form analytical decomposition via Möbius inversion on the partition lattice, combined with orbit collapse in the inner-product case. Available on cosine similarity, point evaluation, and total mass. Subject to a precision guard ($K - r \ge 2$, below which the alternating partition sum is prone to **catastrophic cancellation** — see the `'cancellationThreshold'` paragraph below); the `'auto'` setting falls back to Bulger's (cosine) or centres (eval) outside this regime. |
| `'centres'` | Explicit centres-array evaluation. Point-evaluation-only (`evalExpTens` and the total-mass term inside `entropyExpTens` under `method='renyi2'`). Same machinery as v2.1 eval. |
| `'direct'` | Direct enumeration without the Möbius alternating sum. Used internally by the MA Möbius inner-product path's safe/unsafe hybrid for events with $K_\text{eff} - r < 2$. |
