...
The Music Perception Toolbox is an open-source package — available in MATLAB and Python — for computing perceptually and cognitively motivated measures of pitch similarity, consonance, and scale and rhythmic structure. It accepts inputs from symbolic pitch data or from spectral peaks extracted from audio recordings.
...
The toolbox covers three broad areas:

**(i) Similarity and complexity via expectation tensors.** Expectation tensors and their cosine similarities provide a general framework for quantifying the similarity of any two multisets of weighted points — whether those points are pitches, pitch classes, time points, or time classes — and the complexity of a single multiset. For pitch, spectral enrichment (adding harmonics via `addSpectra`) yields spectral pitch class similarity (SPCS), a powerful predictor of perceived tonal fit and similarity. The only component of this framework that is specific to pitch is the spectral enrichment itself; the underlying tensor machinery applies equally to rhythmic patterns.

**(ii) Consonance and harmonicity.** The toolbox provides several complementary measures of consonance via spectral entropy, template harmonicity, tensor harmonicity, and sensory roughness. These model two of the three factors contemporary models distinguish as contributing to consonance — roughness and harmonicity — but not the third, familiarity, which requires corpus-based or learning-based approaches.

**(iii) Scale and rhythm structure.** A suite of functions computes structural and perceptual features of multisets of points on a circle, applicable to both pitches and positions (e.g., time onsets). These include Fourier-based measures (balance and evenness), interval-structure measures (coherence, sameness, n-tuple entropy), and further predictors (edge detection, projected centroid, mean offset, autocorrelation phase matrices, Markov prediction).
...
A notated pitch (e.g., middle C) is an abstraction — when sounded on a real instrument, it produces not a pure tone but a complex tone with a spectrum of partials at frequencies approximately integer multiples of the fundamental. When working with symbolic pitch data (e.g., MIDI note numbers, scale degrees, or theoretical pitch sets), the `addSpectra` function models this acoustic reality by appending synthetic partials to each pitch in a set, returning expanded pitch and weight vectors. These expanded vectors can then be passed to the expectation tensor functions. This is unnecessary when working with empirical spectral peaks extracted from audio recordings (via `audioPeaks`), since those already represent the full spectrum. Spectral enrichment is the one component of the toolbox that is specific to pitched sounds; the expectation tensor framework itself applies to any domain.
...
- **Pitch similarity (PS):** No spectral enrichment, non-periodic domain. Compares two multisets of fundamental pitches on an unbounded pitch line (Milne, Sethares, Laney, & Sharp, 2011).
- **Pitch class similarity (PCS):** No spectral enrichment, periodic domain (period = 1200). Pitches an octave apart are identified as equivalent, but the multisets represent fundamentals only (Milne et al., 2011).
- **Spectral pitch similarity (SPS):** Non-periodic domain with spectral enrichment applied to symbolic input via `addSpectra`. When input is derived from audio spectral peaks (e.g., via `audioPeaks`), the harmonic structure is already present and enrichment is not applied. "Spectral" indicates the comparison operates over full spectra rather than fundamentals alone; "pitch" (without "class") indicates the domain is not periodic (Dean, Milne, & Bailes, 2019).
- **Spectral pitch class similarity (SPCS):** As SPS, but with a periodic domain (period = 1200). Harmonics are added to symbolic input *and* the domain wraps at the octave, so partials an octave apart reinforce each other. This is the most commonly used measure: for Western-enculturated participants, SPCS has consistently outperformed SPS at modelling perceived tonal fit, which is attributed to the perceptual importance of octave equivalence (Milne, Laney, & Sharp, 2015).

In all four cases, the standard parameters are r = 1 (monad tensor) and isRel = false (absolute). Each multiset is mapped to a density over pitch (or pitch class), and the cosine similarity between the two densities quantifies how similar they are. SPCS has been validated as a predictor of probe tone ratings and tonal hierarchies (Milne, Laney, & Sharp, 2015), perceived tonal affinity with microtonal melodies and inharmonic spectra (Milne, Laney, & Sharp, 2016), perceived triadic distance (Milne & Holland, 2016), perceived change in sound-based music (Dean, Milne, & Bailes, 2019), affect in unfamiliar chords (Smit et al., 2019), consonance across multiple datasets (Eerola & Lahdelma, 2021), cross-cultural tonal stability (Milne, Smit, Sarvasy, & Dean, 2023), individual differences in harmony perception (Eitel et al., 2024), spectral models of musical knowledge (Homer et al., 2024), and contextual tonal stability in microtonal scales (Hearne, Dean, & Milne, 2025).
...
The toolbox provides several complementary measures of consonance. Contemporary models distinguish at least three contributing factors (Harrison & Pearce, 2020; Eerola & Lahdelma, 2021): *roughness* (rapid amplitude fluctuation produced by close-frequency partials), *harmonicity* (alignment of a chord's intervals or composite spectrum with a harmonic series), and *familiarity* (culturally learned expectations). The toolbox implements models of the first two; it does not model familiarity, which requires corpus-based or learning-based approaches. These measures have been used individually and in combination to predict consonance ratings, perceived affect, and tonal stability across a variety of musical contexts including unfamiliar chords and microtonal tuning systems (Smit et al., 2019; Harrison & Pearce, 2020; Eerola & Lahdelma, 2021; Milne, Smit, Sarvasy, & Dean, 2023; Hearne, Dean, & Milne, 2025).

- **Spectral entropy** (`spectralEntropy`) measures the Shannon entropy of a chord's composite spectrum after Gaussian smoothing. A chord whose partials cluster at a small number of pitch classes — as happens when the chord's intervals are simple frequency ratios — produces a low-entropy (ordered) spectrum. Spectral entropy approximates the aggregation of all pairwise spectral pitch class similarities within a single chord: greater overlap between each pair of partials increases their SPCS and decreases the overall entropy (Milne, Bulger, & Herff, 2017; Milne, 2024). Unlike SPCS, which compares two pitch sets, spectral entropy is a property of a single (potentially composite) set.
- **Template harmonicity** (`templateHarmonicity`) cross-correlates a chord's composite spectrum with a harmonic template, returning both the maximum of the normalised cross-correlation (hMax; Milne, 2013) — the cosine similarity at the best-matching transposition — and the entropy of the cross-correlation profile (hEntropy; Harrison & Pearce, 2020), which captures whether the chord has one clear virtual fundamental or several competing ones.
- **Tensor harmonicity** (`tensorHarmonicity`) measures how probable a chord's specific ordered intervals are within a single harmonic series. It builds a relative r-ad expectation tensor from a harmonic template and evaluates it at the chord's ordered interval vector — the intervals measured from the lowest pitch to each of the remaining pitches (Smit et al., 2019).
- **Roughness** (`roughness`) computes sensory roughness from the beating of close partial pairs, using Sethares' (1993) parameterisation of Plomp and Levelt's (1965) empirical dissonance curve. The implementation extends the original model in two ways: first, pairwise roughnesses are combined via a configurable p-norm (default p = 1; higher values of p give greater weight to the roughest pairs); second, optional averaging over the number of partial pairs makes roughness values comparable across chords with different numbers of partials. Unlike the pitch-class-based measures, roughness is register-dependent and requires frequencies in Hz.
- **Virtual pitches** (`virtualPitches`) returns the full cross-correlation profile from which template harmonicity extracts its summary statistics. Peaks indicate candidate virtual pitches — the perceived fundamentals that the auditory system infers from a complex of partials.
...
- **Balance** (`balanceCircular`) measures how close the weighted centre of gravity of a set of points on a circle is to the circle's centre. It is defined as 1 − |F(0)| / Σwₖ, where F(0) is the zeroth-order DFT coefficient. Balance ranges from 0 (all weight at one point) to 1 (the centre of gravity lies exactly at the circle's centre). A perfectly balanced set need not be evenly spaced: Milne, Bulger, & Herff (2017) show that perfectly balanced patterns form a rich combinatorial class that includes highly irregular rhythms and scales.
- **Evenness** (`evennessCircular`) measures closeness to equal spacing using the normalised magnitude of the first-order DFT coefficient |F(1)|. It ranges from 0 to 1, equalling 1 only for equally spaced sets (Milne, Bulger, & Herff, 2017).
