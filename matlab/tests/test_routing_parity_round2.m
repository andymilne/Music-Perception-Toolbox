...
dAbs = buildMaet({pAbs}, {[]}, 0.7, 2, false, false, 0, 'verbose', false);
dBoth = buildMaet({pAbs, pRel}, {[], []}, [0.7 1.0], [2 1], ...
                     [false true], [false false], [0 0], 'verbose', false);
hAbs = entropyMaet(dAbs, 'method', 'renyi2', 'verbose', false);
hBoth = entropyMaet(dBoth, 'method', 'renyi2', 'verbose', false);
...
results{end, 2}   = throwsErrorWithId(@() simMaet(P1b, [], P2b, [], ...
    S2, 2, false, false, 0, false, 'verbose', false), ...
    'simMaet:batchedOrderedUnsupported');
...
for rp2_i = 1:numel(rp2_shapes)
    ...
    dx1 = buildMaet({px}, {[]}, sigA, rA, relA, perA, perVal, ...
                       'wrap', {wrapA}, 'verbose', false);
    dy1 = buildMaet({py}, {[]}, sigA, rA, relA, perA, perVal, ...
                       'wrap', {wrapA}, 'verbose', false);
    dx2 = buildMaet({px, 3.0 * ones(1, 2)}, {[], []}, [sigA 1.0], ...
                       [rA 1], [relA false], [perA false], [perVal 0], ...
                       'wrap', {wrapA, 'full-image'}, 'verbose', false);
    dy2 = buildMaet({py, 3.0 * ones(1, 2)}, {[], []}, [sigA 1.0], ...
                       [rA 1], [relA false], [perA false], [perVal 0], ...
                       'wrap', {wrapA, 'full-image'}, 'verbose', false);
    for rp2_ts = {4, Inf}
        s1 = simMaet(dx1, dy1, 'method', 'bulger', ...
            'truncationSigmas', rp2_ts{1}, 'verbose', false);
        s2 = simMaet(dx2, dy2, 'method', 'bulger', ...
            'truncationSigmas', rp2_ts{1}, 'verbose', false);
...
sDbl = simMaet(kx, ky, 'method', 'bulger', 'verbose', false);
sSgl = simMaet(kx, ky, 'method', 'bulger', ...
...
[~, kxS, ~] = simMaet(kx, ky, 'method', 'bulger', ...
                            'kernelPrecision', 'single', 'verbose', false);
[~, kxD, ~] = simMaet(kx, ky, 'method', 'bulger', 'verbose', false);
...
sy = buildMaet({randn(4, 3) * 3}, [], 0.9, 2, false, false, NaN, true, ...
                  'verbose', false);
offS = [-2 0 1.5];
[sw1, sxOut, syOut] = sweepSimMaet(sx, sy, offS, 'method', 'mixture', ...
                                         'verbose', false);
swKeys = syOut.selfIP.keys;
sw2 = sweepSimMaet(sxOut, syOut, offS, 'method', 'mixture', ...
                         'verbose', false);
...
[sN1, nxOut, nyOut] = simMaet(nx, ny, 'method', 'centres', ...
                                    'verbose', false);
sN2 = simMaet(nxOut, nyOut, 'method', 'centres', 'verbose', false);
...
sFull = simMaet(xf, yf, 'verbose', false);
sSingle = simMaet(xs, ys, 'verbose', false);
outL = simMaet({xf, xs, xf}, {yf, ys, yf}, 'verbose', false);
...
% to the total weight, and the Shannon entropy entropyMaet reports
...
hFull = entropyMaet(dFull, 'method', 'shannon', 'nPointsPerDim', ...
    nCells, 'truncationSigmas', Inf, 'verbose', false);
hSingle = entropyMaet(dSingle, 'method', 'shannon', 'nPointsPerDim', ...
    nCells, 'truncationSigmas', Inf, 'verbose', false);
...
sigS = 0.02 * rp2_P;
hFullS = entropyMaet(rp2Chord(chordB, sigS, 1, false, true, ...
    'full-image', rp2_P), 'method', 'shannon', 'nPointsPerDim', nCells, ...
    'verbose', false);
hSingleS = entropyMaet(rp2Chord(chordB, sigS, 1, false, true, ...
...
hNarrow = entropyMaet(dFull, 'method', 'shannon', 'nPointsPerDim', ...
...
function d = rp2Chord(values, sigma, r, isRel, isPer, wrap, P)
    ...
    d = buildMaet({values(:)}, {[]}, sigma, r, isRel, isPer, period, ...
                     'wrap', {wrap}, 'verbose', false);
end
