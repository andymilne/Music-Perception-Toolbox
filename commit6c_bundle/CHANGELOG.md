...
- **Window centre — global scalar broadcasting (`window_tensor` / `windowTensor`).** A numeric size-1 input to `window_spec['centre']` (Python: a Python scalar, 0-D ndarray, or numeric ndarray of size 1; MATLAB: a numeric scalar or 1×1 numeric array) is now broadcast to fill every per-attribute slot uniformly. Convenient when one window centre value is wanted everywhere — the previous API required either a length-A list/cell of full-length per-attribute vectors or a single flat dim-length vector. List/cell inputs are always interpreted structurally and never broadcast, even when size-1: a length-1 list/cell on a density with A > 1 raises rather than silently broadcasting, preserving the pre-fix contract that a wrong-length list/cell is a user error. Per-attribute scalars in cell/list form (e.g. `{5.0, 10.0}` or `[5.0, 10.0]` for two attributes with d_a > 1 each, intending one uniform centre per attribute) are likewise rejected on length mismatch by the existing flat-form / cell-form dispatcher; users wanting per-attribute uniform centres at distinct values per attribute should pass an explicit cell/list of full-length vectors. No behavioural change for any centre input that was previously accepted.

- **`seqWeights` / `seq_weights` — `'uAsym'` profile.** A three-parameter U-shape with separate decay rates for the primacy and recency components: `alpha * exp(-decayRateStart * t) + (1 - alpha) * exp(-decayRateEnd * (t_end - t))`. Useful when the data have asymmetric primacy and recency decay; reduces to `'uShape'` when `decayRateStart == decayRateEnd`. Both `decayRateStart` and `decayRateEnd` fall back to the existing `decayRate` when not supplied.

- **`seqWeights` / `seq_weights` — callable profile spec.** In addition to a named string and an explicit length-N vector, `spec` may now be a function (Python callable, MATLAB function handle) of the time vector that returns a length-N profile. Allows arbitrary user-defined profiles (hyperbolic decay, custom mixtures, etc.) to be applied with the same length validation and `w` multiplication as built-in specs.
...
- **Windowed cosine similarity — within-attribute centre symmetrisation (Python and MATLAB).** `cos_sim_exp_tens` (Python) and `cosSimExpTens` (MATLAB) now return the framework-correct (perm-perm) windowed inner product when the per-attribute `centre` vector has non-uniform components within an attribute. Previously the perm-comb evaluation form used internally for efficiency assumed within-attribute slot-symmetry of the windowed integrand; this holds when the within-attribute centre is uniform (a scalar broadcast or a vector with all entries equal — the typical use case) but breaks when the within-attribute components differ, producing a value that depended on the canonical ordering of the comb-side input multiset and on the entry order of the centre vector. The MAET density is symmetric under within-attribute coordinate permutations (JMM Theorem on the windowed inner product, remark on within-attribute symmetry of the windowed integral), so the integral depends on the centre only through its multiset within each attribute. The fix detects non-uniform within-attribute centres at the inner-product level and averages the IP over within-attribute centre permutations, restoring framework-correct behaviour. No change for uniform within-attribute centres (the common case): the fast path is byte-identical to the previous v2.1 implementation. Affects `cos_sim_exp_tens` / `cosSimExpTens` and `windowed_similarity` / `windowedSimilarity` only when called with non-uniform within-attribute centres on absolute groups of effective dimension ≥ 2 in the factorisable case (1-D and multi-D absolute).
