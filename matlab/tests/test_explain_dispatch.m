...
for xdI = 1:size(xdCases, 1)
    ...
    xdD = buildMaet(xdPts(xdK), [], xdS, xdR, xdRel, xdPer, ...
...
xdD = buildMaet(xdPts(12), [], 60, 3, true, true, xdP, 'verbose', false);
...
xdDnp = buildMaet(xdPts(12), [], 60, 3, false, false, 0, ...
                     'verbose', false);
...
xdRep = explainDispatch( ...
    buildMaet(xdPts(12), [], 15, 3, true, false, 0, 'verbose', false), ...
    200);
...
xdX = buildMaet(xdPts(12), [], 60, 3, true, true, xdP, 'verbose', false);
xdY = buildMaet(xdP * mod((1:12) * 0.61, 1).', [], 60, 3, true, true, ...
                   xdP, 'verbose', false);
xdRep = explainDispatch(xdX, xdY);
results{end+1, 1} = 'explainDispatch: a density pair explains a cosine';
results{end, 2}   = strcmp(xdRep.call, 'simMaet') && ...
...
xdBig = buildMaet(xdPts(60), [], 20, 7, false, false, 0, ...
                     'verbose', false);
...
xdRep = explainDispatch( ...
    buildMaet(xdPts(12), [], 24, 3, true, true, xdP, 'verbose', false), ...
    200);
...
xdRep = explainDispatch( ...
    buildMaet(xdPts(12), [], 15, 1, false, false, 0, 'verbose', false), ...
    200);
xdCentres = xdRep.routeMs(strcmp(xdRep.routeNames, 'centres'));
xdMobius  = xdRep.routeMs(strcmp(xdRep.routeNames, 'mobius'));
...
xdDSing = buildMaet(xdPts(12), [], 60, 3, true, true, xdP, ...
                       'wrap', 'single-image', 'verbose', false);
xdDFull = buildMaet(xdPts(12), [], 60, 3, true, true, xdP, ...
                       'verbose', false);
