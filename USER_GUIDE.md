...
The windowed inner product (`windowedSimilarity`) has a single closed-form path and accepts no `method` keyword; its dispatch announce reads `windowedSimilarity: chose 'closed-form' path.` accordingly.
...
Precomputes tuple indices, pitch matrices, and weight vectors for a weighted pitch multiset. Returns a struct (`tag = 'MaetDensity'`) that can be passed to `evalExpTens` and `cosSimExpTens`.
