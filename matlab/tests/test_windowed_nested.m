...
%  it stands in for (weightEvents / translateAttributes -> buildMaet with
%  'specs' -> simMaet / entropyMaet). To prove the geometry is read
...
for i = 1:numel(centres)
    ...
    dc = buildMaet(pc, wc, 'sigma', SIG, 'isPer', ISP, 'period', PER, ...
        'specs', sc, 'verbose', false);
    offs = {[], centres(i) - muQ};
    [pq, wq, sq] = unpackPreMaet(translateAttributes(qryP, wqryP, offs, 'specs', sbP));
    dq = buildMaet(pq, wq, 'sigma', SIG, 'isPer', ISP, 'period', PER, ...
        'specs', sq, 'verbose', false);
    ref(i) = simMaet(dc, dq, 'normalize', 'oneSidedDenom', 'verbose', false);
end
...
for i = 1:numel(centres)
    ...
    dq = buildMaet(pq, wq, 'sigma', SIG, 'isPer', ISP, 'period', PER, ...
        'specs', sq, 'verbose', false);
    refS(i) = simMaet(dc, dq, 'normalize', 'oneSidedDenom', 'verbose', false);
end
...
for i = 1:numel(eCentres)
    ...
    densH = buildMaet(pw, ww, 'sigma', SIG, 'isPer', ISP, 'period', PER, ...
        'specs', sw, 'verbose', false);
    refH(i) = entropyMaet(densH, 'method', 'renyi2', 'verbose', false);
end
