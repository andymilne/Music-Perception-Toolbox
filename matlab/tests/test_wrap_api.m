...
wrapPrevW = warning('off', 'buildMaet:absPerSingleImage');
...
pW = [0; 100; 300; 700];
wW = ones(4, 1);
dW = buildMaet({pW}, {wW}, 60, 2, false, true, wrapPERIOD, 'verbose', false);
results(end+1, :) = { ...
...
pW2 = [50; 200; 400];
wW3 = ones(3, 1);
dW = buildMaet({pW1, pW2}, {wW3, wW3}, [60 60], [2 2], ...
...
wrapBadRaised = false;
try
    buildMaet({pW1}, {wW3}, 60, 2, false, true, wrapPERIOD, ...
        'wrap', 'torus', 'verbose', false); %#ok<NASGU>
catch e
    wrapBadRaised = strcmp(e.identifier, 'buildMaet:invalidWrap');
end
results(end+1, :) = { ...
...
wrapBadRaised = false;
try
    buildMaet({pW1, pW2}, {wW3, wW3}, [60 60], [2 2], ...
        [false false], [true true], [wrapPERIOD wrapPERIOD], ...
...
for sopIter = [0.20, 0.30, 0.50]
    ...
    d1 = buildMaet({wrapPa}, {wrapW4}, sig, 2, false, true, wrapPERIOD, ...
        'verbose', false);
    d2 = buildMaet({wrapPb}, {wrapW4}, sig, 2, false, true, wrapPERIOD, ...
        'verbose', false);
    c12 = simMaet(d1, d2, 'verbose', false);
    c11 = simMaet(d1, d1, 'verbose', false);
    c22 = simMaet(d2, d2, 'verbose', false);
...
d1F = buildMaet({wrapPa}, {wrapW4}, sig, 2, false, true, wrapPERIOD, ...
    'verbose', false);
d1S = buildMaet({wrapPa}, {wrapW4}, sig, 2, false, true, wrapPERIOD, ...
    'wrap', 'single-image', 'verbose', false);
d2F = buildMaet({wrapPb}, {wrapW4}, sig, 2, false, true, wrapPERIOD, ...
    'verbose', false);
d2S = buildMaet({wrapPb}, {wrapW4}, sig, 2, false, true, wrapPERIOD, ...
    'wrap', 'single-image', 'verbose', false);
cF = simMaet(d1F, d2F, 'verbose', false);
cS = simMaet(d1S, d2S, 'verbose', false);
results(end+1, :) = { ...
...
pNP = [0; 100; 300; 700];
wNP = ones(4, 1);
d1 = buildMaet({pNP}, {wNP}, 50, 2, false, false, 0, 'verbose', false);
d1S = buildMaet({pNP}, {wNP}, 50, 2, false, false, 0, ...
    'wrap', 'single-image', 'verbose', false);
c1 = simMaet(d1, d1, 'verbose', false);
c1S = simMaet(d1S, d1S, 'verbose', false);
results(end+1, :) = { ...
...
sig = 0.20 * wrapPERIOD;
d1a = buildMaet({wrapPa}, {wrapW4}, sig, 2, true, true, wrapPERIOD, ...
    'verbose', false);
d1b = buildMaet({wrapPa}, {wrapW4}, sig, 2, true, true, wrapPERIOD, ...
    'wrap', 'single-image', 'verbose', false);
d2a = buildMaet({wrapPb}, {wrapW4}, sig, 2, true, true, wrapPERIOD, ...
    'verbose', false);
d2b = buildMaet({wrapPb}, {wrapW4}, sig, 2, true, true, wrapPERIOD, ...
    'wrap', 'single-image', 'verbose', false);
cF = simMaet(d1a, d2a, 'verbose', false);
cS = simMaet(d1b, d2b, 'verbose', false);
results(end+1, :) = { ...
...
%
%  Both transformations rebuild the density, and buildMaet defaults
...
pW = {[1 5 9 2; 3 7 11 4]};
wW = {[1 1 0 1; 1 1 0 1]};
dW = buildMaet(pW, wW, 1.2, 2, true, true, 12, true, ...
                  'wrap', 'single-image', 'verbose', false);

dWmat = internal.ensureMaetExpensive(dW);
results(end+1, :) = { ...
    'wrap api: survives materialising the expensive fields', ...
    strcmp(char(dWmat.wrap{1}), 'single-image')};

dWpruned = internal.prunedMaet(dW);
results(end+1, :) = { ...
    'wrap api: survives event pruning', ...
    strcmp(char(dWpruned.wrap{1}), 'single-image')};

dWboth = internal.ensureMaetExpensive(internal.prunedMaet(dW));
results(end+1, :) = { ...
