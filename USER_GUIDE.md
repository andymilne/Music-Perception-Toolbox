...
**Event translation.** `translateEvents` shifts every value of every attribute belonging to selected groups by a per-group offset, leaving non-selected groups unchanged. The output `pAttrTranslated` feeds directly into `buildExpTens` without further massaging; weights pass through unchanged, since uniform translation does not affect any event's probability of perception. The shift is applied uniformly to every slot in every event's multiset, so the operation is well-defined for any $K_a \ge 1$ — unlike windowing, where a per-event scalar weight would have to summarise multiple slots' distances from a localising centre, translation has no analogous obstruction at $K_a > 1$. For absolute groups the operation is `value + mu`, regardless of periodicity: the wrapped periodic Gaussian kernel of `buildExpTens` is invariant under any additive shift by a multiple of $P$, so periodic groups need no canonical wrap of the translated values — the kernel handles periodicity downstream. For relative groups translation is a structural no-op — a uniform shift cancels in every within-tuple difference, so the relative MAET is unchanged — and the function emits a `translateEvents:noOpRelative` (MATLAB) or `TranslateEventsNoOpWarning` (Python) and leaves the group unchanged. Multiple groups can be translated simultaneously in one call by supplying a per-group offset vector (or sparse dict in Python). An entire sweep over $M$ offset configurations is produced in one call by passing offsets as a $G$-by-$M$ matrix; the return is then a length-$M$ list of translated `pAttr` blocks, one per offset column. In MATLAB the vector form requires a $1$-by-$G$ row layout (or a scalar when $G = 1$); a length-$G$ column vector reads as a $G$-by-$1$ matrix and so dispatches to matrix form with $M = 1$ — the only way to specify $M = 1$ in MATLAB without an extra flag.

`translateEvents` is the **pre-tensor route to a sliding comparison** along an attribute axis. For each candidate offset $\mu$ on a sweep grid, translate the events, build the MAET, and compute `cosSimExpTens` against an un-shifted reference; the peak of the resulting profile is the offset that best aligns the two densities, and the peak value is the standard cosine similarity at that alignment. The matrix form of `translateEvents` produces the whole sweep's translated `pAttr` list in one call, and the **raw-MA scalar-vs-list mode** of `cosSimExpTens` consumes that list directly, building the reference once and each translated query once internally; the explicit per-offset `buildExpTens` loop is no longer required. Because the operation is rigid translation followed by standard cosine, the resulting similarity values are symmetric in their two arguments and bounded in $[0, 1]$. The post-tensor route to the same kind of sliding comparison is provided by `windowedSimilarity` (next subsection): instead of translating the query and comparing whole-density-to-whole-density, the context's density is multiplied pointwise by a localising window that slides along the axis, and the windowed inner product against the un-windowed query is computed at each window position. The two routes are conceptually related — both answer "how does the comparison change as we slide along this axis?" — but they differ in two ways. First, the pre-tensor route uses the query's own intrinsic support as the matching template, so locality is set by the query; the post-tensor route uses an externally chosen window, decoupling locality from the query. Second, the post-tensor route is magnitude-aware by default (so similarity values can exceed $1$ and the query/context roles are asymmetric, see "Magnitude-aware normalisation" below), whereas the pre-tensor route preserves the symmetric cosine. The two routes coincide in the limit where the post-tensor window has the same shape as the query density. In practice, pre-tensor translation is the natural choice when two whole densities are being aligned under an unknown transposition or time-shift (e.g. cadence-progression comparison up to musical transposition), and post-tensor windowing is the natural choice when a fixed query is being matched against a longer context with controllable inspection scale (e.g. motif-finding within a chorale). On periodic groups both routes are well-defined and operationally interchangeable for whole-density alignment; on relative groups only the post-tensor route is available, since pre-tensor translation is a no-op there — but the corresponding post-tensor sweep then operates on interval-size space, an unusual analytic move that is rarely the question one is asking.
...
| `translateEvents` | `translate_events` | *Preprocessing:* shift selected groups by a per-group offset; matrix form sweeps M offsets in one call |
...
Cross-event preprocessing for MAET input. Shifts every value of every attribute in selected groups by a per-group offset, returning a transformed `pAttrTranslated` that feeds directly into `buildExpTens`. Weights are unaffected and are not part of the signature. Semantics are by group geometry: absolute groups are translated as `value + mu` (unwrapped — the periodic kernel handles wrap downstream when `isPer[g] = True`); relative groups pass through unchanged and emit a single `translateEvents:noOpRelative` (MATLAB) / `TranslateEventsNoOpWarning` (Python) per call. `offsets` accepts two shapes — a $1$-by-$G$ row vector in MATLAB (or a sparse `{group_index: mu}` dict / 1-D length-$G$ ndarray in Python) returning a single translated `pAttr`, or a $G$-by-$M$ matrix returning a length-$M$ list of translated `pAttr` blocks (matrix-form sweep). NaN entries in either shape mean "do not translate this group" on that column. The matrix form's output is the natural input to the raw-MA scalar-vs-list mode of `cosSimExpTens` (§6.1 above) for a one-call sliding-comparison sweep. In MATLAB, a length-$G$ column vector reads as a $G$-by-$1$ matrix — i.e. matrix form with $M = 1$, not vector form; Python is shape-driven and has no analogous ambiguity. See §3.1 (Event translation) for the conceptual framing of pre-tensor versus post-tensor sliding.
