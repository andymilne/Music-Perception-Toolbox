...
transposition-equivalent on the circle (the chain of ``-gen`` is the
chain of ``gen`` shifted by ``-(n-1)*gen``), so for ``is_rel=True`` they
yield identical PCS values. The demo simply evaluates the full range
and lets ``sim_maet``'s canonical-form dedup collapse this
...
n_gens = len(gen_range)

# Generator-chain multisets: each row is a different generator value.
p_mat_b = np.full((n_gens, n_tones), np.nan)
for i, gen in enumerate(gen_range):
...
# gen and period-gen produce transposition-equivalent chains under
# is_rel=True (chain(period-gen) is chain(gen) shifted by -(n-1)*gen on
# the circle), so sim_maet's canonical-form dedup collapses
# each pair to one cached density internally — no manual half-range
# shortcut is needed.
print(f"Computing PCS of {n_tones}-tone generator-chains "
      f"(gen = 0 to {period - gen_step:.1f}, step {gen_step}) "
      f"against {ref_name}...")
...
ax1.set_ylabel('Pitch class similarity')
ax1.set_title(f'PCS of {n_tones}-tone generator-chains with {ref_name}\n'
...
ax2.set_title(f'PCS of {n_tones}-tone generator-chains with {ref_name}\n'
...
print(f"{'gen (cents)':<14s}  {'(complement)':<14s}  {'PCS'}")
print('-' * 42)
labelled_t = []
printed = 0
for li in range(len(pk_idx)):
    ...
    best_pcs = max(s[idx_gen], s[idx_comp])

    print(f"{gen_val:<14.1f}  ({complement:<10.1f})  {best_pcs:.3f}")
