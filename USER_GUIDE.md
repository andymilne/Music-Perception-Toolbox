...
**Version 2.2.0** is primarily a performance release. A new analytical decomposition — the **Möbius method** — joins Bulger's existing inner-product decomposition as a second analytical route, with a per-call cost-model dispatcher on `cosSimExpTens` selecting the cheaper of the two per call (large $r$ or large $K$ favouring the Möbius method, small $r$ and small $K$ favouring Bulger's). The Möbius method also extends to **point evaluation** and **total mass**, where Bulger's decomposition does not apply; it provides efficiency alternatives to v2.1's centres-array routes for these, and `evalExpTens` accordingly has its own per-call dispatcher (centres vs Möbius) along the same lines as the inner-product dispatcher above. The inclusion-exclusion technique at the core of the Möbius method was first published in Milne et al. 2011 and has been used in MPT since v1 for numerical computation of discrete tensors; v2.2 applies it as a closed-form analytical decomposition, combined with orbit collapse in the inner-product case. Beyond the dispatcher, v2.2 adds three user-controllable kernel-evaluation options (`truncation_sigmas`, `kernel_precision`, and `kernel_chunk_bytes`) and several batching refinements that replace v2.1 sequential per-item loops with grouped contractions. A closed-form **Rényi-2 differential entropy** is also exposed as `method='renyi2'` on `entropyExpTens` (Shannon differential entropy continues to require numerical-grid integration, since it has no closed form in any version). See §4 for the method-selection API.

Alongside the user-facing additions above, v2.2 includes substantial internal tidying-up: a developer-facing `ARCHITECTURE.md` companion to this User Guide is added; the Python implementation is refactored into `_tensor/` and `_circular/` sub-packages with the public API unchanged; a MATLAB docstring audit fills in `help` coverage for the v2.2 kwargs; and a series of kernel-evaluation inner-loop refinements deliver further per-call speedups across `cosSimExpTens`, `evalExpTens`, and `windowedSimilarity` (partial vectorisation of the pairwise-wrap loop in periodic-relative mode, hoisting of per-density normalisers outside `windowedSimilarity`'s offset sweep, elimination of redundant component-wise outer wraps where pairwise wrapping subsumes them, and a `scipy.signal.correlate` swap in `templateHarmonicity`). See `CHANGELOG.md` for the full list.
...
Every attribute is self-contained: it carries its own σ, isRel, isPer, period, and r. There is no separate "group" construct — attributes that should behave alike (e.g. several pitch voices) simply take equal parameter values. The word *group* survives in this guide only as informal English shorthand ("the pitch attributes"), never as an API argument or a structural unit; the post-tensor window specification of `windowTensor` is likewise indexed per attribute.
...
The MAET framework provides four preprocessing helpers that take the `(pAttr, w, specs)` carrier feeding `buildExpTens` and return a transformed carrier of the same shape: `differenceEvents` and `bindEvents` (cross-event), and `translateAttributes` and `weightEvents` (per-event). The `specs` carrier (synthesised flat with `flatSpecs`, or threaded through from a prior step) holds the per-attribute level geometry; it is optional and assumed flat when omitted. See §3.7 for details, including the canonical windowed-entropy and pre-tensor sliding-comparison constructions.

#### Post-tensor windowing

A common analysis question — "at each point in this longer passage, how similar is the query to the local content?" — is handled by two functions that operate on pre-built MAETs: `windowTensor` and `windowedSimilarity`. `windowTensor` wraps a MAET with a window specification (per-attribute widths, shapes, and centres), returning a `WindowedMaetDensity` that is consumed lazily by `evalExpTens` (pointwise multiplication by the window) and by `windowedSimilarity` (closed-form windowed inner product, applied per-offset under the cross-correlation semantics described below). `windowedSimilarity` sweeps a series of *offsets* and returns the windowed-similarity profile — one similarity value per offset. The most common application is a sliding time window — "show me where in this melody the motif recurs" — but the same machinery applies to any attribute, so windowing in register, in pitch class, or along any other attribute axis — or combinations of them — is equally possible.

**Cross-correlation semantics.** `windowedSimilarity` implements cross-correlation: at each offset $\delta_a$ in a windowed attribute, the query is translated so that its effective-space centroid $\mu_q$ within that attribute moves onto the window centre $\mu_q + \delta_a$, and then the windowed similarity between the translated query and the window-weighted context is computed. Offset zero therefore places the window on the query's own centroid, and a peak at offset $\delta$ means the query pattern is present in the context displaced by $\delta$ from the query's centroid — the standard convention for matched-filter cross-correlation, where the profile is indexed by lag rather than by absolute position. A relative attribute (`isRel = true`) is translation-invariant within its own effective space, so when a window is placed on that attribute it does not need the cross-correlation translation; windows on other attributes (typically time, alongside a relative pitch attribute) operate on their own offsets independently, and do need the translation. Cross-correlation semantics apply only to `windowedSimilarity`; unwindowed calls to `cosSimExpTens` compute the cosine similarity directly, with no translation involved. `cosSimExpTens` does not accept `WindowedMaetDensity` operands at all — the windowed similarity is a magnitude-aware quantity rather than a strict cosine similarity, and the relevant entry point is always `windowedSimilarity`.
...
The choice always matters if queries differ in any property that shifts peaks in the similarity profile. For single-atom attributes ($K_a = 1$), between-query variation of an atom's position is a translation of the tuple centres on that attribute; the default's reference then co-moves with the query and a fixed reference does not, and the choice depends on whether offsets should be read as query-relative displacement or as positions relative to an absolute anchor. The more substantive distinctions emerge when $K_a > 1$, because queries can then differ not only in atom *positions* but also in atom *count* (adding or removing partials) and in atom *weights* (e.g., rolloff changes). In practice, multi-atom attributes are pitch attributes: either pitch attributes with $K_a > 1$ representing several pitches per event (e.g., chords with exchangeable voices), or pitch attributes expanded by `addSpectra` to place partials alongside the fundamentals. The remainder of this subsection discusses the reference-point choice in the context of multi-atom pitch attributes.

Whether the two reference methods give peak offsets that line up across a family of queries depends on how the queries vary. If they differ only in atom *weights* (for example, a rolloff sweep), both methods give identical and stable peak offsets, for any atom structure. If they differ in atom *positions* (for example, a partial-stretching sweep), the default's peak offset drifts while a fixed reference's stays put, again for any atom structure. If they differ in atom *count* (for example, adding partials), both methods give drifting peak offsets in general, but one special case is distinguished: for queries whose atoms lie at (or close to) integer-harmonic positions — that is, at $f_e + 1200 \log_2 n_k$ cents for some set of positive integers $\{n_k\}$ — the default's peak offset is stable across atom-count changes, because the geometric centroid and the best-match absolute pitch co-move closely as partials are added. We refer to queries of this form as **harmonic queries** on the pitch attribute; the term here refers only to atom placement on a log-frequency axis and is independent of any musical-harmony usage of the word. The demo `demo_windowingReference` documents these behaviours in detail, with plots of similarity profiles across incremental sweeps in atom weights, atom positions, and atom count, and with the harmonic special case separately characterised.

**Practical guidance.**

*Default reference.* Appropriate when both queries being compared are harmonic queries on their multi-atom pitch attribute and the between-query variation is restricted to atom count and/or atom weights. The default then places peak offsets at the musical transposition interval between the queries and keeps this reading stable across the sweep.

*Fixed reference calibrated to a canonical harmonic query.* Appropriate for analyses that include any non-harmonic query, or any atom-position variation, or whenever the offset axis is to be calibrated to musical transposition intervals independent of the query being scored. Choose the fixed reference as the unweighted centroid of a canonical harmonic query (e.g., a 12-partial complex tone on the chosen fundamentals) and hold it fixed across all queries. Under this calibration, the harmonic baseline reproduces the default's musical-interval reading; non-harmonic perturbations then appear as changes in the profile's shape and peak amplitude rather than as drifts of the peak offset.
...
**A shape-parameterised window family.** The window is specified per attribute by two numbers: a width parameter `size` (in multiples of that attribute's σ) and a shape parameter `mix` in [0, 1]. At `mix = 0` the window is a pure Gaussian; at `mix = 1` it is a pure rectangular (boxcar) window; intermediate values produce flat-topped shapes formed by convolution of a rectangular window with a Gaussian. This one-parameter interpolation lets the user match the window shape to the problem at hand — soft edges when a smooth trade-off between locality and coverage is wanted, sharp edges when an in-or-out selection of events is wanted, or anything in between. The `size` parameter controls the window's effective width independently of its shape. The windowed inner product between a windowed MAET and an unwindowed MAET has a closed-form analytical solution across the entire `(size, mix)` family for all 1-D attributes (effective dimension 1, e.g. time) and for all multi-dimensional absolute attributes; for multi-dimensional relative attributes ($r \ge 3$ with `isRel = true`), the closed form is currently established for the pure-Gaussian case only, with a Gaussian-mixture approximation of the full family being a possible extension. Evaluation of a windowed MAET via `evalExpTens` (pointwise multiplication by the window) is closed-form across the full family in any attribute dimensionality — the inner-product restriction does not apply to pointwise evaluation, so `entropyExpTens` on a windowed MAET is also supported uniformly.

**Periodic attributes: wrapped-Gaussian image summation.** For periodic attributes, the window is the *wrapped Gaussian* (or the wrapped rectangular-convolved-with-Gaussian for `mix > 0`): the sum of line-case window functions at all periodic images of the centre, $h_{\text{per}}(x) = \sum_{n \in \mathbb{Z}} h_{\text{line}}(x - nP)$. Because the windowed inner product is linear in the window, the wrapped windowed inner product is the same sum of line-case inner products evaluated at shifted window centres. The toolbox sums this series adaptively, truncating when the latest image-pair's contribution falls below $10^{-12}$ of the running maximum (or $10^{-7}$ for `kernel_precision = 'single'`). The truncation is at floating-point precision, so the result equals the exact wrapped-Gaussian windowed inner product to machine accuracy across all regimes of $\lambda\sigma/P$. For multi-D absolute periodic attributes, the per-pair $F$ factor factorises across axes, so the image sum factorises into per-axis sums — the cost is linear in dimension rather than exponential. For multi-D relative periodic attributes (which require a Cartesian-product sum over a non-factorising kernel), image summation is not currently implemented; in that case the line-case window is used directly, which produces an approximation when $\lambda\sigma$ is not small relative to $P$.
...
**Normalisation: one-sided versus strict shape-only.** `windowedSimilarity` exposes two normalisation modes via the `normalize` keyword (also accepted as `normalise`), differing only in the denominator applied to the windowed inner product. The default is `'oneSidedDenom'`:

$$ s_{\text{one-sided}} = \frac{\langle h \cdot f_X, f_Y \rangle}{\langle f_Y, f_Y \rangle} $$

where $f_Y$ is the unwindowed query and $f_X$ (windowed) is the context. This is *magnitude-aware*: self-similarity at full window coverage equals 1; the profile drops below 1 when the window does not cover all of the query's mass, and rises above 1 when the windowed context has more matching mass in the window region than the query carries in total. This behaviour is what is wanted for sliding-motif and probe-tone analyses — a region of the context with a small amount of near-perfectly-matching content scores lower than a region with a large amount of equally-matching content, because the windowed inner product scales with the amount of matching mass and the (query-only) denominator does not. The output is not bounded in $[-1, 1]$ across sweep positions and does not correspond to an inner product on a single Hilbert space. Multiplying the query's weights by a constant rescales the profile by that constant (one factor of the query in the numerator versus two factors in the denominator), so profiles from queries whose weight scales differ are not directly comparable on absolute magnitude. Multiplying the context's weights by a constant rescales the profile linearly.

The alternative is `normalize='cosine'`, the strict shape-only cosine:

$$ s_{\text{cosine}} = \frac{\langle h \cdot f_X, f_Y \rangle}{\sqrt{\langle h \cdot f_X, h \cdot f_X \rangle \, \langle f_Y, f_Y \rangle}} $$

The denominator now uses the *windowed* context's self inner product on one side and the unwindowed query's on the other. The result is bounded in $[-1, 1]$ across sweep positions and invariant to a positive scalar on either operand — the same shape match scores the same regardless of how much mass falls inside the window. The closed-form windowed self inner product $\langle h \cdot f_X, h \cdot f_X \rangle$ requires that the pointwise square $h^2$ of the window remain in the toolbox's `(size, mix)` family. This holds for the two endpoint window shapes — pure Gaussian (`mix = 0`, with $h^2$ also Gaussian and width scaled by $1/\sqrt{2}$) and pure boxcar (`mix = 1`, with $h^2 = h$ since $h \in \{0, 1\}$ pointwise). For intermediate `mix`, $h^2$ falls outside the family and the cosine option raises; in that regime the one-sided default is the available choice (or the user can re-run at `mix = 0` or `mix = 1`).

The two normalisations answer different questions: the one-sided default scores *how much of the query is present at this offset*, while the cosine option scores *how well the local content's shape matches the query, ignoring magnitude*. The former is appropriate for sliding-motif analysis; the latter for shape-only cross-correlation. Asymmetry of the two arguments is structural under both: the first positional argument `densContext` is the operand the window multiplies (it shifts with each sweep step), while the second positional argument `densQuery` is the unwindowed comparison template whose self inner product appears in the denominator under both modes.
...
**Soft sigma semantics.** As of v2.1.0, six of the structural measures — `sameness`, `coherence`, `nTupleEntropy`, `balanceCircular`, `evennessCircular`, and `projCentroid` — accept an optional positive `sigma` argument that softens the underlying computation against Gaussian positional uncertainty on each event,
...
The MAET framework provides four preprocessing helpers that act on the `(pAttr, w, specs)` carrier feeding `buildExpTens`. They form a closed family — each takes one carrier, returns another in the same shape, and chains directly into further preprocessing or into MAET construction — organised by their scope of action:
...
`bindEvents` gathers $L$ consecutive events into a single nested super-event, sliding a window of width $L$ across the input. Each window becomes one *nested* output attribute: the bound events form an **ordered outer level** (carrying event order, so the binding is lossless), with the original per-event values at the inner level. The output is the transformed `(pAttr, w, specs)` carrier — the `specs` now encode the two-level (inner/outer) geometry — feeding straight into `buildExpTens`. The outer level defaults to reading the whole window ($r = L$), ordered (`[sym] = 0`, so ordered tuples are not collapsed to unordered ones), and absolute; making the outer level relative (`relOuter = 1`) gives a within-window transposition-invariant n-gram. Per-event weights propagate so that a bound super-event's effective weight is the product of its $L$ constituent events' weights — matching the differencing rule under the toolbox's standard probability-of-perception reading. The default output has $N - L + 1$ super-events; a `circular` option produces $N$ by wrapping around the end of the input, the natural choice for cyclic event sequences. Unlike `differenceEvents`, `bindEvents` accepts any $K_a \ge 1$: each atom of the underlying events is carried through the nesting, preserving within-attribute atom exchangeability.
...
**`deleteInput` (mandatory keyword).** A boolean flag — keyword-only, no default — selects whether the input attribute is retained in the returned `(pAttr, wOut, specs)` carrier or removed. When `deleteInput = true` and `inputAttr != targetAttr`, the input attribute is dropped from all three output components and the higher attribute indices are decremented to keep numbering contiguous. When `deleteInput = false`, the input attribute is preserved unchanged in the output. `deleteInput = true` paired with `inputAttr == targetAttr` raises an error: deleting the input would discard the factor just written to it. The auto-delete pattern is the standard idiom for windowed-entropy and local-mass analyses where the input attribute (typically time) provides the scaffolding for the window and is no longer needed downstream once its positions have been transferred to the target's weights.
...
**Centre-shift identity.** When event weighting and attribute translation target overlapping attributes, the operations commute up to a shift of the window centre: applying `translateAttributes` by an offset $\mu$ after `weightEvents` centred at $c$ gives the same pre-MAET as applying `weightEvents` centred at $c + \mu$ after the translation. The factor evaluates on the same residual $v - c$ either way. This identity underlies the windowed-entropy and pre-tensor sliding-comparison constructions of §3.7.5.

#### 3.7.5 Compositions and canonical uses

The four preprocessing operations chain freely. Operations targeting disjoint attribute sets commute by construction, since each operation acts only on the slices of the per-attribute value and weight matrices corresponding to its targets. On overlapping targets, order dependence reflects different analytic questions about the data. Two compositions are canonical and named.
...
This is the *pre-tensor* route to a sliding comparison, complementary to the *post-tensor* `windowedSimilarity` route described in §3.1 (Post-tensor windowing). The two routes differ in two ways. First, the pre-tensor route uses the query's own intrinsic support as the matching scale; the post-tensor route uses an externally chosen window, decoupling the comparison region's scale from the query. Second, the post-tensor route is magnitude-aware by default (so similarity values can exceed $1$ and the query/context roles are asymmetric — see §3.1's "Normalisation: one-sided versus strict shape-only"), whereas the pre-tensor route preserves the symmetric cosine. In practice, pre-tensor translation is the natural choice when two whole densities are being aligned under an unknown transposition or time-shift (e.g. cadence-progression comparison up to musical transposition), and post-tensor windowing is the natural choice when a fixed query is being matched against a longer context with controllable inspection scale (e.g. motif-finding within a chorale). On periodic groups both routes are well-defined and operationally interchangeable for whole-density alignment; on relative groups only the post-tensor route is available, since pre-tensor translation is a no-op there — but the corresponding post-tensor sweep then operates on interval-size space, an unusual analytic move that is rarely the question one is asking.
...
### Method selection (v2.2+)

The v2.2 release introduces method-selection dispatchers for two of the toolbox's analytical computations: the **inner product** (used by `cosSimExpTens`, and internally by `entropyExpTens` under `method='renyi2'` for the $\langle T, T \rangle$ term) and **point evaluation** (used by `evalExpTens`, and internally by `entropyExpTens` under `method='renyi2'` for the total-mass $\int T$ term). Each user-facing function accepts a `'method'` keyword whose `'auto'` setting (default) routes via a per-call cost model; the explicit alternatives let users force a specific decomposition.

Three analytical decompositions of the relevant tuple sums, plus a brute-force baseline, are available across the v2.2 surface:

- **Bulger's method** (`method='bulger'`) — the v2.1 inner-product decomposition. Organises the tuple sum by combinations on one side and permutations on the other, using the within-tuple multinomial symmetry to avoid enumerating equivalent orderings. **Inner-product only.** Has essentially no per-call fixed overhead and tends to win at small $r$ and small $K$.

- **The Möbius method** (`method='mobius'`) — new in v2.2. Rewrites the tuple sum via Möbius inversion on the partition lattice: the distinct-index constraint becomes an alternating sum over set partitions, with each partition's term factorising across its blocks. For the inner product this combines with **orbit collapse** under the joint atom-permutation symmetry, reducing the partition-pair count from $B_r^2$ to $|\Omega_r|$ orbit equivalence classes (4, 10, 33, 92, 306, 948, 3210 for $r = 2, \ldots, 8$). **Applies uniformly to inner product, point evaluation, and total mass.** Carries fixed per-call overhead (orbit-table lookup, $|\Omega_r|$ tensor contractions) and tends to win at large $r$ or large $K$ where the tuple loop in Bulger's method or the explicit centres array of the centres-array path blows up. The decomposition is grid-free in **absolute** mode ($O(B_r\cdot r\cdot K)$ per query for point evaluation, each partition block collapsing to an $O(K)$ event sum at the query). In **relative** mode it is *not* grid-free: the relative tensor is the translation marginal of the absolute tensor, and the alternating partition sum only factorises across atoms at fixed translation $u$, so the Möbius method integrates over a u-grid of $N_u$ points. Two integrand strategies serve that grid: a *direct* strategy at $O(B_r\cdot r\cdot K\cdot N_u)$ per query, and (non-periodic only) a *factored* strategy at $O(B_r\cdot r\cdot N_u)$ per query after an amortised tabulation of $r$ smoothed event distributions, selected automatically by a cost gate; the factored read-back's accuracy is tied to `truncation_sigmas` (targeting the kernel-truncation floor $\exp(-k^2/2)$, clamped to $[10^{-12}, 10^{-3}]$). The centres/Bulger path avoids the grid by carrying the closed-form relative Gaussian per tuple instead, at the price of enumerating the $K!/(K-r)!$ tuples; consequently, in relative mode the centres path stays cheaper for scalar queries at small $K$ (no $N_u$ factor), with the Möbius method taking over as $K$ or the batch size grows, whereas in absolute mode it wins outright from $r \ge 3$. The underlying inclusion-exclusion technique was first published in Milne et al. 2011 (framed there as inclusion-exclusion rather than Möbius inversion on the partition lattice; the two are equivalent formulations) and has been used in MPT since v1 for numerical computation of discrete tensors; v2.2 applies it as a closed-form analytical decomposition.

- **The centres-array path** (`method='centres'`) — the v2.1 point-evaluation strategy. Every $r$-tuple in the source cartesian product is materialised as a Gaussian centre, and each query point is evaluated by summing kernel contributions from all centres. **Point-evaluation only.** Numerically robust, no precision guard required. Tends to win at small $r$ where materialising the centres array is cheap.

- **The centres method** (`method='centres'` where accepted) — the path that evaluates the closed-form expression as written, with no clever combinatorial decomposition. Two consumers realise this differently:

    * On the **inner product** (`cosSimExpTens`), `centres` enumerates every ordered $r$-tuple on each side and sums kernel contributions over every pair, with no Bulger combinations-vs-permutations factoring and no Möbius alternating sum. Cost $O\!\left(\frac{K_x!}{(K_x-r)!} \cdot \frac{K_y!}{(K_y-r)!}\right)$ — far more expensive than either at any appreciable $K$, but exact at any $K \ge r$ and immune to cancellation, all summands being non-negative. It is the reference route: it shares no reduction with the other two, so agreement with it is a stronger check than agreement between them. All four modes are supported. Force `method='centres'` for benchmarking or sanity-checking; under `'auto'` the dispatcher never selects it.

    * On the **MA per-attribute inner-product matrix**, the dispatcher routes unsafe-event pairs to direct enumeration, since the K-grouped batched form vectorises across event pairs as a single tensor contraction more cleanly than Bulger's combinations-vs-permutations factoring (and the ordered-$r$-tuple count is small at the low $K_\text{eff}$ that defines this regime). Not user-selectable; routing is internal to the MA inner-product dispatch.

    * On the **windowed inner product** (`windowedSimilarity`), the closed-form analytical expression for the windowed similarity (Section 3.1, "Post-tensor windowing") is evaluated as written in Gaussian-integral form. It is the only path available — there is no Bulger or Möbius variant of the windowed inner product to compare against — and the dispatch announce reads `windowedSimilarity: chose 'closed-form' path.` accordingly. `windowedSimilarity` does not accept a `method` keyword; the label is descriptive vocabulary for what the function does, not a chosen path among alternatives.

    On `evalExpTens`, the same route is named `'centres'`, and means the same thing: evaluate at the enumerated tuple centres rather than through the Möbius alternating sum. One algorithm, one name, across both functions.

    (`'direct'` was accepted in earlier versions. On `cosSimExpTens` it resolved to Bulger's method, so the name promised an unrestricted enumeration the toolbox never performed; on `evalExpTens` it was a synonym for `'centres'`. It is retired in favour of `'centres'`, which now names the unrestricted enumeration in both functions.)

Under `'auto'`, `cosSimExpTens` picks between Bulger's and the Möbius method using a per-call cost model that considers $r$, $K$, $N$, and $\sigma/\text{period}$, with a **timing-probe fallback** for SA cosine similarity when the cost-model pre-screen is not decisive: the dispatcher times each candidate on a small subset of the workload and routes the full call accordingly. `evalExpTens` picks between centres and the Möbius method using the same cost-model considerations, but with **no timing probe** — the centres-path overhead is small enough that probing would cost more than it saves. Both dispatchers honour the Möbius precision guard ($K - r \ge 2$); outside it, `'auto'` falls back to Bulger's method (cosine) or to the centres path (eval). The Bulger and Möbius methods agree to floating-point precision on inputs where both are valid (the inner-product case); the centres and Möbius methods likewise agree to floating-point precision on the eval case in default modes, with the same Bulger-vs-Möbius divergence pattern in periodic-relative mode at $\sigma/P > 0.03$ (the pairwise-wrap form of Eq 6 versus the full integral form, agreeing as $O((\sigma/P)^{\infty})$).

Most callers should leave `method` at the default `'auto'`. Override only when measuring Möbius-vs-Bulger or Möbius-vs-centres agreement, debugging cancellation issues, or forcing a specific method for benchmarking.

The companion `'cancellationThreshold'` keyword on `cosSimExpTens` (default `1e-12`) guards the Möbius method's alternating partition sum against **catastrophic cancellation** — the floating-point phenomenon where subtracting numbers of similar magnitude leaves a result whose true value is much smaller than the accumulated round-off noise, so the computed answer is dominated by that noise rather than by the signal. In the Möbius case, the partition sum's alternating signs can produce large positive and negative contributions that should sum to a much smaller true value; if their worst-case cancellation ratio drops below the threshold, the dispatcher falls back to Bulger's method (which sums only non-negative contributions and is unaffected). Lowering the threshold relaxes the guard; raising it tightens it (forcing Bulger's method more aggressively).

### Four-method entropy API and adaptive differential entropy (v2.2+)
...
- **`method='renyi2'`.** Analytical Rényi-2 (collision) entropy $H_2 = -\log_b(\langle T, T\rangle / Z^2)$ computed in closed form via the Möbius method's inner product for $\langle T, T\rangle$ and total mass for $Z$. No grid, no caller choice of resolution; the result is exact (analytical) and works at high $r$ where any grid path would exhaust memory. Currently restricted to single-density input; errors at `sigma=0`. `normalize=True` is currently unsupported (the natural normaliser yields a $(-\infty, 1]$ range that doesn't compose with Shannon's $[0, 1]$); divide externally if needed.

The two continuous methods (`'differential'`, `'renyi2'`) ignore `n_points_per_dim` and the grid-bounds kwargs (`x_min`, `x_max`); the two discrete methods require an explicit `n_points_per_dim` (no toolbox-wide default in v2.2). `spectralEntropy` defaults to `method='differential'`; `nTupleEntropy` defaults to `method='normalized'` (matching the v2.1 default behaviour at $\sigma = 0$). See the MIGRATION guide for callers updating from v2.1's implicit-grid Shannon path.
...
% Build MAETs for both. Pitch attribute: absolute periodic (σ = 10 cents).
% Time attribute: absolute non-periodic (σ = 0.2 time units).
...
S = windowedSimilarity(melody, query, spec, offsets);
disp(S);
```
...
S = mpt.windowed_similarity(melody, query, spec, offsets, verbose=False)
print(S)
```
...
| `windowTensor` | `window_tensor` | *Core:* wrap a MAET with a post-tensor window specification |
| `windowedSimilarity` | `windowed_similarity` | *Core:* sliding-window similarity profile (magnitude-aware or cosine) |
| `flatSpecs` | `flat_specs` | *Preprocessing:* synthesise the canonical flat `specs` carrier for bare attributes |
...
The preprocessing functions all operate on the canonical `(pAttr, w, specs)` carrier and return a transformed carrier of the same form, so they chain freely and feed straight into `buildExpTens`. The `specs` argument (a per-attribute list of level-geometry structs; synthesise a flat one with `flatSpecs`) is optional and keyword-only — omit it and a flat carrier is assumed.
...
- `size` — Per-attribute window effective standard deviation in multiples of that attribute's σ. Scalar or length-A vector; NaN or Inf on an entry means the attribute is not windowed.
- `mix` — Per-attribute shape parameter in [0, 1]: 0 is pure Gaussian, 1 is pure rectangular, intermediate values are rectangular-convolved-with-Gaussian.
...
No math is performed at construction time; the window is applied lazily by `evalExpTens` (pointwise multiplication by the window function) and by `windowedSimilarity` (closed-form windowed inner product, applied per-offset). `cosSimExpTens` does not accept `WindowedMaetDensity` operands.

**windowedSimilarity(densContext, densQuery, windowSpec, offsets [, 'normalize', n] [, 'verbose', v])**

Sliding-window similarity profile (cross-correlation). For each column of `offsets` (a `dim × M` matrix), windows `densContext` with the given `windowSpec` at the corresponding centre and computes the windowed similarity against `densQuery` (unwindowed). Returns a 1 × M profile. The two operands play asymmetric roles: `densContext` is the operand the window multiplies (it shifts with each sweep step), `densQuery` is the unwindowed comparison template whose self inner product appears in the denominator. Offsets are measured from the query's effective-space centroid to the window centre; the centroid is the unweighted column mean of the query's per-attribute tuple centres — a purely geometric property of the event positions, independent of the query's weight assignment (see Section 3.1 for the rationale for using the unweighted rather than the weighted mean). At offset 0 the window sits on the query's own centroid, so a peak at offset $\delta$ means the query pattern is present in the context displaced by $\delta$ from its centroid — the standard cross-correlation convention. Any `centre` field in the caller's `windowSpec` is ignored; offsets replace it.

The `normalize` keyword (also accepted as `normalise`) selects the denominator: `'oneSidedDenom'` (default) divides by the query's unwindowed self inner product, giving a magnitude-aware windowed similarity unbounded above with self-match at full coverage equal to 1; `'cosine'` divides by the geometric mean of the windowed-context and unwindowed-query self inner products, giving the strict shape-only cosine in $[-1, 1]$. The cosine option requires every attribute's `mix` to be 0 (pure Gaussian) or 1 (pure boxcar); intermediate `mix` raises and directs the user back to `'oneSidedDenom'`. See Section 3.1, "Normalisation: one-sided versus strict shape-only" for the full discussion.

For periodic attributes, the window is the wrapped Gaussian (or wrapped rect-conv-Gaussian for `mix > 0`); the toolbox sums periodic images of the line-case window adaptively to floating-point precision, so the result equals the exact wrapped-window inner product at all $\lambda\sigma / P$. See Section 3.1, "Periodic attributes: wrapped-Gaussian image summation".

The analytical inner product is closed-form for the full `(size, mix)` family on 1-D attributes (e.g., time) and on multi-D absolute attributes. For multi-D relative attributes (r ≥ 3 with isRel = true) only the pure-Gaussian case (mix = 0) is currently supported; a clear error is raised if a rectangular or raised-rectangular window is requested on such an attribute.

*MAET preprocessing.*

The preprocessing functions share a common shape: they take the canonical `(pAttr, w, specs)` carrier — the same triple `buildExpTens` consumes — apply a transform, and return a carrier of the same form. `specs` is keyword-only and optional; when omitted, a flat (one-level, `r = 1`, `[rel] = false`, `[sym] = true`) carrier is assumed. Because the output is again a `(pAttr, w, specs)` triple, the operators chain in any order and the final triple feeds directly into `buildExpTens`.

**flatSpecs(pAttr, 'r', r, 'rel', rel, 'sym', sym [, 'name', name])**

Convenience constructor for the canonical `specs` carrier. Given a length-$A$ list/cell of per-attribute value matrices (used only for its length $A$ — the values are not inspected), returns a length-$A$ list of flat spec structs `struct('r', ., 'rel', ., 'sym', .)`, broadcasting scalar geometry across attributes. This is the trivial flat-specs synthesis at the entry of a pre-MAET chain (raw attributes carry no level structure yet) and an ergonomic alternative to hand-writing the structs for `buildExpTens(..., 'specs', specs)`. Name-value pairs: `'r'` (scalar or length-$A$ read-arity, default 1), `'rel'` (scalar or length-$A$ `[rel]`, default false), `'sym'` (scalar or length-$A$ `[sym]`, default true), and `'name'` (optional per-attribute names). Most pre-MAET chains never call it directly — the preprocessing operators synthesise a flat carrier internally when `specs` is omitted — but it is the explicit hook when you want to set non-default `r`, `[rel]`, or `[sym]` before differencing, binding, translating, or weighting.

**differenceEvents(pAttr, w, diffOrders [, 'circular', false] [, 'specs', specs])**

Cross-event preprocessing for MAET input. Takes the `(pAttr, w, specs)` carrier and replaces the event sequences of selected attributes with their $k$-th finite differences along the event axis, returning a transformed `(pAttrDiff, wDiff, specs)` carrier in the same format. `diffOrders` is per-attribute: `diffOrders[a] = k` produces the $k$-th difference of attribute $a$; order 0 leaves an attribute unchanged (a scalar broadcasts to every attribute). Raw signed differences are emitted regardless of periodicity; periodic attributes' mod-period wrap is handled by the kernel at MAET-construction time. Weights follow the toolbox's standard broadcast convention ([§4](#4-api-conventions)) and propagate as rolling products of width $k + 1$, so the weight of a differenced event is the product of the weights of the $k + 1$ constituent events it depends on (interpretable as the probability that all constituents are jointly perceived). When `'circular'` is `false` (default), the output event count is $N' = N - \max_a k_a$; when different attributes use different orders, lower-order attributes have leading events dropped to keep columns aligned. When `'circular'` is `true`, the difference operator wraps at the sequence boundary ($\Delta p_a(n) = p_a(n) - p_a(\mathrm{prev}(n))$ with $\mathrm{prev}(1) = N$) and every attribute retains $N$ events at every order; no leading-event drop is needed, and weights propagate via a cyclic rolling product. The circular variant parallels the same flag on `bindEvents` and is the natural choice for cyclic event sequences (looped rhythms, ostinati) in which the boundary difference is a genuine inter-event interval. The returned carrier feeds directly into `buildExpTens`. Inputs are restricted to $K_a = 1$ per attribute; see [§3.1](#31-multi-attribute-expectation-tensors) (Preprocessing) for the rationale and the voices-as-attributes pipeline for polyphonic analyses. Event differencing is distinct from `isRel = true`: the former replaces absolute per-event values with differences between adjacent events (a preprocessing step); the latter is a property of tensor construction that makes the within-r-ad density translation-invariant (no preprocessing involved). Both can be used together or independently. Composes with `bindEvents` (below) to produce n-grams of consecutive inter-event differences.

**bindEvents(pAttr, w, bindOrders [, 'circular', false] [, 'specs', specs] [, 'rOuter', …] [, 'symOuter', false] [, 'relOuter', false])**

Cross-event preprocessing for MAET input. For each attribute, a sliding window of width $L_a$ (`bindOrders`, per-attribute; a scalar broadcasts) is laid across the event axis and the $L_a$ consecutive events are nested into a single output attribute: the bound events form an **ordered outer level** (event order — `symOuter = 0` by default, so the binding is lossless and order-preserving), and the original per-event values sit at the inner level. The outer level defaults to reading the whole window (`rOuter = L_a`), ordered (`symOuter = 0`), and absolute (`relOuter = 0`); `L_a = 1` is the no-op. Setting `relOuter = 1` makes the outer level relative, giving a within-window transposition-invariant n-gram. The result is a transformed `(pAttrBound, wBound, specs)` carrier whose `specs` now encode the two-level (inner/outer) geometry, suitable straight as input to `buildExpTens`. Per-event weights propagate per row: each bound super-event's effective weight is the product of its $L_a$ constituent events' weights (the kernel product over the nested tuple recovers it). Inputs may have any $K_a \ge 1$; each atom of the underlying events is carried through the nesting, preserving within-attribute atom exchangeability. The output event count is $N' = N - \max_a L_a + 1$ (non-circular) or $N$ (`'circular'` true, wrapping at the sequence boundary as on `differenceEvents`); attributes with $L_a < \max_a L_a$ keep their leading $N'$ events to stay column-aligned. `'specs'` lets you bind an attribute that already carries level structure; binding an already-nested ($L \ge 3$ deep) attribute is not yet supported. See [§3.1](#31-multi-attribute-expectation-tensors) (Preprocessing) and the toolbox specification for the nesting model.
...
**translateAttributes(pAttr, w, offsets [, 'specs', specs])**

Per-attribute preprocessing for MAET input. Shifts selected attributes' values by a chosen offset on the `(pAttr, w, specs)` carrier, returning a transformed carrier that feeds directly into `buildExpTens`. Weights and `specs` pass through unchanged — only the values move. The transform is per-row: an offset is one position per row of the $K_\text{total} \times N$ position matrix, held constant across the sequence (column) axis, which is exactly what makes a uniform shift cancel inside relative attributes.

`offsets` is a **length-$A$ list** (Python) / $1 \times A$ cell (MATLAB), one entry per attribute, each of: `None` / empty `[]` (do not translate this attribute); a scalar or length-1 vector (broadcast to all rows — a global transposition); a length-$K_\text{total}$ vector (per-row); a $1 \times M$ row (a per-sweep global shift, one scalar per sweep index, broadcast across rows); or a $K_\text{total} \times M$ matrix (per-row by sweep index, rows down and sweep index across). `NaN` entries skip the corresponding row (left untranslated); $\pm\infty$ is rejected. All entries carrying $M > 1$ must agree on $M$; scalar, length-1, and single-column entries broadcast across the sweep.
...
When any entry implies a sweep ($M > 1$), the output is a length-$M$ list (Python) / $1 \times M$ cell (MATLAB) of per-attribute value-lists, each a separate pre-MAET input to build and compare; otherwise a single per-attribute value-list. The sweep output is the natural input to the raw-MA scalar-vs-list mode of `cosSimExpTens` (above) for a one-call sliding-comparison sweep over $M$ offsets, with no explicit per-offset `buildExpTens` loop. See [§3.1](#31-multi-attribute-expectation-tensors) (Attribute translation) for the conceptual framing of pre-tensor versus post-tensor sliding.

**weightEvents(pAttr, w, inputAttr, targetAttr, centre, shape, {'sd', s | 'width', L}, 'deleteInput', tf [, 'specs', specs] [, 'isPer', false] [, 'period', 0])**

Per-event preprocessing for MAET input. Reads the $K = 1$ value at every event from `inputAttr`, evaluates a window factor centred at `centre` with shape parameter $\gamma$ = `shape`, and multiplies the resulting $(1, N)$ per-event factor into the weights of `targetAttr` (on top of any weight already there), returning a transformed `(pAttr, wOut, specs)` carrier that feeds directly into `buildExpTens`.
...
`deleteInput` is a mandatory keyword-only boolean (no default). When `true` and `inputAttr != targetAttr`, the input attribute is dropped from the returned carrier (its `specs` entry removed and higher indices renumbered). When `false`, the input attribute is preserved. `deleteInput = true` paired with `inputAttr == targetAttr` raises an error (the just-written factor would be discarded).

Multi-axis windowing is expressed as a sequence of `weightEvents` calls with the same `targetAttr`; each call's factor multiplies into the target's existing weights. The canonical composition `weightEvents` (with `deleteInput = true`, `inputAttr` typically time, `targetAttr` typically pitch) $\to$ `buildExpTens` $\to$ `entropyExpTens` is the windowed-entropy construction of [§3.7.5](#375-compositions-and-canonical-uses). `weightEvents` chains cleanly with `translateAttributes` under a centre-shift identity ($\mathcal{T}_\mu \circ \mathcal{W}_c = \mathcal{W}_{c+\mu} \circ \mathcal{T}_\mu$ on overlapping targets) and passes through `differenceEvents` / `bindEvents`.
...
**dftCircular(p, w, period)** — Returns complex Fourier coefficients F and their magnitudes. F(1) (MATLAB 1-based) is the k = 0 coefficient; F(2) is k = 1; etc. Deterministic; unchanged from v2.0.
...
profile = windowedSimilarity(ctx_maet, probe_maet, spec_w, offsets);
...
profile = mpt.windowed_similarity(ctx_maet, probe_maet, spec_w, offsets,
                                 verbose=False)
```
...
% differences).
[mel_pd, ~] = differenceEvents({melody_p, melody_t}, [], [1 0]);
[qry_pd, ~] = differenceEvents({query_p,  query_t},  [], [1 0]);
...
% Pitch attribute: sigma = 50 cents gives clean separation between 100-cent
% interval categories; non-periodic so that, e.g., +1300 is distinct
% from +100. Time attribute: sigma = 0.3 time units.
...
mel_pd, _, _ = mpt.difference_events([melody_p[None, :], melody_t[None, :]],
                                     None, [1, 0])
qry_pd, _, _ = mpt.difference_events([query_p[None, :],  query_t[None, :]],
                                     None, [1, 0])
...
| `demo_helixBlend` | Pitch-class and register routed simultaneously through periodic pitch-class and linear register attributes; sweeping the register σ produces a continuum between pitch-class and pitch-height similarity | — |
| `demo_maetWindowing` | Time-windowed similarity sweep against a query motif: motif recurrence localisation in a melody | — |
| `demo_windowingReference` | Reference-point option for `windowedSimilarity` / `windowed_similarity`: harmonic-baseline, atom-count, atom-position, and atom-weight sweeps with the harmonic-calibrated fixed-reference strategy | — |
...
| `demo_translateSweep` | Pre-tensor sliding-comparison sweep: `translateAttributes` matrix form drives a 2-D (pitch transposition, time shift) sweep of a 3-note motif against an embedded-occurrence reference, scored in one call via the raw-MA scalar-vs-list mode of `cosSimExpTens`; closes with a parity check against the explicit `buildExpTens` loop | — |
...
| `demo_translate_sweep.py` | `demo_translateSweep` | Pre-tensor sliding-comparison sweep via `translate_attributes` matrix form and the raw-MA scalar-vs-list mode of `cos_sim_exp_tens` |
