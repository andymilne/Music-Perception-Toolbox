...
This document describes the toolbox as it currently exists. Significant restructuring of the Python `tensor.py` module into a `_tensor/` sub-package is planned but not yet applied; the corresponding section flags the current and target states.
...
├── tensor.py *            Density classes, build, eval, cos-sim, IP cores,
│                          dispatchers, canonical-form dedup, windowing,
│                          preprocessing, simplex_vertices
...
\* `tensor.py` is currently a ~8,000-line single module. A refactor into a `_tensor/` sub-package (with `density.py`, `build.py`, `eval.py`, `cosine.py`, `preprocessing.py`, `windowing.py`, `dispatch.py`, `canonical.py`) is planned. Public API surface remains unchanged after the refactor.
