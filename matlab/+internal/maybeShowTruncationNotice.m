...
%   See also: MPTDEFAULTS.
    ...
    warning('mpt:truncationDefault', ...
        ['Kernel evaluation truncates the Gaussian kernel at 6 sigma ' ...
         'by default, which runs much faster than a wider cutoff; the ' ...
         'speed-up grows with tuple size r and multiset size, where ' ...
         'a wider cutoff has many kernel centres and becomes expensive. ' ...
         'Worst-case error vs an essentially exhaustive sum is about ' ...
         '2e-8 at 6 sigma (the default), ~1e-5 at 5, and ~1e-3 at 4. ' ...
         'Set mptDefaults(''truncationSigmas'', Inf) for the accuracy ' ...
         'floor (~7.43 sigma, ~1e-12 error). This warning shows only ' ...
         'once per session.']);
end
