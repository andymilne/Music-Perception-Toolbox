...
class TestMAET:
    ...
    def test_bind_returns_three_tuple(self):
        """bind_events returns (p_attr_bound, w_bound, groups_bound)."""
        p = [np.array([[1.0, 2.0, 3.0, 4.0]])]
        out = mpt.bind_events(p, None, None, 2)
        assert len(out) == 3

    def test_bind_order_1_is_identity(self):
        """L = 1 is the no-op: each attribute becomes one super-attr
        equal to itself."""
        p = [np.array([[1.0, 2.0, 3.0]]),
             np.array([[10.0, 20.0, 30.0]])]
        pb, _, gb = mpt.bind_events(p, None, None, 1)
        assert len(pb) == 2
        np.testing.assert_array_equal(pb[0], p[0])
        np.testing.assert_array_equal(pb[1], p[1])
        np.testing.assert_array_equal(gb, [0, 1])

    def test_bind_basic_n2_noncircular(self):
        """L = 2 non-circular: one input attribute becomes 2 super-attrs
        with N' = N - 1 events each."""
        p = [np.array([[1.0, 2.0, 3.0, 4.0]])]   # N = 4
        pb, _, gb = mpt.bind_events(p, None, None, 2, circular=False)
        # 2 super-attrs, N' = 3
        assert len(pb) == 2
        # super-attr 0: columns 0..2 of input = [1, 2, 3]
        np.testing.assert_array_equal(pb[0], [[1.0, 2.0, 3.0]])
        # super-attr 1: columns 1..3 of input = [2, 3, 4]
        np.testing.assert_array_equal(pb[1], [[2.0, 3.0, 4.0]])
        # both super-attrs stay in source attribute's group
        np.testing.assert_array_equal(gb, [0, 0])

    def test_bind_basic_n2_circular(self):
        """L = 2 circular: wraparound; N' = N."""
        p = [np.array([[1.0, 2.0, 3.0, 4.0]])]
        pb, _, _ = mpt.bind_events(p, None, None, 2, circular=True)
        assert len(pb) == 2
        np.testing.assert_array_equal(pb[0], [[1.0, 2.0, 3.0, 4.0]])
        np.testing.assert_array_equal(pb[1], [[2.0, 3.0, 4.0, 1.0]])

    def test_bind_scalar_broadcasts_to_all_attributes(self):
        """A scalar bind_orders broadcasts to all attributes."""
        p = [np.array([[1.0, 2.0, 3.0]]),
             np.array([[10.0, 20.0, 30.0]])]
        pb, _, gb = mpt.bind_events(p, None, None, 2)
        # 2 input attrs * 2 (L=2 each) = 4 super-attrs
        assert len(pb) == 4
        np.testing.assert_array_equal(gb, [0, 0, 1, 1])

    def test_bind_per_attribute_orders(self):
        """A length-A vector gives per-attribute orders."""
        p = [np.array([[1.0, 2.0, 3.0, 4.0]]),
             np.array([[10.0, 20.0, 30.0, 40.0]])]
        # L = [2, 3]: attr 0 -> 2 super-attrs, attr 1 -> 3 super-attrs
        pb, _, gb = mpt.bind_events(p, None, None, [2, 3])
        # A' = 5, N' = N - max(L) + 1 = 4 - 3 + 1 = 2
        assert len(pb) == 5
        # attr 0 (L=2), super-attrs columns 0..1 and 1..2 of input,
        # trailing-truncated to first 2 events: [1,2] and [2,3]
        np.testing.assert_array_equal(pb[0], [[1.0, 2.0]])
        np.testing.assert_array_equal(pb[1], [[2.0, 3.0]])
        # attr 1 (L=3): columns 0..2, 1..3, 2..4 of input;
        # natural N' = 2 already, no truncation: [10,20], [20,30], [30,40]
        np.testing.assert_array_equal(pb[2], [[10.0, 20.0]])
        np.testing.assert_array_equal(pb[3], [[20.0, 30.0]])
        np.testing.assert_array_equal(pb[4], [[30.0, 40.0]])
        # groups: attr 0's 2 super-attrs in group 1, attr 1's 3 in group 2
        np.testing.assert_array_equal(gb, [0, 0, 1, 1, 1])

    def test_bind_per_group_orders_via_g_vector(self):
        """A length-G vector (G != A) broadcasts within each group."""
        # A=3, G=2: attrs 0,1 in group 1, attr 2 in group 2.
        p = [np.array([[1.0, 2.0, 3.0]]),
             np.array([[10.0, 20.0, 30.0]]),
             np.array([[100.0, 200.0, 300.0]])]
        groups = [0, 0, 1]
        # L = [2, 1]: group 1 L=2, group 2 L=1.
        pb, _, gb = mpt.bind_events(p, None, groups, [2, 1])
        # A' = 2 + 2 + 1 = 5, N' = N - max(L) + 1 = 3 - 2 + 1 = 2.
        assert len(pb) == 5
        # attr 0 (group 1, L=2): super-attrs [1,2] and [2,3]
        np.testing.assert_array_equal(pb[0], [[1.0, 2.0]])
        np.testing.assert_array_equal(pb[1], [[2.0, 3.0]])
        # attr 1 (group 1, L=2): super-attrs [10,20] and [20,30]
        np.testing.assert_array_equal(pb[2], [[10.0, 20.0]])
        np.testing.assert_array_equal(pb[3], [[20.0, 30.0]])
        # attr 2 (group 2, L=1): no-op (with trailing-drop to N'=2);
        # super-attr = input truncated to first 2 events.
        np.testing.assert_array_equal(pb[4], [[100.0, 200.0]])
        np.testing.assert_array_equal(gb, [0, 0, 0, 0, 1])

    def test_bind_dict_form_per_group(self):
        """Dict form: each group can get its own scalar or per-attribute spec."""
        p = [np.array([[1.0, 2.0, 3.0]]),
             np.array([[10.0, 20.0, 30.0]]),
             np.array([[100.0, 200.0, 300.0]])]
        groups = [0, 0, 1]
        # Dict: group 0 (1-indexed group 1) at L=2 (scalar broadcast);
        # group 1 (1-indexed group 2) omitted = L=1 (no-op).
        pb, _, gb = mpt.bind_events(p, None, groups, {0: 2})
        # A' = 2 + 2 + 1 = 5, N' = 2.
        assert len(pb) == 5
        np.testing.assert_array_equal(pb[0], [[1.0, 2.0]])
        np.testing.assert_array_equal(pb[4], [[100.0, 200.0]])
    ...
    def test_bind_dict_form_per_attribute_within_group(self):
        """Dict form supports per-attribute orders inside a group."""
        p = [np.array([[1.0, 2.0, 3.0, 4.0]]),
             np.array([[10.0, 20.0, 30.0, 40.0]])]
        groups = [0, 0]
        # Per-attribute within group 0: [2, 3]
        pb, _, gb = mpt.bind_events(p, None, groups, {0: [2, 3]})
        # A' = 2 + 3 = 5, N' = N - max(L) + 1 = 4 - 3 + 1 = 2.
        assert len(pb) == 5
        np.testing.assert_array_equal(pb[0], [[1.0, 2.0]])
        np.testing.assert_array_equal(pb[1], [[2.0, 3.0]])
        np.testing.assert_array_equal(pb[2], [[10.0, 20.0]])
        np.testing.assert_array_equal(pb[3], [[20.0, 30.0]])
        np.testing.assert_array_equal(pb[4], [[30.0, 40.0]])
        np.testing.assert_array_equal(gb, [0, 0, 0, 0, 0])

    def test_bind_k_a_greater_than_one(self):
        """K_a > 1 attributes are accepted; each super-attr preserves K_a."""
        # K_a = 2, N = 4
        p = [np.array([[1.0, 2.0, 3.0, 4.0],
                       [10.0, 20.0, 30.0, 40.0]])]
        pb, _, _ = mpt.bind_events(p, None, None, 2)
        assert len(pb) == 2
        # Each super-attr is K_a x N' = 2 x 3
        assert pb[0].shape == (2, 3)
        assert pb[1].shape == (2, 3)
        np.testing.assert_array_equal(pb[0], [[1.0, 2.0, 3.0],
                                              [10.0, 20.0, 30.0]])
        np.testing.assert_array_equal(pb[1], [[2.0, 3.0, 4.0],
                                              [20.0, 30.0, 40.0]])

    def test_bind_empty_attribute_errors(self):
        """K_a = 0 is hard-rejected."""
        p = [np.zeros((0, 3))]
        with pytest.raises(ValueError, match=r"K_a\s*=\s*0|empty attribute"):
            mpt.bind_events(p, None, None, 2)

    def test_bind_order_too_high_noncircular_errors(self):
        """L > N (non-circular) raises."""
        p = [np.array([[1.0, 2.0, 3.0]])]
        with pytest.raises(ValueError, match="too high|window"):
            mpt.bind_events(p, None, None, 4)

    def test_bind_order_too_high_circular_errors(self):
        """L > N (circular) raises (silly to wrap more than once)."""
        p = [np.array([[1.0, 2.0, 3.0]])]
        with pytest.raises(ValueError, match="window|exceeds"):
            mpt.bind_events(p, None, None, 4, circular=True)

    def test_bind_zero_order_errors(self):
        """L = 0 is invalid (L >= 1 required)."""
        p = [np.array([[1.0, 2.0, 3.0]])]
        with pytest.raises(ValueError, match=r"positive integer|>= 1"):
            mpt.bind_events(p, None, None, 0)

    def test_bind_negative_order_errors(self):
        """Negative bind_orders are rejected."""
        p = [np.array([[1.0, 2.0, 3.0]])]
        with pytest.raises(ValueError, match=r"positive integer|>= 1"):
            mpt.bind_events(p, None, None, -1)

    def test_bind_non_integer_order_errors(self):
        """Non-integer bind_orders are rejected."""
        p = [np.array([[1.0, 2.0, 3.0]])]
        with pytest.raises(ValueError, match=r"integer"):
            mpt.bind_events(p, None, None, 1.5)

    def test_bind_scalar_weight_passes_through(self):
        """A scalar weight stays a scalar (broadcast in build_exp_tens
        recovers the product across the L_a super-attributes)."""
        p = [np.array([[1.0, 2.0, 3.0, 4.0]])]
        _, wb, _ = mpt.bind_events(p, 0.7, None, 2)
        assert wb == 0.7

    def test_bind_event_dependent_weight_slices_per_lag(self):
        """Event-dependent weights are sliced by lag for each super-attr."""
        p = [np.array([[1.0, 2.0, 3.0, 4.0]])]
        w = [np.array([[0.1, 0.2, 0.3, 0.4]])]   # 1 x N
        _, wb, _ = mpt.bind_events(p, w, None, 2)
        # 2 super-attrs, N' = 3
        np.testing.assert_array_equal(wb[0], [[0.1, 0.2, 0.3]])
        np.testing.assert_array_equal(wb[1], [[0.2, 0.3, 0.4]])

    def test_bind_super_attrs_stay_in_source_group(self):
        """Each input attribute's L_a super-attrs are in its source group."""
        p = [np.array([[1.0, 2.0, 3.0]]),
             np.array([[10.0, 20.0, 30.0]]),
             np.array([[100.0, 200.0, 300.0]])]
        groups = [0, 1, 0]   # A=3, G=2; attr 0 and 2 in group 1, attr 1 in group 2
        _, _, gb = mpt.bind_events(p, None, groups, [2, 2, 3])
        # attr 0 (group 1, L=2): 2 super-attrs in group 1
        # attr 1 (group 2, L=2): 2 super-attrs in group 2
        # attr 2 (group 1, L=3): 3 super-attrs in group 1
        np.testing.assert_array_equal(gb, [0, 0, 1, 1, 0, 0, 0])

    def test_bind_a_prime_equals_sum_of_orders(self):
        """A' = sum(L_a)."""
        p = [np.array([[1.0, 2.0, 3.0, 4.0]]),
             np.array([[10.0, 20.0, 30.0, 40.0]]),
             np.array([[100.0, 200.0, 300.0, 400.0]])]
        pb, _, _ = mpt.bind_events(p, None, None, [1, 2, 3])
        assert len(pb) == 1 + 2 + 3

    def test_bind_commutes_with_difference(self):
        """D ∘ B = B ∘ D up to numerical equality (value-wise).
        Single attribute, scalar orders."""
        # Single K_a = 1 attribute, N = 5.
        p = [np.array([[0.0, 3.0, 7.0, 12.0, 18.0]])]
        # D then B
        pd, _, gd = mpt.difference_events(p, None, None, 1)
        pdb, _, _ = mpt.bind_events(pd, None, gd, 2)
        # B then D
        pb, _, gb = mpt.bind_events(p, None, None, 2)
        # Now difference each of the 2 super-attrs separately.
        # bind_events output has A' = 2 super-attrs in 1 group; we
        # difference both at order 1.
        pbd, _, _ = mpt.difference_events(pb, None, gb, 1)
        # Both routes give 2 super-attrs with N'' = 5 - 1 - 2 + 1 = 3 events
        assert len(pdb) == len(pbd) == 2
