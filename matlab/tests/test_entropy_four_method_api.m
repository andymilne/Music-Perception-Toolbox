...
%
%  Tests for entropyMaet's method kwarg in its post-Stage-2 form:
...
dens1d = buildMaet([100, 200, 300], ones(1, 3), 20, 1, false, false, 0, ...
                       'verbose', false);

% --- British 'normalised' alias matches 'normalized' ---
H_us = entropyMaet(dens1d, 'method', 'normalized', ...
                       'nPointsPerDim', 200, 'xMin', 50, 'xMax', 350, ...
                       'verbose', false);
H_uk = entropyMaet(dens1d, 'method', 'normalised', ...
                       'nPointsPerDim', 200, 'xMin', 50, 'xMax', 350, ...
                       'verbose', false);
...
results{end,2}   = throwsErrorWithId(@() entropyMaet(dens1d, ...
    'method', 'bogus', 'nPointsPerDim', 200, 'verbose', false), ...
    'entropyMaet:badMethod');

% =====================================================================
% Grid-requirement policy
% =====================================================================

dens = buildMaet([100, 200, 300], ones(1, 3), 20, 1, false, false, 0, ...
                     'verbose', false);

% --- Shannon without explicit grid errors ---
results{end+1,1} = 'gridReq: shannon without nPointsPerDim errors';
results{end,2}   = throwsErrorWithId(@() entropyMaet(dens, ...
    'method', 'shannon', 'verbose', false), ...
    'entropyMaet:gridRequired');
...
N = 200;
xMin = 50; xMax = 350;
H_shan = entropyMaet(dens, 'method', 'shannon', ...
                         'nPointsPerDim', N, 'xMin', xMin, 'xMax', xMax, ...
                         'verbose', false);
H_norm = entropyMaet(dens, 'method', 'normalized', ...
                         'nPointsPerDim', N, 'xMin', xMin, 'xMax', xMax, ...
                         'verbose', false);
...
densSE = buildMaet([6000, 6300, 6700], ones(1, 3), 20, 1, ...
                       false, false, 0, 'verbose', false);
hDiff1D = entropyMaet(densSE, 'method', 'differential', 'verbose', false);
N_ref = 25000;
xMinRef = 5500; xMaxRef = 7200;
H_shanRef = entropyMaet(densSE, 'method', 'shannon', ...
                            'nPointsPerDim', N_ref, ...
...
densA = buildMaet([6500, 6600], ones(1, 2), 15, 1, ...
                      false, false, 0, 'verbose', false);
densB = buildMaet([6000, 6400, 6800, 7200], ones(1, 4), 15, 1, ...
                      false, false, 0, 'verbose', false);
h_A = entropyMaet(densA, 'method', 'differential', 'verbose', false);
h_B = entropyMaet(densB, 'method', 'differential', 'verbose', false);
...
h_ts6 = entropyMaet(densSE, 'method', 'differential', ...
                        'truncationSigmas', 6.0, 'verbose', false);
h_ts8 = entropyMaet(densSE, 'method', 'differential', ...
...
h_tsInf = entropyMaet(densSE, 'method', 'differential', ...
                          'truncationSigmas', Inf, 'verbose', false);
h_tsFloor = entropyMaet(densSE, 'method', 'differential', ...
...
period = 12.0;
sigmaEff = sqrt(2);
densMA = buildMaet({P2, P2}, {W2, W2}, [sigmaEff sigmaEff], ...
                       [1, 1], [false false], [true true], [period period], ...
                       'verbose', false);
hDiffMA = entropyMaet(densMA, 'method', 'differential', ...
                          'truncationSigmas', 6, 'verbose', false);
H_shanMA = entropyMaet(densMA, 'method', 'shannon', ...
                           'nPointsPerDim', 500, ...
                           'truncationSigmas', 6, 'verbose', false);
...
% entropyMaet supports five input forms; shannon and normalized
...
sa_w     = [1, 1, 1];
sa_sigma = 20.0;
sa_dens  = buildMaet(sa_p, sa_w, sa_sigma, 1, false, false, 0, ...
    'verbose', false);
sa_dens_list = {
    buildMaet(sa_p,        sa_w, sa_sigma, 1, false, false, 0, 'verbose', false), ...
    buildMaet(sa_p +  50,  sa_w, sa_sigma, 1, false, false, 0, 'verbose', false), ...
    buildMaet(sa_p + 100,  sa_w, sa_sigma, 1, false, false, 0, 'verbose', false)};
...
ma_dens   = buildMaet(ma_pAttr, ma_w, [20, 5], [1, 1], ...
    [false, false], [false, false], [0, 0], 'verbose', false);

% --- Shannon: 5 input forms ---
h_sa_dens   = entropyMaet(sa_dens, 'method', 'shannon', ...
    'nPointsPerDim', 200, 'xMin', 0, 'xMax', 500, 'verbose', false);
h_sa_list   = entropyMaet(sa_dens_list, 'method', 'shannon', ...
    'nPointsPerDim', 200, 'xMin', 0, 'xMax', 500, 'verbose', false);
h_sa_raw    = entropyMaet(sa_p, sa_w, sa_sigma, 1, false, false, 0, ...
    'method', 'shannon', 'nPointsPerDim', 200, ...
    'xMin', 0, 'xMax', 500, 'verbose', false);
h_sa_batch  = entropyMaet(sa_P, sa_W, sa_sigma, 1, false, false, 0, ...
    'method', 'shannon', 'nPointsPerDim', 200, ...
    'xMin', 0, 'xMax', 500, 'verbose', false);
h_ma_dens   = entropyMaet(ma_dens, 'method', 'shannon', ...
...
n_sa_dens   = entropyMaet(sa_dens, 'method', 'normalized', ...
    'nPointsPerDim', 200, 'xMin', 0, 'xMax', 500, 'verbose', false);
n_sa_list   = entropyMaet(sa_dens_list, 'method', 'normalized', ...
    'nPointsPerDim', 200, 'xMin', 0, 'xMax', 500, 'verbose', false);
n_sa_raw    = entropyMaet(sa_p, sa_w, sa_sigma, 1, false, false, 0, ...
    'method', 'normalized', 'nPointsPerDim', 200, ...
    'xMin', 0, 'xMax', 500, 'verbose', false);
n_sa_batch  = entropyMaet(sa_P, sa_W, sa_sigma, 1, false, false, 0, ...
    'method', 'normalized', 'nPointsPerDim', 200, ...
    'xMin', 0, 'xMax', 500, 'verbose', false);
n_ma_dens   = entropyMaet(ma_dens, 'method', 'normalized', ...
    'nPointsPerDim', 80, 'xMin', [0, 0], 'xMax', [500, 50], 'verbose', false);
inUnit = @(x) x >= 0 && x <= 1;
...
% comes back.
d_sa_dens = entropyMaet(sa_dens, 'method', 'differential', 'verbose', false);
d_sa_raw  = entropyMaet(sa_p, sa_w, sa_sigma, 1, false, false, 0, ...
    'method', 'differential', 'verbose', false);
d_ma_dens = entropyMaet(ma_dens, 'method', 'differential', ...
...
results{end,2}   = throwsError(@() entropyMaet( ...
...
r_sa_dens = entropyMaet(sa_dens, 'method', 'renyi2', 'verbose', false);
r_sa_raw  = entropyMaet(sa_p, sa_w, sa_sigma, 1, false, false, 0, ...
    'method', 'renyi2', 'verbose', false);
r_ma_dens = entropyMaet(ma_dens, 'method', 'renyi2', 'verbose', false);
...
% Passing 'normalize' (any value) to entropyMaet raises a TypeError-
% equivalent (id entropyMaet:normalizeRemoved). Tested across each
...
for mi = 1:numel(methods_to_test)
    for vi = 1:numel(values_to_test)
        ok_one = false;
        try
            entropyMaet(sa_dens, 'method', methods_to_test{mi}, ...
                'normalize', values_to_test{vi}, ...
                'nPointsPerDim', 100, 'xMin', 0, 'xMax', 500, ...
                'verbose', false);
        catch ME
            ok_one = strcmp(ME.identifier, 'entropyMaet:normalizeRemoved');
            last_msg = ME.message;
        end
        all_migration_ok = all_migration_ok && ok_one;
    end
end
results{end+1,1} = 'entropyMaet: legacy normalize kwarg raises migration error';
results{end,2}   = all_migration_ok;

% Migration message points to replacement methods.
results{end+1,1} = 'entropyMaet: migration message names ''normalized'' and ''shannon''';
...
dens_diff_ref = buildMaet([6000, 6300, 6700], ones(1,3), 20.0, 1, ...
    false, false, 0, 'verbose', false);
h_ts3 = entropyMaet(dens_diff_ref, 'method', 'differential', ...
    'truncationSigmas', 3, 'verbose', false);
h_ts6 = entropyMaet(dens_diff_ref, 'method', 'differential', ...
    'truncationSigmas', 6, 'verbose', false);
results{end+1,1} = 'differential ts=3: finite and within 1e-2 of ts=6';
results{end,2}   = isfinite(h_ts3) && abs(h_ts3 - h_ts6) < 1e-2;

dens_concentrated = buildMaet([6500, 6600], ones(1, 2), 15.0, 1, ...
    false, false, 0, 'verbose', false);
dens_spread       = buildMaet([6000, 6400, 6800, 7200], ones(1, 4), 15.0, ...
    1, false, false, 0, 'verbose', false);
h_conc   = entropyMaet(dens_concentrated, 'method', 'differential', ...
    'truncationSigmas', 3, 'verbose', false);
h_spread = entropyMaet(dens_spread, 'method', 'differential', ...
    'truncationSigmas', 3, 'verbose', false);
