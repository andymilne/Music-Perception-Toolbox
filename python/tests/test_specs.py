"""Tests for the canonical ``specs=`` form of build_maet (3c-i).
...
def test_flat_specs_match_old_positional():
    ...
    d_old = build_maet(P2, None, [50.0, 30.0], [2, 1], [True, False],
...
def test_nested_spec_matches_nested_kwarg():
    ...
    nsp = dict(tags=[0, 0, 1, 1], r=[2, 2], exch=[True, False], rel="outermost")
...
def test_specs_guards():
    ...
    with pytest.raises(ValueError):
        build_maet(P2, None, [50.0, 30.0], [2, 1], [True, False],
...
def test_flat_specs_scalar_broadcast():
    ...
    assert s == [{"r": 2, "rel": False, "exch": True}] * 3


def test_nested_spec_omitted_exch_defaults_true():
    """A nested spec without 'exch' defaults every level symmetric."""
    pv = [np.array([[0.0], [4.0], [7.0], [11.0]])]
