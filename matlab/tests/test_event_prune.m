...
H_auto = entropyExpTens(p_w, w_w, 0.5, 1, g_w, false, false, 0.0, ...
    'method', 'renyi2', 'verbose', false);
keep = event_w > 0;
p_w_pruned = cellfun(@(p) p(:, keep), p_w, 'UniformOutput', false);
w_w_pruned = cellfun(@(w) w(:, keep), w_w, 'UniformOutput', false);
H_manual = entropyExpTens(p_w_pruned, w_w_pruned, 0.5, 1, g_w, ...
...
[p_wb, w_wb, g_wb] = weightEvents({pitchesBig, timesBig}, ...
    {ones(1, N_big), ones(1, N_big)}, [1 2], ...
    2, 1, timesBig(round(N_big / 2)), 0.0, false, 0.0, ...
    'sd', 1.0, 'deleteInput', true);
t0 = tic;
H_big = entropyExpTens(p_wb, w_wb, 0.5, 1, g_wb, false, false, 0.0, ...
    'method', 'renyi2', 'verbose', false);
elapsed = toc(t0);
