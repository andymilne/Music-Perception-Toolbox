...
%TEST_COSSIM_CENTRES_ROUTING  Stage 2b: simMaet centres-IP routing.
%
%   Verifies that simMaet routes the single-multiset centres-IP through
...
%
%   Coverage:
%     - simMaet(dens_x, dens_y, 'truncationSigmas', k) returns the
...
function [dens_x, dens_y] = buildPair(isRel, isPer)
    ...
    dens_x = buildMaet(p_x(:), ones(3, 1), sigma, r, ...
        isRel, isPer, Jval, 'verbose', false);
    dens_y = buildMaet(p_y(:), ones(3, 1), sigma, r, ...
        isRel, isPer, Jval, 'verbose', false);
end
...
function test_abs_nonper_exact_vs_truncated(testCase)
    ...
    s_exact = simMaet(dens_x, dens_y, ...
        'method', 'bulger', 'verbose', false);
    s_trunc = simMaet(dens_x, dens_y, ...
...
function test_rel_per_unaffected_by_truncation_kwarg(testCase)
    ...
    s_default = simMaet(dens_x, dens_y, ...
        'method', 'bulger', 'verbose', false);
    s_with = simMaet(dens_x, dens_y, ...
        'method', 'bulger', 'truncationSigmas', 6, 'verbose', false);
