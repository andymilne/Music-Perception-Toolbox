...
dens = mpt.build_maet(pm_back, verbose=False)
...
for sigma_pitch in (0.05, 0.15, 0.6, 2.0):
    ...
    sim = float(mpt.sim_maet(mpt.build_maet(pm, **kw),
                                     mpt.build_maet(pm_up, **kw),
                                     verbose=False))
...
print("  A nested attribute's r, rel and exch are per-level, so an override")
...
d_log = mpt.build_maet(pm_log, sigma=[0.05], verbose=False)
...
print("=== 7. A kernel covariance, as a flag and three scalars ===\n")
...
# carries.
cov_csv = (
    "name,sigma,r,rel,per,P,exch,n = 1,n = 2\n"
...
d_c = mpt.build_maet(pm_c, verbose=False)
