"""How often does the impossible-value check fire, and is it ever needed?
...
for a value no inner product can take --- a non-finite inner product, a
negative Gram diagonal, or a cosine outside [-1, 1] by more than 1e-6 ---
and recomputes by enumeration when it finds any. That recompute is the
only place in the flat path where a route is chosen after the fact, so
its incidence decides whether the check can simply raise instead.
...
a firing can be read as either a genuine impossibility or a false alarm.
"""
import warnings
...
def main():
    ...
    print("Impossible-value check: how often does it fire?\n")
