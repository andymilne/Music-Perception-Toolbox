...
%  entropyMaet and simMaet. The prune lives in
%  internal.prunedMaet and is the MATLAB counterpart of the Python
...
%% -----------------------------------------------------------------
%% Liveness rule, observed through internal.prunedMaet
%% -----------------------------------------------------------------

% Single-multiset: live iff finite and nonzero. Zeros kill; finite
% nonzero (incl. negative) survive. prunedMaet returns a MaetDensity
...
densSingleMultiset = buildMaet([60 62 64 66 68], [1 0 0 2 -3], 0.5, 1, ...
                      false, false, 0);
prSingleMultiset = internal.singleMultisetView(internal.prunedMaet(densSingleMultiset));
...
pTime  = [0 1 2];
densMA = buildMaet({pPitch, pTime}, {wPitch, ones(1,3)}, ...
                      [0.5 0.15], [2 1], ...
                      [false false], [false false], [0 0]);
prMA = internal.prunedMaet(densMA);
...
densMA2 = buildMaet({[60 62 64], [0 1 2]}, ...
                       {ones(1,3), [1 0 1]}, ...
                       [0.5 0.15], [1 1], ...
                       [false false], [false false], [0 0]);
prMA2 = internal.prunedMaet(densMA2);
...
densLiveSingleMultiset = buildMaet([60 62 64], [1 1 2], 0.5, 1, false, false, 0);
results{end+1,1} = 'single-multiset prune returns density unchanged when all live';
results{end,2}   = isequaln(internal.prunedMaet(densLiveSingleMultiset), densLiveSingleMultiset);

densLiveMA = buildMaet({[60 62 64], [0 1 2]}, {ones(1,3), ones(1,3)}, ...
                          [0.5 0.15], [1 1], ...
                          [false false], [false false], [0 0]);
results{end+1,1} = 'MA prune returns density unchanged when all live';
results{end,2}   = isequaln(internal.prunedMaet(densLiveMA), densLiveMA);
...
H_sa_with = entropyMaet(buildMaet(pSingleMultiset, wSingleMultisetd, 0.5, 1, false, false, 0), ...
                           'method', 'renyi2', 'verbose', false);
H_sa_without = entropyMaet( ...
    buildMaet(pSingleMultiset(keepSingleMultiset), wSingleMultiset(keepSingleMultiset), 0.5, 1, false, false, 0), ...
    'method', 'renyi2', 'verbose', false);
...
dMA_with = buildMaet({pPm, pTm}, {wPm, ones(1,8)}, ...
                        [0.5 0.15], [1 1], ...
                        [false false], [false false], [0 0]);
dMA_without = buildMaet({pPm(keepMA), pTm(keepMA)}, ...
                           {wPm(keepMA), ones(1, nnz(keepMA))}, ...
                           [0.5 0.15], [1 1], ...
                           [false false], [false false], [0 0]);
H_ma_with = entropyMaet(dMA_with, 'method', 'renyi2', 'verbose', false);
H_ma_without = entropyMaet(dMA_without, 'method', 'renyi2', 'verbose', false);
...
keepX = wX ~= 0;
dQ = buildMaet([61 63 65], [1 1 1], 0.5, 1, false, false, 0);
s_sa_with = simMaet( ...
    buildMaet(pX, wX, 0.5, 1, false, false, 0), dQ, 'verbose', false);
s_sa_without = simMaet( ...
    buildMaet(pX(keepX), wX(keepX), 0.5, 1, false, false, 0), dQ, ...
    'verbose', false);
...
keepXm = wXp ~= 0;
dQm = buildMaet({[61 64 67], [0.1 0.4 0.7]}, {ones(1,3), ones(1,3)}, ...
                   [0.5 0.15], [1 1], ...
                   [false false], [false false], [0 0]);
dXm_with = buildMaet({pXp, pXt}, {wXp, ones(1,6)}, ...
                        [0.5 0.15], [1 1], ...
                        [false false], [false false], [0 0]);
dXm_without = buildMaet({pXp(keepXm), pXt(keepXm)}, ...
                           {wXp(keepXm), ones(1, nnz(keepXm))}, ...
                           [0.5 0.15], [1 1], ...
                           [false false], [false false], [0 0]);
s_ma_with = simMaet(dXm_with, dQm, 'verbose', false);
s_ma_without = simMaet(dXm_without, dQm, 'verbose', false);
...
[p_wb, w_wb, g_wb] = unpackPreMaet(weightEvents({pitchesBig, timesBig}, {ones(1, N_big), ones(1, N_big)}, 2, 1, timesBig(round(N_big / 2)), 0.0, 'sd', 1.0, 'dropInputAttr', true));
t0 = tic;
H_big = entropyMaet(p_wb, w_wb, 0.5, 1, false, false, 0.0, ...
    'method', 'renyi2', 'verbose', false);
elapsed = toc(t0);
...
densDeadMA = buildMaet({[60 62 64], [0 1 2]}, {zeros(1,3), ones(1,3)}, ...
                          [0.5 0.15], [1 1], ...
                          [false false], [false false], [0 0]);
results{end+1,1} = 'all-dead MA prune yields N = 0';
results{end,2}   = (internal.prunedMaet(densDeadMA).N == 0);
H_dead_ma = entropyMaet(densDeadMA, 'method', 'renyi2', 'verbose', false);
...
densDeadSingleMultiset = buildMaet([60 62 64], [0 0 0], 0.5, 1, false, false, 0);
H_dead_sa = entropyMaet(densDeadSingleMultiset, 'method', 'renyi2', 'verbose', false);
