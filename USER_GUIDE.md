...
Two analytical methods are available for computing the expectation-tensor inner product, exposed via the `'method'` keyword on `cosSimExpTens` (and on `evalExpTens` / `entropyExpTens` where applicable; see below):

- **Bulger's method** (`method='bulger'`) — the v2.1 decomposition (previously called `'pairwise'`). Organises the slot-tuple sum by combinations on one side and permutations on the other, using the within-tuple multinomial symmetry to avoid enumerating equivalent orderings. Inner-product-only. Has essentially no per-call fixed overhead and tends to win at small $r$ and small $K$.

- **The Möbius method** (`method='mobius'`) — new in v2.2 (previously called `'orbit'`). Rewrites the slot-tuple sum via Möbius inversion on the partition lattice: the distinct-index constraint becomes an alternating sum over set partitions, with each partition's term factorising across its blocks. For the inner product this combines with **orbit collapse** under the joint slot-permutation symmetry, reducing the partition-pair count from $B_r^2$ to $|\Omega_r|$ orbit equivalence classes (4, 10, 33, 92, 306, 948, 3210 for $r = 2, \ldots, 8$). Beyond the inner product, the Möbius method also applies to **point evaluation** $T(q)$ (where it is an efficiency alternative to the explicit centres-array path) and **total mass** $\int T \, \mathrm{d}z$ (used internally for Rényi-2 entropy). Carries fixed per-call overhead (orbit-table lookup, $|\Omega_r|$ einsum contractions) and tends to win at large $r$ or large $K$ where the slot-tuple loop in Bulger's method blows up. The underlying inclusion-exclusion technique was first published in Milne et al. 2011 (framed there as inclusion-exclusion rather than Möbius inversion on the partition lattice; the two are equivalent formulations) and has been used in MPT since v1 for numerical computation of discrete tensors; v2.2 applies it as a closed-form analytical decomposition.

The two methods agree to floating-point precision on inputs where both are valid (the inner-product case). The `'auto'` setting (default) runs a per-call cost model that picks the cheaper of the two for each call; for SA cosine similarity it additionally times each candidate on a small subset of the workload when the cost-model pre-screen is not decisive.

| Method | Description |
|:---|:---|
| `'auto'` (default) | Per-call cost-model dispatcher (with timing-probe fallback for SA cosine similarity). Picks between Bulger's and the Möbius method based on $r$, $K$, $N$, and $\sigma/\text{period}$. Agrees with explicit `'bulger'` to floating-point precision in the regimes where both are valid. |
| `'bulger'` | Bulger's method (v2.1; previously `'pairwise'`). Inner-product-only. Direct enumeration of ordered $r$-tuples with kernel matrix multiplication. Exact and stable for any input. |
| `'mobius'` | The Möbius method (v2.2; previously `'orbit'`). Closed-form analytical decomposition via Möbius inversion on the partition lattice, combined with orbit collapse in the inner-product case. Subject to a precision guard ($K - r \ge 2$); the `'auto'` setting falls back to Bulger's method outside this regime. |
| `'centres'` (eval only) | Explicit centres-array evaluation. Same machinery as v2.1 eval. |
...
Most callers should leave `method` at the default `'auto'`. Override only when measuring Möbius-vs-Bulger agreement, debugging cancellation issues, or forcing a specific method for benchmarking.

The companion `'cancellationThreshold'` keyword on `cosSimExpTens` (default `1e-12`) guards the Möbius method's alternating partition sum against catastrophic cancellation: if the worst-case ratio drops below the threshold, the dispatcher falls back to Bulger's method. Lowering the threshold relaxes the guard; raising it tightens it (forcing Bulger's method more aggressively).
