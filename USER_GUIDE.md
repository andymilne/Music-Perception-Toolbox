...
The MAET framework provides three preprocessing helpers that transform an input event sequence into a derived sequence before tensor construction: `differenceEvents` (replacing the sequence with inter-event differences), `bindEvents` (gathering n consecutive events into a single super-event), and `translateEvents` (shifting selected attributes' values by a chosen offset). `differenceEvents` and `bindEvents` act per group; `translateEvents` is per-attribute, with broadcast-within-group as a common case. All three propagate per-event weights through the toolbox's standard probability-of-perception reading where applicable. They differ in what the derived sequence represents and in their handling of multi-value attributes. `differenceEvents` is restricted to single-value ($K_a = 1$) attributes because column-wise subtraction across adjacent events requires a slot correspondence that the within-event slot exchangeability does not provide. `bindEvents` and `translateEvents` accept any $K_a \ge 1$: `bindEvents` because each output attribute simply carries the full slot vector of its underlying event with no cross-event slot alignment imposed; `translateEvents` because every slot in the per-event multiset is shifted by the same offset and so no slot-correspondence question arises. Each operation is useful on its own. Differencing and binding can be chained — differencing first, since it requires $K_a = 1$ input and produces $K_a = 1$ output, which `bindEvents` then accepts — and translation can be combined with either, before or after, since it leaves shapes unchanged. The MAET built on a derived sequence aggregates across its entries in the usual way, so the resulting density is a distribution over derived values rather than a function of position in the derived sequence.
...
**Event translation.** `translateEvents` shifts selected attributes' values by a chosen offset, leaving non-selected attributes unchanged. The output `pAttrTranslated` feeds directly into `buildExpTens` without further massaging; weights pass through unchanged, since uniform translation does not affect any event's probability of perception. The shift is applied uniformly to every slot in each event's per-attribute multiset, so the operation is well-defined for any $K_a \ge 1$ — unlike windowing, where a per-event scalar weight would have to summarise multiple slots' distances from a localising centre, translation has no analogous obstruction at $K_a > 1$. For absolute groups the operation is `value + mu`, regardless of periodicity: the wrapped periodic Gaussian kernel of `buildExpTens` is invariant under any additive shift by a multiple of $P$, so periodic groups need no canonical wrap of the translated values — the kernel handles periodicity downstream. For relative groups translation is a structural no-op — a uniform shift cancels in every within-tuple difference, so the relative MAET is unchanged — and the function emits a `translateEvents:noOpRelative` (MATLAB) or `TranslateEventsNoOpWarning` (Python) and leaves the group unchanged.

`translateEvents` accepts offsets in two forms: a single numeric block (for uniform broadcast or fully per-attribute layouts), or a container keyed by group (for mixed-per-group layouts, e.g.\ broadcast within one group and per-attribute within another in the same call).
...
| `translateEvents` | `translate_events` | Tensor core (preprocessing) |
...
| `translateEvents` | `translate_events` | *Preprocessing:* shift selected attributes by a chosen offset; per-attribute or per-group, with optional matrix-form sweep over $M$ offsets in one call |
...
**translateEvents(pAttr, groups, offsets, isRel, isPer, periods)**

Cross-event preprocessing for MAET input. Shifts selected attributes' values by a chosen offset, returning a transformed `pAttrTranslated` that feeds directly into `buildExpTens`. Weights are unaffected and are not part of the signature. Semantics are by group geometry: absolute groups are translated as `value + mu` (unwrapped — the periodic kernel handles wrap downstream when `isPer[g] = True`); relative groups pass through unchanged and emit a single `translateEvents:noOpRelative` (MATLAB) / `TranslateEventsNoOpWarning` (Python) per call.
...
| `demo_translate_sweep.py` | `demo_translateSweep` | Pre-tensor sliding-comparison sweep via `translate_events` matrix form and the raw-MA scalar-vs-list mode of `cos_sim_exp_tens` |
