%% test_dispatch_sm_cossim.m — v3 single multiset method dispatch in simMaet
%
...
%% ---- simMaet method kwarg validation ----

% Bad method string raises informative error.
ok_badMethod = false;
try
    simMaet([0 4 7], [], [0 4 7], [], 12, 2, false, true, 1200, ...
        'method', 'bogus', 'verbose', false);
catch ME
    ok_badMethod = strcmp(ME.identifier, 'simMaet:badMethod');
end
results{end+1,1} = 'dispatch.single multiset: bad method string raises simMaet:badMethod';
...
s_auto = simMaet(p_x, w_x, p_y, w_y, sigma, r, isRel, isPer, period, ...
    'verbose', false);
s_pair = simMaet(p_x, w_x, p_y, w_y, sigma, r, isRel, isPer, period, ...
    'method', 'bulger', 'verbose', false);
s_orb  = simMaet(p_x, w_x, p_y, w_y, sigma, r, isRel, isPer, period, ...
...
w_y = ones(6, 1);
sigma = 50; r = 2;
s_auto2 = simMaet(p_x, w_x, p_y, w_y, sigma, r, false, false, 0, ...
    'verbose', false);
s_pair2 = simMaet(p_x, w_x, p_y, w_y, sigma, r, false, false, 0, ...
    'method', 'bulger', 'verbose', false);
...
p_y = p_x;
w_y = w_x;
s_auto3 = simMaet(p_x, w_x, p_y, w_y, 50, 3, false, false, 0, ...
    'verbose', false);
s_pair3 = simMaet(p_x, w_x, p_y, w_y, 50, 3, false, false, 0, ...
    'method', 'bulger', 'verbose', false);
...
w = 0.5 + rand(7, 1);
s_self_pair = simMaet(p, w, p, w, 30, 3, false, false, 0, ...
    'method', 'bulger', 'verbose', false);
s_self_orb  = simMaet(p, w, p, w, 30, 3, false, false, 0, ...
    'method', 'mobius', 'verbose', false);
...
warnState = warning('on', 'simMaet:relPerAllImage');
lastwarn('');
s_warn = simMaet(p_x, w_x, p_y, w_y, 60, 3, true, true, 1200, ...
    'verbose', true);
[wmsg, wid] = lastwarn;
warning(warnState);
results{end+1,1} = 'dispatch.single multiset: rel+per sigma/P=0.05 stays silent (warning retired)';
results{end,2}   = ~strcmp(wid, 'simMaet:relPerAllImage');
% Auto still takes the all-image (Möbius) path at this sigma/P.
s_mob_warn = simMaet(p_x, w_x, p_y, w_y, 60, 3, true, true, 1200, ...
    'method', 'mobius', 'verbose', false);
...
p_y = [0; 5; 7; 12; 14];
w_y = ones(5, 1);
dens_x_skinny = buildMaet(p_x, w_x, 30, 3, false, false, 0, 'verbose', false);
dens_y_skinny = buildMaet(p_y, w_y, 30, 3, false, false, 0, 'verbose', false);
...
s_skinny_orb = simMaet(dens_x_skinny, dens_y_skinny, ...
    'method', 'mobius', 'verbose', false);
s_pair_check = simMaet(p_x, w_x, p_y, w_y, 30, 3, false, false, 0, ...
    'method', 'bulger', 'verbose', false);
