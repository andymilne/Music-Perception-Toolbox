...
- **The direct method** (`method='direct'`) — the brute-force baseline for the inner product. Every ordered $r$-tuple on each side is enumerated and its kernel contribution summed, with no Bulger combinations-vs-permutations factoring and no Möbius alternating sum. **Inner-product only.** Cost $O\!\left(\frac{K_x!}{(K_x-r)!} \cdot \frac{K_y!}{(K_y-r)!}\right)$ — expensive at moderate $K$ relative to Bulger or Möbius, but exact at any $K \ge r$ and immune to cancellation (all summands are non-negative). In the unsafe regime ($K_\text{eff} - r < 2$, where the Möbius precision guard refuses to fire) both direct and Bulger are precise alternatives, but the dispatcher chooses between them per consumer: the SA cosine-similarity dispatcher `'auto'` routes here to **Bulger's method**, while the **MA per-attribute inner-product matrix** routes unsafe-event pairs to **direct enumeration**, since its K-grouped batched form vectorises across event pairs as a single tensor contraction more cleanly than Bulger's combinations-vs-permutations factoring (and the ordered-$r$-tuple count is small at the low $K_\text{eff}$ that defines this regime). Users can also force `method='direct'` on the SA path for benchmarking or sanity-checking. On `evalExpTens`, `'direct'` is accepted as a synonym for `'centres'` so the same keyword works consistently across functions; there is no second meaning for eval (eval has no Möbius alternating sum to avoid).
...
- **Dispatch decisions** from `evalExpTens` and `cosSimExpTens`: short messages naming the path the dispatcher selected. The unprobed form is just `cosSimExpTens: chose 'bulger' path.`; the probed form (when the dispatcher timed both candidates on a small sample) adds the estimate, e.g. `cosSimExpTens: chose 'bulger' path (estimated 1.5 min); Ctrl+C to cancel.`. Both are throttled to once per top-level user call per unique `(function, chosen)` pair: nested toolbox calls within one user call share a seen-set so a batched-raw loop produces one announce per unique chosen path (not one per item, and not one per routing reason — two different reasons leading to the same chosen path collapse to a single announce). Each new top-level call re-announces. The seen-set is cleared automatically on every top-level toolbox entry, and also explicitly by `mptDefaults('reset')` / `mpt.reset_defaults()`.
