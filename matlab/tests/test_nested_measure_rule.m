...
nmr_TAGS = [0 0 1 1];
nmr_SPEC = struct('tags', nmr_TAGS, 'r', [2 2], 'exch', [true true], ...
                  'rel', [0 1]);
...
nmr_SPEC9 = struct('tags', nmr_TAGS9, 'r', [2 2], 'exch', [true true], ...
                   'rel', [0 1]);
...
for nmr_ii = 1:numel(nmr_sops)
    ...
    simMaet(nmrDens(nmr_px9, nmr_sigma, nmr_SPEC9, [], nmr_P), ...
...
for nmr_ii = 1:numel(nmr_methods)
    ...
    simMaet(nmr_lowDx, nmr_lowDy, 'method', nmr_methods{nmr_ii}, ...
                  'verbose', false);
    nmr_r = internal.lastNestedRoutes();
    nmr_ok = numel(nmr_r) == 1;
...
results{end, 2} = throwsErrorWithId(@() simMaet(nmr_hiDx, nmr_hiDy, ...
    'method', 'centres', 'verbose', false), 'simMaet:centresUnavailable') ...
    && errorMessageContains(@() simMaet(nmr_hiDx, nmr_hiDy, ...
        'method', 'centres', 'verbose', false), 'single-image');

% method = 'contract' still applies to nested densities only.
nmr_flat = buildMaet([0 4 7], [], 1.0, 2, false, false, 0, ...
                        'verbose', false);
results{end+1, 1} = 'nested measure rule: contract is rejected on a flat density';
results{end, 2} = throwsErrorWithId(@() simMaet(nmr_flat, nmr_flat, ...
    'method', 'contract', 'verbose', false), 'simMaet:contractUnavailable');
...
nmr_out = evalc('simMaet(nmr_hiDx, nmr_hiDy, ''verbose'', false);');
...
nmr_maSpecs = {nmr_SPEC, struct('r', 1, 'rel', false, 'exch', true)};
nmr_v2x = rand(1, 2) * 5;
nmr_v2y = nmr_v2x + 0.3;
nmr_maDx = buildMaet({nmr_px, nmr_v2x}, {[], []}, 'specs', nmr_maSpecs, ...
    'sigma', [nmr_lowSig, 1.0], 'isPer', [true, false], ...
    'period', [nmr_P, 0.0], 'verbose', false);
nmr_maDy = buildMaet({nmr_py, nmr_v2y}, {[], []}, 'specs', nmr_maSpecs, ...
    'sigma', [nmr_lowSig, 1.0], 'isPer', [true, false], ...
    'period', [nmr_P, 0.0], 'verbose', false);
[nmr_ma1, nmr_maXo, nmr_maYo] = simMaet(nmr_maDx, nmr_maDy, ...
                                              'verbose', false);
...
nmr_ma2 = simMaet(nmr_maXo, nmr_maYo, 'verbose', false);
...
nmr_maSiX = buildMaet({nmr_px, nmr_v2x}, {[], []}, 'specs', nmr_siSpecs, ...
    'sigma', [nmr_hiSig, 1.0], 'isPer', [true, false], ...
    'period', [nmr_P, 0.0], 'wrap', {'single-image', 'full-image'}, ...
    'verbose', false);
nmr_maSiY = buildMaet({nmr_py, nmr_v2y}, {[], []}, 'specs', nmr_siSpecs, ...
    'sigma', [nmr_hiSig, 1.0], 'isPer', [true, false], ...
    'period', [nmr_P, 0.0], 'wrap', {'single-image', 'full-image'}, ...
    'verbose', false);
[~, nmr_siXo] = simMaet(nmr_maSiX, nmr_maSiY, 'verbose', false);
nmr_maFiX = buildMaet({nmr_px, nmr_v2x}, {[], []}, 'specs', nmr_siSpecs, ...
    'sigma', [nmr_hiSig, 1.0], 'isPer', [true, false], ...
    'period', [nmr_P, 0.0], 'verbose', false);
nmr_maFiY = buildMaet({nmr_py, nmr_v2y}, {[], []}, 'specs', nmr_siSpecs, ...
    'sigma', [nmr_hiSig, 1.0], 'isPer', [true, false], ...
    'period', [nmr_P, 0.0], 'verbose', false);
[~, nmr_fiXo] = simMaet(nmr_maFiX, nmr_maFiY, 'verbose', false);
...
% An ordered flat companion ([exch] = false, r > 1) must go through the
...
nmr_ordSpecs = {nmr_SPEC, struct('r', 2, 'rel', false, 'exch', false)};
nmr_o2x = rand(3, 2) * 5;
nmr_o2y = nmr_o2x + 0.3;
nmr_ordKw = {'sigma', [nmr_lowSig, 1.0], 'isPer', [true, false], ...
             'period', [nmr_P, 0.0], 'verbose', false};
nmr_ordC = simMaet( ...
    buildMaet({nmr_px, nmr_o2x}, {[], []}, 'specs', nmr_ordSpecs, nmr_ordKw{:}), ...
    buildMaet({nmr_py, nmr_o2y}, {[], []}, 'specs', nmr_ordSpecs, nmr_ordKw{:}), ...
    'verbose', false);
nmr_ordB = simMaet( ...
    buildMaet({nmr_px, nmr_o2x}, {[], []}, 'specs', nmr_ordSpecs, nmr_ordKw{:}), ...
    buildMaet({nmr_py, nmr_o2y}, {[], []}, 'specs', nmr_ordSpecs, nmr_ordKw{:}), ...
...
for nmr_ii = 1:size(nmr_bulgerCases, 1)
    ...
    nmr_specsB = {nmr_spec, struct('r', 1, 'rel', false, 'exch', true)};
    ...
    nmr_bC = simMaet( ...
        buildMaet({nmr_px, nmr_v2x}, {[], []}, 'specs', nmr_specsB, nmr_kwB{:}), ...
        buildMaet({nmr_py, nmr_v2y}, {[], []}, 'specs', nmr_specsB, nmr_kwB{:}), ...
        'verbose', false);
    nmr_bB = simMaet( ...
        buildMaet({nmr_px, nmr_v2x}, {[], []}, 'specs', nmr_specsB, nmr_kwB{:}), ...
        buildMaet({nmr_py, nmr_v2y}, {[], []}, 'specs', nmr_specsB, nmr_kwB{:}), ...
...
function T = nmrTuples(col)
    % Every nested tuple of the r = [2, 2], exch = [true true] attribute on
