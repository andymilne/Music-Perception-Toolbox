...
%  Covers:
%    - method kwarg validation (bad string, normalize=true rejected).
...
% method='renyi2' with normalize=true (the default) must error with a
% clear message, since normalisation has no [0,1] interpretation in
% Rényi-2.
ok_normTrue = false;
try
    entropyExpTens([0 4 7], [], 30, 2, false, false, 0, ...
        'method', 'renyi2', 'verbose', false);   % default normalize=true
catch ME
    ok_normTrue = strcmp(ME.identifier, ...
        'entropyExpTens:renyi2NormalizeNotSupported');
end
results{end+1,1} = 'entropy.renyi2: normalize=true rejected with informative error';
results{end,2}   = ok_normTrue;
...
H_renyi = entropyExpTens(p1, w1, sigma, 1, false, false, 0, ...
    'method', 'renyi2', 'normalize', false, 'base', 2, 'verbose', false);
...
H_be = entropyExpTens(p3, w3, sigma, 3, false, false, 0, ...
    'method', 'renyi2', 'normalize', false, 'base', exp(1), 'verbose', false);
...
ok_listReject = false;
try
    entropyExpTens(dens_list, 'method', 'renyi2', 'normalize', false, ...
        'verbose', false);
catch ME
