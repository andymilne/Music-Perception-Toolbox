...
for r = [2 3]
    for N = [20 60]
        ...
        fnCosFresh = @() simMaet(dX, dY, ...
            'method', 'mobius', 'verbose', false);
        fnOsdFresh = @() simMaet(dX, dY, ...
        ...
        [~, dXw, dYw] = simMaet(dX, dY, 'method', 'mobius', ...
            'verbose', false);
        fnCosWarm = @() simMaet(dXw, dYw, 'method', 'mobius', ...
            'verbose', false);
...
for fi = 1:nCheck
    ...
    for methC = {'bulger', 'mobius'}
        ...
        [~, dXw, dYw] = simMaet(dX, dY, 'method', meth, ...
            'verbose', false);
        fn = @() simMaet(dXw, dYw, 'method', meth, ...
            'verbose', false);
...
%IBUILDABS  Deterministic absolute density (mirrors bench_cost_model.py).
    ...
    d = buildMaet({vals}, [], 30.0, r, false, false, 0, ...
        'verbose', false);
end
