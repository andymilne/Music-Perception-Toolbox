%% test_specs.m — canonical specs= form of buildMaet (3c-i)
%
...
d_old = buildMaet(P2, [], [50 30], [2 1], [true false], ...
...
for q = {[5; 100], [7; 140], [3; 120]}
    okFlat = okFlat && abs( ...
        evalMaet(d_old,  q{1}, 'verbose', false) ...
      - evalMaet(d_spec, q{1}, 'verbose', false)) < 1e-12;
end
...
nsp = struct('tags', [0 0 1 1], 'r', [2 2], 'exch', [true false], 'rel', 'outermost');
...
nspNamed = struct('tags', [0 0 1 1], 'r', [2 2], 'exch', [true false], ...
...
try
    buildMaet(P2, [], [50 30], [2 1], [true false], [false false], ...
...
for q = {[5; 100], [7; 140]}
    okFS = okFS && abs(evalMaet(dFS,  q{1}, 'verbose', false) ...
                     - evalMaet(dPos, q{1}, 'verbose', false)) < 1e-12;
end
...
results{end,2}   = numel(sSB) == 3 && sSB{2}.r == 2 ...
                   && sSB{3}.rel == false && sSB{1}.exch == true;


% --- Nested omitted exch defaults to all-True ---
pvT = {[0; 4; 7; 11]};
