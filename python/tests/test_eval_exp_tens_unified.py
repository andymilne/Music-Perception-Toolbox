"""Tests for the unified :func:`eval_maet`.
...
class TestDensityScalar:
    def test_returns_1d_array(self, density_set, query_grid):
        vals = eval_maet(density_set["major"], query_grid, verbose=False)
        assert isinstance(vals, np.ndarray)
        assert vals.shape == (8,)

    def test_positional_normalize(self, density_set, query_grid):
        vals_default = eval_maet(density_set["major"], query_grid, verbose=False)
        vals_pdf = eval_maet(density_set["major"], query_grid, "pdf", verbose=False)
    ...
    def test_kwarg_normalize(self, density_set, query_grid):
        vals_pos = eval_maet(density_set["major"], query_grid, "pdf", verbose=False)
        vals_kw = eval_maet(
...
class TestDensityList:
    def test_basic_shape(self, density_set, query_grid):
        ...
        vals = eval_maet(ds, query_grid, verbose=False)
        assert isinstance(vals, np.ndarray)
    ...
    def test_values_match_per_density_calls(self, density_set, query_grid):
        ...
        for i, d in enumerate(ds):
            ref = eval_maet(d, query_grid, verbose=False)
    ...
    def test_empty_list_returns_empty_array(self, density_set, query_grid):
        vals = eval_maet([], query_grid, verbose=False)
    ...
    def test_dedup_default_matches_no_dedup(self, density_set, query_grid):
        ...
        vals_dedup = eval_maet(ds, query_grid, verbose=False)
        vals_no_dedup = eval_maet(ds, query_grid, dedup=False, verbose=False)
    ...
    def test_normalize_kwarg(self, density_set, query_grid):
        ...
        vals = eval_maet(ds, query_grid, normalize="pdf", verbose=False)
...
class TestRawSAScalar:
    def test_matches_density_scalar(self, query_grid):
        ...
        d = build_maet(p, None, sigma, r, False, True, period, verbose=False)
        ref = eval_maet(d, query_grid, verbose=False)
    ...
    def test_with_normalize_positional(self, query_grid):
        p = [0.0, 400.0, 700.0]
        d = build_maet(p, None, 15.0, 2, False, True, 1200.0, verbose=False)
        ref = eval_maet(d, query_grid, "pdf", verbose=False)
        vals = eval_maet(
...
class TestRawMAScalar:
    def test_basic_call(self):
        ...
        ref = eval_maet(d, x, verbose=False)
