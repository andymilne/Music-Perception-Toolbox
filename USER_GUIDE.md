...
**Generalised equation.** A MAET represents events via $A$ *attributes* (each a kind of property — pitches in a chord, pitch of a voice, time, register, timbre, …), each carrying its own perceptual parameters (*attributes* and *slots* are defined in detail below). At each event, each attribute $a$ carries $K_a$ value-*slots* (for example, a pitch attribute representing a three-pitch chord has $K_a = 3$); the tensor is built at order $r_a$ per attribute. The density at a query point $\mathbf{x} = (\mathbf{x}_1, \ldots, \mathbf{x}_A)$ is
...
The distinction between "several slots within one attribute" and "several attributes" is the modelling choice between exchangeable and distinguishable components. Consider a three-voice chord:
...
- **Implicit**: the number of slots per event (K) is read from the shape of the input.
...
`translateAttributes` shifts selected attributes' values by a chosen offset, leaving non-selected attributes unchanged. The transformed `(pAttr, w, specs)` carrier feeds directly into `buildExpTens`; weights and `specs` pass through unchanged, since uniform translation does not affect any event's probability of perception. The shift is per-slot — one value per slot of an attribute, held constant across the event axis — so the operation is well-defined for any $K_a \ge 1$; unlike windowing, where a per-event scalar weight would have to summarise multiple slots' distances from a localising centre, translation has no analogous obstruction at $K_a > 1$. Relative geometry is read per-attribute from `specs`. On an absolute attribute the operation is `value + mu`, regardless of periodicity: the wrapped periodic Gaussian kernel of `buildExpTens` is invariant under an additive shift by a multiple of $P$, so periodic attributes need no canonical wrap of the translated values — the kernel handles periodicity downstream. On an attribute whose outermost level is relative, a *uniform* shift is a structural no-op — it cancels in every within-tuple difference, so the relative MAET is unchanged — and the function emits a `translateAttributes:noOpRelative` (MATLAB) or `TranslateAttributesNoOpWarning` (Python) and leaves that attribute unchanged; a *non-uniform* (per-slot) offset is not a no-op even on a relative attribute and is applied.

`offsets` is a **length-$A$ list** (Python) / $1 \times A$ cell (MATLAB), one entry per attribute. Each entry is `None` / empty `[]` (skip this attribute), a scalar or length-1 vector (broadcast to every slot — a global transposition), a length-$K_\text{total}$ vector (per-slot), a $1 \times M$ row (a per-sweep global shift, one scalar per sweep index), or a $K_\text{total} \times M$ matrix (per-slot by sweep index). `NaN` entries skip the corresponding slot; $\pm\infty$ is rejected. All entries carrying $M > 1$ must agree on $M$; scalar, length-1, and single-column entries broadcast across the sweep.
...
**`deleteInput` (mandatory keyword).** A boolean flag — keyword-only, no default — selects whether the input attribute is retained in the returned `(pAttr, wOut, specs)` carrier or removed. When `deleteInput = true` and `inputAttr != targetAttr`, the input attribute is dropped from all three output components and the higher attribute indices are decremented to keep numbering contiguous. When `deleteInput = false`, the input attribute is preserved unchanged in the output. `deleteInput = true` paired with `inputAttr == targetAttr` raises an error: deleting the input would discard the factor just written to it. The auto-delete pattern is the standard idiom for windowed-entropy and local-mass analyses where the input attribute (typically time) provides the scaffolding for the window and is no longer needed downstream once its values have been transferred to the target's weight slot.

**Multi-axis windowing.** A single `weightEvents` call windows on one input attribute. To window on multiple axes simultaneously — say, time *and* register, both targeting pitch — call `weightEvents` twice in sequence with the same `targetAttr`. Each call's factor multiplies into the target's existing weight slot, so the composition naturally gives the product window. Set `deleteInput = false` on intermediate calls (so the next call's input is still present), and `deleteInput = true` on the last (to drop both inputs at once is then a matter of a final lookup or a separate composition step).
...
- **The direct method** (`method='direct'` where accepted) — the path that evaluates the closed-form expression as written, with no clever combinatorial decomposition. Three consumers realise this differently:

    * On the **SA inner product** (`cosSimExpTens`), `direct` enumerates every ordered $r$-tuple on each side and sums kernel contributions, with no Bulger combinations-vs-permutations factoring and no Möbius alternating sum. Cost $O\!\left(\frac{K_x!}{(K_x-r)!} \cdot \frac{K_y!}{(K_y-r)!}\right)$ — expensive at moderate $K$ relative to Bulger or Möbius, but exact at any $K \ge r$ and immune to cancellation (all summands are non-negative). Users can force `method='direct'` on the SA path for benchmarking or sanity-checking; under `'auto'` in the unsafe regime ($K_\text{eff} - r < 2$, where the Möbius precision guard refuses to fire) the SA dispatcher routes to **Bulger's method**, not direct enumeration — both are precise but Bulger is faster at the small $K_\text{eff}$ that defines the regime.
    ...
    * On the **windowed inner product** (`windowedSimilarity`), `direct` is the closed-form analytical expression for the windowed similarity (Section 3.1, "Post-tensor windowing"), evaluated as written in Gaussian-integral form. It is the only path available — there is no Bulger or Möbius variant of the windowed inner product to compare against — and the dispatch announce reads `windowedSimilarity: chose 'direct' path.` accordingly. `windowedSimilarity` does not accept a `method` keyword; the `direct` label is the descriptive vocabulary for what the function does, not a chosen path among alternatives.

    On `evalExpTens`, `'direct'` is accepted as a synonym for `'centres'` so the same keyword works consistently across functions; there is no second meaning for eval (eval has no Möbius alternating sum to avoid).
...
- **Reordering-invariant outputs** (every function above) collapse slot permutations and, for periodic attributes, mod-reduction.
...
| `weightEvents` | `weight_events` | *Preprocessing:* multiply a per-event window factor read from one attribute into another attribute's weight slot |
...
Per-attribute preprocessing for MAET input. Shifts selected attributes' values by a chosen offset on the `(pAttr, w, specs)` carrier, returning a transformed carrier that feeds directly into `buildExpTens`. Weights and `specs` pass through unchanged — only the values move. The transform is per-slot: an offset is one value per slot (the rows of the $K_\text{total} \times N$ value matrix), held constant across the sequence (column) axis, which is exactly what makes a uniform shift cancel inside relative attributes.

`offsets` is a **length-$A$ list** (Python) / $1 \times A$ cell (MATLAB), one entry per attribute, each of: `None` / empty `[]` (do not translate this attribute); a scalar or length-1 vector (broadcast to all slots — a global transposition); a length-$K_\text{total}$ vector (per-slot); a $1 \times M$ row (a per-sweep global shift, one scalar per sweep index, broadcast across slots); or a $K_\text{total} \times M$ matrix (per-slot by sweep index — slots down, sweep index across). `NaN` entries skip the corresponding slot (left untranslated); $\pm\infty$ is rejected. All entries carrying $M > 1$ must agree on $M$; scalar, length-1, and single-column entries broadcast across the sweep.

Relative geometry is read per-attribute from `specs` (there is no separate `isRel` argument): a *uniform* finite offset on an attribute whose outermost level is relative is a structural no-op (it cancels in every within-tuple difference), so that attribute is left unchanged and a single `translateAttributes:noOpRelative` (MATLAB) / `TranslateAttributesNoOpWarning` (Python) is emitted per call; a *non-uniform* (per-slot) offset is **not** a no-op even on a relative attribute and is applied. `isPer` / `period` are not consulted here — translation emits unwrapped values and the periodic kernel in `buildExpTens` wraps downstream.
...
Per-event preprocessing for MAET input. Reads the $K = 1$ value at every event from `inputAttr`, evaluates a window factor centred at `centre` with shape parameter $\gamma$ = `shape`, and multiplies the resulting $(1, N)$ per-event factor into the weight slot of `targetAttr` (on top of any weight already there), returning a transformed `(pAttr, wOut, specs)` carrier that feeds directly into `buildExpTens`.
...
Multi-axis windowing is expressed as a sequence of `weightEvents` calls with the same `targetAttr`; each call's factor multiplies into the target's existing weight slot. The canonical composition `weightEvents` (with `deleteInput = true`, `inputAttr` typically time, `targetAttr` typically pitch) $\to$ `buildExpTens` $\to$ `entropyExpTens` is the windowed-entropy construction of [§3.7.5](#375-compositions-and-canonical-uses). `weightEvents` chains cleanly with `translateAttributes` under a centre-shift identity ($\mathcal{T}_\mu \circ \mathcal{W}_c = \mathcal{W}_{c+\mu} \circ \mathcal{T}_\mu$ on overlapping targets) and passes through `differenceEvents` / `bindEvents`.
...
| `demo_windowingReference` | Reference-point option for `windowedSimilarity` / `windowed_similarity`: harmonic-baseline, slot-count, slot-value, and slot-weight sweeps with the harmonic-calibrated fixed-reference strategy | — |
