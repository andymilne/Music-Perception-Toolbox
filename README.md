...
Version 3.0.0 is a major release relative to the last public line (2.0.x); versions 2.1 and 2.2 were internal milestones that were never released, and their changes ship together here. There are two strands.
...
- **Sequential-analysis and encoding utilities.** `continuity` summarises the recent direction trend leading up to a query; `seqWeights` constructs position-weight vectors with named time-based decay profiles; `simplexVertices` returns equidistant vertex coordinates for simplex-coded categorical attributes (voice identity, instrument, etc.) suitable for MAET inputs.
...
- **Mathematical: the Möbius method.** A new analytical decomposition for the three core quantities — inner product, point evaluation, and total mass — that complements the existing Bulger's method for the inner product. Bulger's method (v1, exposed as `method='bulger'`) is inner-product-only, has essentially no per-call overhead, and tends to win at small tensor order $r$ and small slot count $K$. The Möbius method (new in v3, exposed as `method='mobius'`) applies uniformly to all three quantities and tends to win at large $r$ or large $K$, where the slot-tuple loop in Bulger's method blows up. A per-call cost-model dispatcher picks the cheaper of the two for each call from structure and the cost models alone; there is no timing probe, so identical workloads take identical routes on every call and every machine. The inclusion-exclusion technique at the core of the Möbius method was first published in Milne et al. 2011 and has been used in MPT since v1 for numerical computation of discrete tensors; v3 applies it as a closed-form analytical decomposition, combined with orbit collapse for the inner product.

- **Computational: kernel precision, kernel truncation, and batching refinements.** Two kernel-evaluation controls — `truncation_sigmas` (skip Gaussian contributions beyond $k$ standard deviations from a centre; the factory default is `6`, and `Inf` recovers the exact v2.0 numerics) and `kernel_precision` (`'single'` instead of `'double'` for ~2× speedup at ~7 significant figures of precision) — affect the centres-path consumers (eval, cos-sim, entropy). Batching refinements replace sequential per-item loops with grouped contractions: ragged (NaN-padded) events flow through the batched Möbius inner product with zero-weight padding, `tensorHarmonicity`'s batched mode is a dedup-and-batch pass, the Möbius relative-mode evaluator vectorises its u-grid and carries factored and spectral strategies, and self inner products are memoised so a sweep of many queries against one context pays the context's self term once.
