...
% in the captured output of simMaet calls (via evalc). The
...
ipprobe_dx = buildMaet(ipprobe_px, ones(20, 1), 1, 3, false, false, 1200, ...
    'verbose', false);
ipprobe_dy = buildMaet(ipprobe_py, ones(20, 1), 1, 3, false, false, 1200, ...
    'verbose', false);
ipprobe_simOrbit = simMaet(ipprobe_dx, ipprobe_dy, ...
    'method', 'mobius', 'verbose', false);
ipprobe_simPair  = simMaet(ipprobe_dx, ipprobe_dy, ...
...
ipprobe_simAuto = simMaet(ipprobe_dx, ipprobe_dy, 'verbose', false);
...
ipprobe_dx1 = buildMaet(ipprobe_px, ones(20, 1), 1, 1, false, false, 1200, ...
    'verbose', false);
ipprobe_dy1 = buildMaet(ipprobe_py, ones(20, 1), 1, 1, false, false, 1200, ...
    'verbose', false);
ipprobe_simR1 = simMaet(ipprobe_dx1, ipprobe_dy1, ...
...
ipprobe_dyS = buildMaet(ipprobe_pyS, ones(4, 1), 1, 3, false, false, 1200, ...
    'verbose', false);
ipprobe_simSafety = simMaet(ipprobe_dx, ipprobe_dyS, ...
    'method', 'auto', 'verbose', false);
...
ipprobe_dxP = buildMaet(ipprobe_pxP, ones(5, 1), 1, 2, false, false, 1200, ...
    'verbose', false);
ipprobe_dyP = buildMaet(ipprobe_pyP, ones(5, 1), 1, 2, false, false, 1200, ...
    'verbose', false);
ipprobe_evalStr = evalc( ...
    'simMaet(ipprobe_dxP, ipprobe_dyP, ''verbose'', true);');
results{end+1, 1} = 'ip_probe: verbose message printed when probe fires';
results{end, 2}   = contains(ipprobe_evalStr, 'simMaet') ...
...
ipprobe_evalStrR1 = evalc( ...
    'simMaet(ipprobe_dx1, ipprobe_dy1, ''verbose'', true);');
results{end+1, 1} = 'ip_probe: verbose message present at r=1 (hard rule)';
results{end, 2}   = contains(ipprobe_evalStrR1, 'simMaet') ...
...
ipprobe_evalStrUser = evalc( ...
    ['simMaet(ipprobe_dx, ipprobe_dy, ''method'', ''bulger'', ' ...
     '''verbose'', true);']);
results{end+1, 1} = 'ip_probe: verbose message present when method set';
results{end, 2}   = contains(ipprobe_evalStrUser, 'simMaet') ...
