...
**Magnitude-aware normalisation.** `windowedSimilarity` normalises the windowed inner product by the *unwindowed* L2 self inner product of the query alone (not the geometric mean of both operands' unwindowed norms). The result is

$$ s = \frac{\langle h \cdot f_X, f_Y \rangle}{\langle f_Y, f_Y \rangle} $$

where $f_Y$ is the unwindowed query and $f_X$ (windowed) is the context. Self-similarity at full window coverage equals 1; the profile drops below 1 when the window does not cover all of the query's mass, and it can rise above 1 when the windowed context has more matching mass in the window region than the query does in total. This magnitude-aware behaviour is what is wanted for sliding-motif and probe-tone analyses: a region of the context with a small amount of near-perfectly-matching content scores lower than a region with a large amount of equally-matching content, because the windowed inner product scales with the amount of matching mass and the (query-only) denominator does not. An alternative normalisation by the *windowed* context norm — a strict shape-only cosine — would cancel out this magnitude information, which is undesirable in the cross-correlation use case. The output is therefore not bounded in $[-1, 1]$ across sweep positions and does not correspond to an inner product on a single Hilbert space; the term *cosine similarity* is reserved for the strict shape-only form (not currently implemented in the toolbox), and *windowed similarity* is used for what `windowedSimilarity` returns. Multiplying the query's weights by a constant rescales the profile by that constant (because the numerator scales linearly with the query weights while the denominator scales quadratically and only one factor of the query appears in the numerator); profiles from queries whose weight scales differ are therefore not directly comparable on absolute magnitude. Multiplying the context's weights by a constant rescales the profile linearly.
