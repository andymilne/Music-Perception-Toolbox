%% test_entropy.m — entropyMaet — single multiset Shannon and Rényi-2
%
...
results{end+1,1} = 'entropyMaet normalized: uniform ≈ 1';
results{end,2}   = H > 0.95;

% --- v3 unified dispatch: entropyMaet list and batched-raw modes ----

% List mode: cell of density structs returns cell of entropy values
de1 = buildMaet([0, 4, 7], [], 50, 1, false, true, 1200, 'verbose', false);
de2 = buildMaet([0, 3, 7], [], 50, 1, false, true, 1200, 'verbose', false);
HCell = entropyMaet({de1, de2}, 'nPointsPerDim', 1200, 'verbose', false);
results{end+1,1} = 'entropyMaet list: returns cell of correct length';
results{end,2}   = iscell(HCell) && numel(HCell) == 2;
H1 = entropyMaet(de1, 'nPointsPerDim', 1200, 'verbose', false);
H2 = entropyMaet(de2, 'nPointsPerDim', 1200, 'verbose', false);
results{end+1,1} = 'entropyMaet list: matches scalar dispatch element-wise';
results{end,2}   = abs(HCell{1} - H1) < 1e-14 ...
                   && abs(HCell{2} - H2) < 1e-14;

% List mode: Option II (length-1 stays length-1)
HCell1 = entropyMaet({de1}, 'nPointsPerDim', 1200, 'verbose', false);
results{end+1,1} = 'entropyMaet list: length-1 returns length-1 cell';
results{end,2}   = iscell(HCell1) && numel(HCell1) == 1;

% List mode: name-value pairs forwarded
HCellNorm = entropyMaet({de1, de2}, 'method', 'shannon', 'base', exp(1), 'nPointsPerDim', 1200, 'verbose', false);
H1_nat = entropyMaet(de1, 'method', 'shannon', 'base', exp(1), 'nPointsPerDim', 1200, 'verbose', false);
results{end+1,1} = 'entropyMaet list: name-value pairs forwarded';
results{end,2}   = abs(HCellNorm{1} - H1_nat) < 1e-12;

% List mode: non-struct entry errors
results{end+1,1} = 'entropyMaet list: non-struct entry errors';
results{end,2}   = throwsErrorWithId( ...
    @() entropyMaet({de1, [1, 2, 3]}, 'verbose', false), ...
    'entropyMaet:listNonStruct');

% Batched-raw mode: 2-D pitch matrix returns vector of entropies
P_h = [0, 4, 7; 0, 3, 7];
H_batched = entropyMaet(P_h, [], 50, 1, false, true, 1200, 'nPointsPerDim', 1200, 'verbose', false);
results{end+1,1} = 'entropyMaet batched: returns vector of correct length';
results{end,2}   = isnumeric(H_batched) && numel(H_batched) == 2;

% Batched-raw matches scalar dispatch row-by-row
H_row1 = entropyMaet(P_h(1, :), [], 50, 1, false, true, 1200, 'nPointsPerDim', 1200, 'verbose', false);
H_row2 = entropyMaet(P_h(2, :), [], 50, 1, false, true, 1200, 'nPointsPerDim', 1200, 'verbose', false);
results{end+1,1} = 'entropyMaet batched: matches scalar dispatch row-by-row';
...
H_batched_nan = entropyMaet(P_h_nan, [], 50, 1, false, true, 1200, 'nPointsPerDim', 1200, 'verbose', false);
results{end+1,1} = 'entropyMaet batched: NaN-padded rows match unpadded';
...
H_short = entropyMaet(P_h_short, [], 50, 2, false, true, 1200, 'nPointsPerDim', 1200, 'verbose', false);
results{end+1,1} = 'entropyMaet batched: row with too few pitches returns NaN';
...
results{end+1,1} = 'entropyMaet single multiset dim=2 periodic: returns finite value';
results{end,2}   = isfinite(H_dim2_per) && H_dim2_per > 0 && H_dim2_per <= 1;

% Non-periodic with explicit bounds
H_dim2_nonper = entropyMaet([0, 400, 700], [], 12, 2, false, false, 1200, ...
    'method', 'normalized', ...
    'xMin', -100, 'xMax', 800, 'nPointsPerDim', 60, 'verbose', false);
results{end+1,1} = 'entropyMaet single multiset dim=2 non-periodic: returns finite value';
results{end,2}   = isfinite(H_dim2_nonper) && H_dim2_nonper > 0 && H_dim2_nonper <= 1;

% gridLimit guard
results{end+1,1} = 'entropyMaet single multiset dim=2: gridLimit guard fires';
results{end,2}   = throwsErrorWithId( ...
    @() entropyMaet([0, 400, 700], [], 12, 2, false, true, 1200, ...
        'nPointsPerDim', 1200, 'gridLimit', 1e3, 'verbose', false), ...
    'entropyMaet:gridLimitExceeded');

% Empty weights treated as uniform (same as buildMaet convention)
H_uni = entropyMaet(0:11, [], 100, 1, false, true, 12, 'nPointsPerDim', 1200, 'verbose', false);
H_ones = entropyMaet(0:11, ones(1, 12), 100, 1, false, true, 12, 'nPointsPerDim', 1200, 'verbose', false);
results{end+1,1} = 'entropyMaet: w=[] equivalent to ones(1,N)';
...
W2 = [1; 1; 1; 1];
sigEff = sqrt(2);
densRefuse = buildMaet({P2; P2}, {W2; W2}, [sigEff sigEff], [1 1], ...
    [false false], [true true], [12 12], 'verbose', false);
prevKcb = mptDefaults('kernelChunkBytes');
mptDefaults('kernelChunkBytes', 8 * 1024 * 1024);
refused = false;
try
    entropyMaet(densRefuse, 'method', 'differential', ...
        'truncationSigmas', Inf, 'verbose', false);
catch ME
    refused = strcmp(ME.identifier, 'entropyMaet:differentialGridLimit');
end
mptDefaults('kernelChunkBytes', prevKcb);
results{end+1,1} = 'entropyMaet differential: tightest 2-D accuracy refuses with guidance';
