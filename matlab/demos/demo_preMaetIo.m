...
dens = buildMaet(pmBack, 'verbose', false);
...
for k = 1:numel(sigmasPitch)
    ...
    sim = simMaet(buildMaet(pm, kw{:}), ...
                        buildMaet(pmUp, kw{:}), 'verbose', false);
...
fprintf(['  A nested attribute''s r, rel and exch are per-level, so an ' ...
...
showPreMaet(pmLog);
try
    buildMaet(pmLog, 'verbose', false);
catch err
    fprintf('\n  build refuses it: %s\n', err.message);
end
fprintf('\n  Supplying a width resolves it, the analyst having chosen one:\n');
dLog = buildMaet(pmLog, 'sigma', 0.05, ...
    'verbose', false);
fprintf('    dim %d\n\n', dLog.dim);

%% ===================================================================
%  7. A kernel covariance, as a flag and three scalars
%  ===================================================================

fprintf('=== 7. A kernel covariance, as a flag and three scalars ===\n\n');

% A covariance is a matrix, but the covariance an analysis wants is
% generated by three widths and a choice of case (does the tuple hold
% positions or their first differences?), so those are what the file
% carries.
covCsv = ['name,sigma,r,rel,per,P,exch,n = 1,n = 2' sprintf('\n') ...
    'trigram,"cov(differenced=1, sd_value=0.2, sd_interval=0.3, sd_shift=0.5)",' ...
...
showPreMaet(pmC);
dC = buildMaet(pmC, 'verbose', false);
