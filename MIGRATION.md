...
- **SA cosine-similarity dispatcher gains a timing probe.** When the analytical pre-screen does not decisively favour one path (i.e., neither cost-model estimate dominates the other by ≥10× at the call's parameters), the dispatcher times each candidate path on a small subset of the workload and routes the full call accordingly. This is future-proof against incremental optimisations on either side. The user-visible default behaviour is unchanged in the regimes where the analytical pre-screen was already decisive (which is the majority of typical workloads); only edge-case calls that previously got the wrong path now route correctly.
