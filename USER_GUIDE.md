...
The MAET framework provides three preprocessing helpers. Two operate cross-event, producing a derived sequence in which each output entry depends on multiple input events: `differenceEvents` (replacing the sequence with inter-event differences) and `bindEvents` (gathering n consecutive events into a single super-event). The third, `translateAttributes`, is per-attribute and event-local: it shifts selected attributes' values by a chosen offset, leaving the event count, ordering, and weight structure unchanged. The cross-event operations act per group; `translateAttributes` acts per-attribute, with broadcast-within-group as a common case. All three propagate per-event weights through the toolbox's standard probability-of-perception reading where applicable. They also differ in their handling of multi-value attributes. `differenceEvents` is restricted to single-value ($K_a = 1$) attributes because column-wise subtraction across adjacent events requires a slot correspondence that the within-event slot exchangeability does not provide. `bindEvents` and `translateAttributes` accept any $K_a \ge 1$: `bindEvents` because each output attribute simply carries the full slot vector of its underlying event with no cross-event slot alignment imposed; `translateAttributes` because every slot in the per-attribute multiset is shifted by the same offset and so no slot-correspondence question arises. Each operation is useful on its own. Differencing and binding can be chained — differencing first, since it requires $K_a = 1$ input and produces $K_a = 1$ output, which `bindEvents` then accepts — and translation can be combined with either, before or after, since it leaves shapes unchanged. The MAET built on a cross-event-derived sequence aggregates across its entries in the usual way, so the resulting density is a distribution over derived values rather than a function of position in the derived sequence.
...
`translateAttributes` is the **pre-tensor route to a sliding comparison** along an attribute axis. For each candidate offset $\mu$ on a sweep grid, translate the events, build the MAET, and compute `cosSimExpTens` against an un-shifted reference; the peak of the resulting profile is the offset that best aligns the two densities. The post-tensor route to the same kind of sliding comparison is provided by `windowedSimilarity` (next subsection): instead of translating the query and comparing whole-density-to-whole-density, the context's density is multiplied pointwise by a localising window that slides along the axis, and the windowed inner product against the un-windowed query is computed at each window position. The two routes differ in two ways. First, the pre-tensor route uses the query's own intrinsic support as the matching scale; the post-tensor route uses an externally chosen window, decoupling the comparison region's scale from the query. Second, the post-tensor route is magnitude-aware by default (so similarity values can exceed $1$ and the query/context roles are asymmetric, see "Magnitude-aware normalisation" below), whereas the pre-tensor route preserves the symmetric cosine. In practice, pre-tensor translation is the natural choice when two whole densities are being aligned under an unknown transposition or time-shift (e.g.\ cadence-progression comparison up to musical transposition), and post-tensor windowing is the natural choice when a fixed query is being matched against a longer context with controllable inspection scale (e.g.\ motif-finding within a chorale). On periodic groups both routes are well-defined and operationally interchangeable for whole-density alignment; on relative groups only the post-tensor route is available, since pre-tensor translation is a no-op there — but the corresponding post-tensor sweep then operates on interval-size space, an unusual analytic move that is rarely the question one is asking.
