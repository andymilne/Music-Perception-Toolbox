...
The MAET framework provides four preprocessing helpers that take the `(pAttr, w, groups)` triple feeding `buildExpTens` and return a transformed triple of the same shape: `differenceEvents` and `bindEvents` (cross-event), and `translateAttributes` and `weightEvents` (per-event). See §3.7 for details, including the canonical windowed-entropy and pre-tensor sliding-comparison constructions.
...
The MAET framework provides four preprocessing helpers that act on the `(pAttr, w, groups)` triple feeding `buildExpTens`. They form a closed family — each takes one triple, returns another in the same shape, and chains directly into further preprocessing or into MAET construction — organised by their scope of action:

- **Cross-event** (each output entry depends on multiple input events; the event sequence is reshaped): `differenceEvents` replaces the sequence with inter-event differences; `bindEvents` gathers $n$ consecutive events into a single super-event.
...
The cross-event operations act per group; the per-event operations act per attribute, with broadcast-within-group as a common case. Per-event weights propagate consistently through all four under the toolbox's standard probability-of-perception reading. Multi-value attributes ($K_a > 1$) are accepted by `bindEvents`, `translateAttributes`, and `weightEvents`'s target attribute; `differenceEvents` requires $K_a = 1$ for the reasons given in §3.7.1, and `weightEvents`'s input attribute is similarly restricted to $K = 1$ (a multi-value driver would not yield a well-defined per-event scalar factor).
...
pAttr  = {pS, pA, pT, pB};
groups = [1 1 1 1];
[pDiff, ~] = differenceEvents(pAttr, [], groups, 1);
...
`bindEvents` gathers $n$ consecutive events into a single super-event, sliding a window of width $n$ across the input and emitting each window with $n$ separate attributes carrying the values at lags $0$ through $n-1$. The output is a length-$n$ list of attribute matrices, suitable as the `pAttr` argument of `buildExpTens` with all $n$ attributes assigned to a single group sharing $\sigma$, `isRel`, `isPer`, and `period`. Lag slots are kept as separate attributes rather than packed into one $K_a = n$ attribute because lag identity is non-exchangeable: the within-attribute multiset symmetry would otherwise collapse ordered tuples to unordered ones. Per-event weights propagate per-attribute: each output attribute inherits the slot weights of the underlying event at its lag, and `buildExpTens` multiplies across attributes during tuple enumeration, so the effective end-to-end weight of a bound super-event equals the product of its $n$ constituent events' weights — matching the differencing rule under the toolbox's standard probability-of-perception reading. The default output has $N - n + 1$ super-events; a `circular` option produces $N$ by wrapping around the end of the input, the natural choice for cyclic event sequences. Unlike `differenceEvents`, `bindEvents` accepts any $K_a \ge 1$: when the input attribute has $K_a > 1$, each output attribute carries the full $K_a$-slot vector of the underlying event at its lag, and within-attribute slot exchangeability is preserved per output attribute. There is no cross-event slot alignment to violate, because the cross-event structure is between output attributes rather than within them.
...
`translateAttributes` shifts selected attributes' values by a chosen offset, leaving non-selected attributes unchanged. The output `pAttrTranslated` feeds directly into `buildExpTens` without further massaging; weights pass through unchanged, since uniform translation does not affect any event's probability of perception. The shift is applied uniformly to every slot in each event's per-attribute multiset, so the operation is well-defined for any $K_a \ge 1$ — unlike windowing, where a per-event scalar weight would have to summarise multiple slots' distances from a localising centre, translation has no analogous obstruction at $K_a > 1$. For absolute groups the operation is `value + mu`, regardless of periodicity: the wrapped periodic Gaussian kernel of `buildExpTens` is invariant under any additive shift by a multiple of $P$, so periodic groups need no canonical wrap of the translated values — the kernel handles periodicity downstream. For relative groups translation is a structural no-op — a uniform shift cancels in every within-tuple difference, so the relative MAET is unchanged — and the function emits a `translateAttributes:noOpRelative` (MATLAB) or `TranslateAttributesNoOpWarning` (Python) and leaves the group unchanged.

`translateAttributes` accepts offsets in two forms: a single numeric block (for uniform broadcast or fully per-attribute layouts), or a container keyed by group (for mixed-per-group layouts, e.g. broadcast within one group and per-attribute within another in the same call).

The **numeric form** is a single numeric block with rows indexing attributes and columns indexing sweep positions. Row count must be exactly 1 (broadcast across all attributes) or $A$ (per-attribute, one row per attribute). A scalar / $1 \times 1$ is broadcast no-sweep; a $1 \times M$ row broadcasts across attributes with $M$ sweep positions; an $A \times 1$ column is per-attribute single translation; an $A \times M$ matrix is per-attribute sweep. Within-group broadcast is expressed by setting that group's rows equal. NaN entries skip the corresponding `(attribute, column)` cell.

The **container form** is a `{group_index: value}` dict in Python, or a $1$-by-$G$ cell array in MATLAB. Each per-group value follows the same orientation convention with $n_g$ (the number of attributes in group $g$) playing the role of $A$: scalar broadcasts within group; $1 \times M$ row broadcasts within group with $M$-sweep; $n_g \times 1$ column is per-attribute within group; $n_g \times M$ matrix is per-attribute within group with $M$-sweep. Groups omitted (Python: missing key; MATLAB: empty `[]` cell) are not translated. All entries across the call carrying $M > 1$ must agree on $M$; scalar and 1-column entries broadcast across the sweep.

When any input implies a sweep ($M > 1$) or any 2-D shape is used, the output is a length-$M$ list (Python) / $1$-by-$M$ cell (MATLAB) of per-attribute blocks; otherwise a single per-attribute block. The matrix-mode output is the natural feed into the raw-MA scalar-vs-list mode of `cosSimExpTens` for a one-call sliding-comparison sweep (§3.7.5).

#### 3.7.4 Event weighting

`weightEvents` computes a per-event window factor from the values of one attribute (the *input attribute*) and multiplies it into the weight slot of another (the *target attribute*). Values and group structure pass through unchanged, except for the optional deletion of the input attribute discussed below; only the target attribute's weight slot is updated.
...
For periodic input attributes, only the centred difference $\delta = v - c$ used inside $h$ is wrapped to $[-P/2, P/2]$; the stored values stay raw. The kernel in `buildExpTens` handles the value-axis periodicity downstream via the group `[per]` flag.

**`deleteInput` (mandatory keyword).** A boolean flag — keyword-only, no default — selects whether the input attribute is retained in the returned `(pAttr, wOut, groups)` triple or removed. When `deleteInput = true` and `inputAttr != targetAttr`, the input attribute is dropped from all three output components; if it was the sole member of its group, that group disappears and higher group indices are decremented to keep numbering contiguous. When `deleteInput = false`, the input attribute is preserved unchanged in the output. `deleteInput = true` paired with `inputAttr == targetAttr` raises an error: deleting the input would discard the factor just written to it. The auto-delete pattern is the standard idiom for windowed-entropy and local-mass analyses where the input attribute (typically time) provides the scaffolding for the window and is no longer needed downstream once its values have been transferred to the target's weight slot.
...
| `translateAttributes` | `translate_attributes` | *Preprocessing:* shift selected attributes by a chosen offset; per-attribute or per-group, with optional matrix-form sweep over $M$ offsets in one call |
...
*Raw multi-attribute scalar-vs-list mode (v2.2+).* The raw multi-attribute calling form `cosSimExpTens(pAttr1, w1, pAttr2, w2, sigma, r, groups, isRel, isPer, periods)` extends to a scalar-vs-list shape: exactly one of `pAttr1`, `pAttr2` is a cell of cells (MATLAB) / list of lists (Python) — a 1-by-`M` list whose entries are themselves single `pAttr` blocks. The scalar operand's density is built once internally; each list entry's density is built once and compared against it. Weights for the list side are shared across every entry — a single `w` value, not a per-entry list. Returns a 1-by-`M` cell (MATLAB) / 1-D `ndarray` (Python). This mode is the natural consumer of the matrix-form output of `translateAttributes`: a pre-tensor sweep over $M$ offsets goes from `translateAttributes` to `cosSimExpTens` in two calls, with no explicit per-offset `buildExpTens` loop required. List-vs-list raw-MA calls are rejected with a directive to use the explicit density-struct list mode instead.
...
**differenceEvents(pAttr, w, groups, diffOrders, 'circular', false)**

Cross-event preprocessing for MAET input. Takes the `(pAttr, w)` pair that would otherwise be passed to `buildExpTens` and replaces the event sequences of selected groups with their $k$-th finite differences along the event axis, returning a `(pAttrDiff, wDiff)` pair in the same format. For each group, `diffOrders[g] = k` produces the $k$-th difference; order 0 leaves a group unchanged. Raw signed differences are emitted regardless of periodicity; periodic groups' mod-period wrap is handled by the kernel at MAET-construction time. Weights follow the toolbox's standard broadcast convention (§4) and propagate as rolling products of width $k + 1$, so the weight of a differenced event is the product of the weights of the $k + 1$ constituent events it depends on (interpretable as the probability that all constituents are jointly perceived). When `'circular'` is `false` (default), the output event count is $N' = N - \max_g k_g$; when different groups use different orders, lower-order groups have leading events dropped to keep columns aligned. When `'circular'` is `true`, the difference operator wraps at the sequence boundary ($\Delta p_a(n) = p_a(n) - p_a(\mathrm{prev}(n))$ with $\mathrm{prev}(1) = N$) and every group retains $N$ events at every order; no leading-event drop is needed, and weights propagate via a cyclic rolling product. The circular variant parallels the same flag on `bindEvents` and is the natural choice for cyclic event sequences (looped rhythms, ostinati) in which the boundary difference is a genuine inter-event interval. The returned `(pAttrDiff, wDiff)` feeds directly into `buildExpTens` without further processing. Inputs are restricted to $K_a = 1$ per attribute; see §3.1 (Preprocessing) for the rationale and the voices-as-attributes pipeline for polyphonic analyses. Event differencing is distinct from `isRel = true`: the former replaces absolute per-event values with differences between adjacent events (a preprocessing step); the latter is a property of tensor construction that makes the within-r-ad density translation-invariant (no preprocessing involved). Both can be used together or independently. Composes with `bindEvents` (below) to produce n-grams of consecutive inter-event differences.

**bindEvents(p, w, n, ...)**

Cross-event preprocessing for MAET input. Slides a window of width $n$ across an event sequence and emits each window as an $n$-attribute super-event whose $j$-th attribute holds the value(s) at lag $j-1$. The output is a length-$n$ list of $K_a \times N'$ matrices (where $N' = N - n + 1$ by default, or $N$ when `circular` is true), suitable as the `pAttr` argument of `buildExpTens` with all $n$ attributes assigned to a single group sharing $\sigma$, $r = 1$, `isRel`, `isPer`, and `period`. Per-event weights propagate per-attribute: each output attribute inherits the slot weights of the underlying event at its lag, and `buildExpTens` multiplies across attributes during tuple enumeration, giving an end-to-end effective weight per bound super-event equal to the product of its $n$ constituent events' weights. Inputs may have any $K_a \ge 1$: when $K_a > 1$, each output attribute carries the full $K_a$-slot vector of the underlying event, preserving within-attribute slot exchangeability per output attribute. Lag slots must be carried as separate attributes, not packed into a single multi-slot attribute, because lag identity is non-exchangeable (within-attribute multiset symmetry would otherwise collapse ordered tuples to unordered ones). See §3.1 (Preprocessing) for the rationale.
...
**translateAttributes(pAttr, groups, offsets, isRel, isPer, periods)**

Per-attribute preprocessing for MAET input. Shifts selected attributes' values by a chosen offset, returning a transformed `pAttrTranslated` that feeds directly into `buildExpTens`. Weights are unaffected and are not part of the signature. Semantics are by group geometry: absolute groups are translated as `value + mu` (unwrapped — the periodic kernel handles wrap downstream when `isPer[g] = True`); relative groups pass through unchanged and emit a single `translateAttributes:noOpRelative` (MATLAB) / `TranslateAttributesNoOpWarning` (Python) per call.

`offsets` accepts two forms. The **numeric form** is a single numeric block with rows indexing attributes and columns indexing sweep positions; row count must be 1 (broadcast across all attributes) or $A$ (per-attribute). Permitted shapes: scalar / $1$-by-$1$ (broadcast, no sweep); $1$-by-$M$ row (broadcast, $M$-sweep); $A$-by-$1$ column (per-attribute, single translation); $A$-by-$M$ matrix (per-attribute, $M$-sweep). The **container form** is a `{group_index: value}` dict in Python, or a $1$-by-$G$ cell in MATLAB; each per-group value follows the same orientation convention with $n_g$ playing the role of $A$ (scalar broadcasts within group; $1 \times M$ row broadcasts within group with $M$-sweep; $n_g \times 1$ column is per-attribute within group; $n_g \times M$ matrix is per-attribute within group with $M$-sweep). Groups omitted (Python: missing key; MATLAB: empty `[]` cell) are not translated. All entries carrying $M > 1$ across the call must agree on $M$; scalar and 1-column entries broadcast across the sweep. NaN entries in any numeric block skip the corresponding `(attribute, column)` cell.

When any input implies a sweep ($M > 1$) or any 2-D shape is used, the output is a length-$M$ list (Python) / $1$-by-$M$ cell (MATLAB) of per-attribute blocks; otherwise a single per-attribute block. The matrix-mode output is the natural input to the raw-MA scalar-vs-list mode of `cosSimExpTens` (§6.1 above) for a one-call sliding-comparison sweep. See §3.1 (Attribute translation) for the conceptual framing of pre-tensor versus post-tensor sliding.

**weightEvents(pAttr, w, groups, inputAttr, targetAttr, centre, width, shape, isPer, period, 'deleteInput', tf)**

Per-event preprocessing for MAET input. Computes a window factor from one attribute's values and multiplies it into the weight slot of another attribute, returning a transformed three-tuple `(pAttr, wOut, groups)` that feeds directly into `buildExpTens`. The window is specified by a centre $c$, a width $w$ (the standard deviation, in absolute units of the input attribute), and a shape parameter $\gamma \in [0, 1]$ interpolating between pure Gaussian ($\gamma = 0$) and pure rectangle ($\gamma = 1$); the factor is the peak-normalised convolution of a rectangle of half-width $w \sqrt{3\gamma}$ and a Gaussian of standard deviation $w \sqrt{1 - \gamma}$, so the window's total variance is $w^2$ for every value of $\gamma$.

`inputAttr` and `targetAttr` are scalar attribute indices (1-based MATLAB / 0-based Python). `inputAttr` names the attribute whose values are passed to the window function; `targetAttr` names the attribute whose weight slot receives the resulting per-event factor. They may be equal or distinct. `inputAttr` must reference an attribute with $K_{\text{input}} = 1$ (single value per event); the factor is a $(1, N)$ row and broadcasts across the target attribute's $K_{\text{target}}$ slots downstream. `centre`, `width`, and `shape` are scalars. `isPer` is a scalar boolean and must mirror the `[per]` flag of `inputAttr`'s group in the downstream `buildExpTens` call; `period` is the corresponding scalar period (ignored when `isPer = false`). When `isPer = true`, the difference $\delta = v - c$ used inside the window function is wrapped to $[-\text{period}/2, \text{period}/2]$ before the shape function is evaluated; stored values are not wrapped.

`deleteInput` is a mandatory keyword-only boolean (no default). When `true` and `inputAttr != targetAttr`, the input attribute is dropped from the returned `pAttr`, `wOut`, and `groups`; if the input was the sole member of its group, that group disappears and higher group indices are decremented to keep numbering contiguous. When `false`, the input attribute is preserved unchanged in the output. `deleteInput = true` paired with `inputAttr == targetAttr` raises an error (the just-written factor would be discarded).

Multi-axis windowing is expressed as a sequence of `weightEvents` calls with the same `targetAttr`; each call's factor multiplies into the target's existing weight slot.

The canonical composition `weightEvents` (with `deleteInput=true`, `inputAttr` typically time, `targetAttr` typically pitch) $\to$ `buildExpTens` $\to$ `entropyExpTens` is the windowed-entropy construction of §3.7.5. `weightEvents` chains cleanly with `translateAttributes` under a centre-shift identity ($\mathcal{T}_\mu \circ \mathcal{W}_c = \mathcal{W}_{c+\mu} \circ \mathcal{T}_\mu$ on overlapping targets) and passes through `differenceEvents` / `bindEvents`.
