...
| `demo_tempoInvariance` | Tempo invariance by degree: `intervalKernelCov`'s ridge against the exact relative quotient, on logarithmic inter-onset intervals, with a `windowedSimilarity` sweep | — |
| `demo_translateSweep` | Pre-tensor sliding-comparison sweep: `translateAttributes` drives a 2-D (pitch transposition, time shift) sweep of a 3-note motif against an embedded-occurrence reference, scored in one call via the raw-MA scalar-vs-list mode of `cosSimExpTens`; closes with parity checks against the explicit `buildExpTens` loop and against `sweepCosSimExpTens` | — |
...
| `demo_batch_processing.py` | `demo_batchProcessing` | Analysing experimental data: per-trial paired and single-set features, with internal dedup of the trial matrix |
...
| `demo_translate_sweep.py` | `demo_translateSweep` | Pre-tensor sliding-comparison sweep via `translate_attributes` and the raw-MA scalar-vs-list mode of `cos_sim_exp_tens`, with a `sweep_cos_sim_exp_tens` parity check |
