...
%   as-is rather than developing precomputed contraction paths or a sharper
...
%   Python output for analysis.
    ...
    for ci = 1:numel(configs)
        ...
        fprintf('matlab,inner_product_orbit_pw_batched,%d,%d,%d,%.4f\n', ...
                r, K, P, t_batched);
        fprintf('matlab,sim_maet_sa_orbit,%d,%d,1,%.4f\n', ...
                r, K, t_cossim);
    end
end
...
function t = benchCosSimSingleMultisetOrbit(r, K, nReps, nWarmup)
    ...
    w  = ones(K, 1);
    t = timed(@() simMaet(p1, w, p2, w, 30, r, false, false, 0, ...
